...
`[F : k(x)] = 1`; in large degree Riemann--Roch evaluates the left-hand side as `l + 1 - g`, so
`g ≤ 0`.  Together with
...
theorem genus_eq_zero_of_adjoin_eq_top (hF : IsFunctionField k F)
  ...
  have hpoles : Divisor.degree (Divisor.poles hF (Units.mk0 x hx0)) = 1 := by
    ...
    exact_mod_cast IntermediateField.finrank_eq_one_iff_eq_top.mpr (by simpa using htop)
