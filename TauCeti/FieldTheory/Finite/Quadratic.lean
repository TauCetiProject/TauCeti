...
This file proves that the binary form `x ^ 2 - a * y ^ 2` attached to a nonzero `a` is universal
over a finite field.

In odd characteristic this is a counting argument: a quadratic polynomial over a finite field of
odd order takes more than half of the values of the field, so the value sets of `x ^ 2` and of
`a * y ^ 2 + b` cannot be disjoint. In characteristic two squaring is onto, so already
