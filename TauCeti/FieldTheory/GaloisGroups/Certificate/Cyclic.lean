...
* `TauCeti.hasSecondRootInRootField_cyclicQuintic`: the second-root evidence.
* `Polynomial.irreducible_cyclicQuintic_zmod_two`: the irreducible reduction at `2`.
* `Polynomial.factorDegrees_cyclicQuintic_two`: the irreducible reduction at `2`.
...
theorem _root_.Polynomial.irreducible_cyclicQuintic_zmod_two :
  ...
  · have : Fact (Nat.Prime 31) := ⟨by decide⟩
    ...
    rw [h3, h5]
    ring
  · rw [show (X ^ 5 + X ^ 4 + X ^ 2 + X + 1 : (ZMod 2)[X]).natDegree = 5 by
      compute_degree!]
    symm
    change orderOf (ZMod.unitOfCoprime 2 (by decide : Nat.Coprime 2 31)) = 5
    ...
    · intro p hp hpdvd
      ...
      exact (show (2 : ZMod 31) ≠ 1 by decide) h'

/-- The cyclic quintic has a single irreducible factor of degree five modulo `2`. -/
theorem _root_.Polynomial.factorDegrees_cyclicQuintic_two :
  ...
  rw [hmap]
  constructor
  · exact Polynomial.irreducible_cyclicQuintic_zmod_two
  · compute_degree!
...
theorem QuinticCertificate.check_cyclicQuintic :
  ...
  have hirr : Irreducible ((X ^ 5 + X ^ 4 - 4 * X ^ 3 - 3 * X ^ 2 + 3 * X + 1 : ℤ[X]).map
    ...
    (Polynomial.factorDegrees_eq_singleton_iff.mp Polynomial.factorDegrees_cyclicQuintic_two).1
  ...
  exact ⟨HasFactorDegrees.mk hgood Polynomial.factorDegrees_cyclicQuintic_two,
