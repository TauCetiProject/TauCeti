...
For an algebraic `M / K` two facts hold for *every* intermediate field, with no separability
anywhere: its fixing subgroup is closed in the Krull topology, being the intersection over the
finite simple subextensions of their open fixing subgroups; and it is unchanged by cutting the
field down to its part inside `separableClosure K M`, because every element has a `q`-th power
iterate there. The second is why a Galois correspondence over an inseparable extension can only be
indexed by the intermediate fields of the separable closure.
...
/-- **Over an algebraic extension every fixing subgroup is closed** for the Krull topology: `E` is
the join of the simple extensions it contains, each of them finite over `K`, so `E.fixingSubgroup`
is the intersection of the open subgroups `K⟮x⟯.fixingSubgroup`.
...
`M / K`. Algebraicity of `M / K` alone suffices, which is what an inseparable extension can still
supply; the suffix names that hypothesis, as `MulAction.stabilizer_isOpen_of_isIntegral` does. -/
theorem fixingSubgroup_isClosed_of_isAlgebraic [Algebra.IsAlgebraic K M]
    (E : IntermediateField K M) : IsClosed (E.fixingSubgroup : Set (M ≃ₐ[K] M)) := by
  ...
  have : FiniteDimensional K K⟮(x : M)⟯ :=
    adjoin.finiteDimensional (Algebra.IsAlgebraic.isAlgebraic (x : M)).isIntegral
  exact fixingSubgroup_isClosed _

/-- **A fixing subgroup sees only the separable closure.** For an algebraic extension `M / K` a
`K`-automorphism of `M` fixing `E ⊓ separableClosure K M` pointwise already fixes the intermediate
field `E` pointwise, because every `x ∈ E` has a power `x ^ q ^ n` in that intersection, `q` the
...
fixing subgroups and fields: it has the same fixing subgroup as its separable part. -/
theorem fixingSubgroup_inf_separableClosure [Algebra.IsAlgebraic K M]
  ...
  have hS : ExpChar (separableClosure K M) (ringExpChar K) :=
    expChar_of_injective_algebraMap (algebraMap K (separableClosure K M)).injective _
  refine le_antisymm (fun σ hσ ↦ ?_) (fixingSubgroup_antitone inf_le_left)
  rw [mem_fixingSubgroup_iff] at hσ ⊢
  intro x hx
  obtain ⟨n, y, hy⟩ := IsPurelyInseparable.pow_mem (separableClosure K M) (ringExpChar K) x
  have hyS : x ^ ringExpChar K ^ n ∈ separableClosure K M := by rw [← hy]; exact y.2
