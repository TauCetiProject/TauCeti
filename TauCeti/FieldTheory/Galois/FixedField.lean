...
theorem fixedField_sup (H H' : Subgroup (M ≃ₐ[K] M)) :
    fixedField (H ⊔ H') = fixedField H ⊓ fixedField H' := by
  apply le_antisymm
  · exact le_inf (fixedField_antitone le_sup_left) (fixedField_antitone le_sup_right)
  · rw [IntermediateField.le_iff_le, sup_le_iff]
    exact ⟨(IntermediateField.le_iff_le _ _).mp inf_le_left,
      (IntermediateField.le_iff_le _ _).mp inf_le_right⟩
...
direction, like `Subgroup.fixedField_sup`, needs no finiteness or Galois hypothesis. -/
@[simp]
theorem fixedField_iSup {I : Sort*} (H : I → Subgroup (M ≃ₐ[K] M)) :
    fixedField (⨆ i, H i) = ⨅ i, fixedField (H i) := by
  apply le_antisymm
  · exact le_iInf fun i ↦ fixedField_antitone (le_iSup H i)
  · rw [IntermediateField.le_iff_le, iSup_le_iff]
    exact fun i ↦ (IntermediateField.le_iff_le _ _).mp (iInf_le _ i)
...
finite Galois extension, intersecting the point stabilizers of `x i` cuts out
`K(Set.range x)`. This is the family form of `fixedField_stabilizer_eq_adjoin_simple`. -/
@[simp]
theorem fixedField_iInf_stabilizer_eq_adjoin_range [FiniteDimensional K M] [IsGalois K M]
    {I : Sort*} (x : I → M) :
    fixedField (⨅ i, MulAction.stabilizer (M ≃ₐ[K] M) (x i)) =
      adjoin K (Set.range x) := by
  rw [Subgroup.fixedField_iInf]
  simp_rw [fixedField_stabilizer_eq_adjoin_simple]
  rw [← adjoin_iUnion]
  congr 1
  ext y
  simp only [Set.mem_iUnion, Set.mem_singleton_iff, Set.mem_range]
