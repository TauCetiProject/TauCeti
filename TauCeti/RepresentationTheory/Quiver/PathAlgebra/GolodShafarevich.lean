...
private abbrev PathInto (R : Type u) [Quiver.{v} R] (n : ℕ) (j : R) : Type _ :=
...
private instance finite_pathInto [Finite R] [∀ a b : R, Finite (a ⟶ b)] (n : ℕ) (j : R) :
...
private theorem card_arrow_mul_card_pathInto_le [Fintype R] [∀ a b : R, Fintype (a ⟶ b)]
    (n : ℕ) (j : R) :
...
private theorem vertexIdempotent_mul_of_mem_pathsInto {n : ℕ} {j : R} {x : pathAlgebra k R}
...
private theorem vertexIdempotent_mul_mem_pathsInto {n : ℕ} (j : R) {x : pathAlgebra k R}
...
`n` into `i`. -/
private theorem exists_eq_sum_ofArrow_mul [Fintype R] [∀ a b : R, Fintype (a ⟶ b)] {n : ℕ} {j : R}
