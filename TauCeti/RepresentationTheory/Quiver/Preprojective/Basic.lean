...
* `TauCeti.gaugedLocalPreprojectiveRelator`: the local relator at a vertex with
  scalar-labelled arrow contributions.
...
* `TauCeti.sum_gaugedLocalPreprojectiveRelator`: the same for a gauged relator.
...
/-- The **gauged local preprojective relator** at a vertex `v`,
...
noncomputable def gaugedLocalPreprojectiveRelator (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k) (v : Q) :
  ...
  (∑ i : Q, ∑ a : (i ⟶ v), ε a • headBacktrackElem k a) -
    ∑ j : Q, ∑ a : (v ⟶ j), ε a • tailBacktrackElem k a

/-- The gauged local relator at a vertex, by its defining sum. This is the defining equation,
exposed for use outside this module. -/
theorem gaugedLocalPreprojectiveRelator_def (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k) (v : Q) :
    gaugedLocalPreprojectiveRelator k ε v =
      (∑ i : Q, ∑ a : (i ⟶ v), ε a • headBacktrackElem k a) -
        ∑ j : Q, ∑ a : (v ⟶ j), ε a • tailBacktrackElem k a := by
  rw [gaugedLocalPreprojectiveRelator]

/-- **The constant gauge `1` is the local preprojective relator.** -/
@[simp]
theorem gaugedLocalPreprojectiveRelator_one (v : Q) :
    gaugedLocalPreprojectiveRelator k (fun _ _ _ => (1 : k)) v = localPreprojectiveRelator k v := by
  simp only [gaugedLocalPreprojectiveRelator, localPreprojectiveRelator, one_smul]

/-- **The gauged global relator is the sum of the gauged local ones**, so the family of gauged
local relations carries the same information as the single gauged global one. -/
theorem sum_gaugedLocalPreprojectiveRelator (ε : ∀ ⦃i j : Q⦄, (i ⟶ j) → k) :
    ∑ v : Q, gaugedLocalPreprojectiveRelator k ε v = gaugedPreprojectiveRelator k ε := by
  simp only [gaugedLocalPreprojectiveRelator, gaugedPreprojectiveRelator_def, smul_sub,
