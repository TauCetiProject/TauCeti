...
omit [Fintype G] in
/-- If `g` generates `G`, the invariants of a representation are the kernel of `ρ(g) - 𝟙`. -/
theorem invariants_eq_ker_applyAsHom_sub (hg : ∀ x, x ∈ Subgroup.zpowers g) :
    M.ρ.invariants = ker (applyAsHom M g - 𝟙 M).hom.toLinearMap := by
  ext x
  simpa [sub_hom, sub_eq_zero] using
...
def tateCohomologyIso₀ (hg : ∀ x, x ∈ Subgroup.zpowers g) :
    tateCohomology M 0 ≅ (normHomCompSub M g).homology := calc
  ...
  _ ≅ (normHomCompSub M g).moduleCatLeftHomologyData.H := by
    refine (Submodule.Quotient.equiv _ _
      (LinearEquiv.ofEq _ _ (invariants_eq_ker_applyAsHom_sub M g hg)) ?_).toModuleIso
...
def tateCohomologyIsoNegOne (hg : ∀ x, x ∈ Subgroup.zpowers g) :
    tateCohomology M (-1) ≅ (subCompNormHom M g).homology := calc
