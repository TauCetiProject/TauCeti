...
and multiplying its value by `n`. The scalar field may be real or complex.
...
theorem tendsto_nsmul_apply_div_of_hasDerivAt
    {𝕜 F : Type*} [RCLike 𝕜] [NormedAddCommGroup F] [NormedSpace 𝕜 F]
  ...
  have hs_ne : ∀ᶠ n : ℕ in (atTop : Filter ℕ), t / (n : 𝕜) ≠ 0 := by
    ...
    exact div_ne_zero ht (show (n : 𝕜) ≠ 0 from
  ...
  · subst n
    simp [hf0]
  rw [show n • f (t / (n : 𝕜)) = (n : 𝕜) • f (t / (n : 𝕜)) by
    exact (Nat.cast_smul_eq_nsmul 𝕜 n _).symm]
