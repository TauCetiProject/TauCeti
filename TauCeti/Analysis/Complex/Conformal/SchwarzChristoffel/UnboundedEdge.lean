...
The proof combines the finite closed-edge formula with the boundary limit at infinity.  The norm of
the displacement from the initial endpoint is a continuous strictly increasing parameter, and its
image is the half-open interval between zero and its limiting value.  This identifies the image of
the unbounded edge without yet asserting that the interior of the upper half-plane is the polygon
interior.
...
theorem schwarzChristoffelBoundary_image_Ici (a e : ι → ℝ) (z₀ : UpperHalfPlane) {p : ℝ}
  ...
  have hdmono : StrictMonoOn d (Ici p) := by
    ...
    have hinj := schwarzChristoffelBoundary_injOn_Icc a e z₀ (hfree hy) hp (hsum hy)
    have hne : B y ≠ B x := by
      intro h
      have : y = x := hinj ⟨hy, le_rfl⟩ ⟨hx, hxy.le⟩ h
  ...
  have hscalar : (fun t : ℝ => B p + (t : ℂ) * u) '' Ico 0 D =
    ...
    · rintro z ⟨t, ⟨ht0, htD⟩, rfl⟩
      ...
      · intro h
        apply htD.ne
        have hEq : B p + ((t : ℂ) * u) = V := by simpa using h
        have hu := Complex.exp_ne_zero (schwarzChristoffelEdgeAngle a e p * Complex.I)
        have hmul : (t : ℂ) * u = (D : ℂ) * u := by
          calc
            (t : ℂ) * u = B p + (t : ℂ) * u - B p := by abel
            _ = V - B p := by rw [hEq]
            _ = (D : ℂ) * u := hVdir
        exact_mod_cast (mul_right_cancel₀ hu hmul)
    · intro z hz
      rcases hz with ⟨hzseg, hzVnot⟩
