...
This is one part of identifying the image with the polygon interior. The analogous avoidance of
the bounded sides and the resulting global image and injectivity statements require further
polygon geometry.
...
theorem im_schwarzChristoffelVertex_zero_lt_primitive
  ...
  let P := (schwarzChristoffelPolygon a e z₀).boundary ℝ
  ...
  have hP : P = range (schwarzChristoffelCompactifiedBoundary a e z₀) :=
    (range_schwarzChristoffelCompactifiedBoundary a e z₀ ha.monotone hfinite hinfty).symm
  have hconv : closedConvexHull ℝ P ⊆ H :=
  ...
  have hmem : F z ∈ interior (closedConvexHull ℝ P) := by
    rw [hP]
    exact image_schwarzChristoffelPrimitive_subset_interior_closedConvexHull
      a e z₀ hfinite hinfty (mem_image_of_mem F hz)
  have hInterior : interior (closedConvexHull ℝ P) ⊆ interior H :=
    interior_mono hconv
  have hmemH := hInterior hmem
  simpa only [H, interior_setOfPred_le_im, mem_ofPred_eq] using hmemH
...
theorem disjoint_image_schwarzChristoffelPrimitive_closingSides
  ...
  intro w hw hside
  obtain ⟨z, hz, rfl⟩ := hw
  have hwlt := im_schwarzChristoffelVertex_zero_lt_primitive a e z₀ ha he hsum hz
  ...
  · have hle := (convex_halfSpace_im_le _).segment_subset
      hlast.le hinfty.le hside
