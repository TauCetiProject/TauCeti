...
The solution is symmetric and, when both `S` and `T` are positive definite, positive definite
itself, so it also solves the congruence equation `A * S * Aᴴ = T`; its inverse is the solution
`TauCeti.geometricMean T⁻¹ʳ S` of the reversed equation.

## Main results

* `Matrix.PosDef.existsUnique_posSemidef_mul_mul`: existence and uniqueness of that matrix.
* `Matrix.PosDef.geometricMean_ringInverse_eq_sqrt_mul_mul_sqrt`: the solution is the congruence
...
/-- **The standard positive matrix.** For a positive-definite `S` the solution `A` of
`A * S * A = T` is the congruence `(√S)⁻¹ * √(√S * T * √S) * (√S)⁻¹` of the positive square
root of `√S * T * √S` by the inverse `(√S)⁻¹` of the positive square root of `S`. -/
theorem PosDef.geometricMean_ringInverse_eq_sqrt_mul_mul_sqrt (hS : S.PosDef) :
  ...
  rw [show S⁻¹ʳ = S⁻¹ from (Matrix.nonsing_inv_eq_ringInverse S).symm,
    geometricMean_eq_sqrt_mul_mul, ← Matrix.nonsing_inv_eq_ringInverse,
    ← hS.posSemidef.inv_sqrt, Matrix.nonsing_inv_nonsing_inv (A := CFC.sqrt S) hSdet]
...
needed, so in particular this applies to the solution `A` of `A * S * A = T` for every `S` and
`T`. -/
...
/-- **The congruence form.** The solution `A` of `A * S * A = T` is Hermitian by
`isHermitian_geometricMean`, so it solves the congruence `A * S * Aᴴ = T` as well. This is the
form in which a covariance matrix transforms under a change of variables; over the reals `Aᴴ` is
the transpose `Aᵀ`. -/
theorem PosDef.mul_mul_conjTranspose_geometricMean (hS : S.PosDef) (hT : T.PosDef) :
    geometricMean S⁻¹ʳ T * S * (geometricMean S⁻¹ʳ T)ᴴ = T := by
  rw [isHermitian_geometricMean.eq,
    geometricMean_ringInverse_mul_mul_geometricMean_ringInverse hS.isStrictlyPositive
      hT.posSemidef.nonneg]
