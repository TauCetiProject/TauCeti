...
/-!
# Diagonal energy lower bounds under nonnegative perturbations

`TauCeti.Analysis.PDE.CoerciveEnergy` proves explicit pointwise diagonal lower bounds for
the finite-dimensional jet integrand `energyIntegrand A b c` from a principal lower bound,
a drift bound, and a mass floor that dominates the drift defect. This file records the
corresponding monotonicity: adding a principal coefficient with nonnegative quadratic form,
and adding a nonnegative mass coefficient, preserves those explicit lower bounds.

These remain pointwise estimates on `ℝ × EuclideanSpace ℝ n`, not coercivity of a bilinear
form in the sense of `IsCoercive` and not integrated Sobolev-space estimates.
...
* `TauCeti.PDE.min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_add_principal_mass_self`:
  the explicit diagonal lower bound after such a perturbation.
...
noncomputable local instance diagonalLowerBoundPerturbationDecidableEq : DecidableEq n :=
  Classical.decEq n
...
/-- The explicit diagonal lower bound remains true after adding a nonnegative principal
quadratic form and a nonnegative mass coefficient. -/
lemma min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_add_principal_mass_self
    (hlam : 0 < lam)
  ...
  min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_self hlam
...
/-- The explicit diagonal lower bound for a uniformly elliptic principal coefficient after
adding a nonnegative principal matrix and nonnegative mass perturbation. -/
lemma min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_add_principal_mass_self
  ...
  PDE.min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_add_principal_mass_self h.pos
    (h.lower_bound hx) hb hc hmu hB hd U

/-- The coefficient-field version of the explicit diagonal lower bound after adding
nonnegative principal and mass perturbation fields. -/
lemma min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_add_principal_mass_self_on
  ...
  h.min_diagonalLowerBound_mul_norm_sq_le_energyIntegrand_add_principal_mass_self hx
