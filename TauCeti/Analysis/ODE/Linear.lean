...
and therefore belongs to neither asymptotic set unless it vanishes.

## Main declarations

* `ContinuousLinearMap.flow`: the flow `exp (t A)` of a bounded linear operator.
* `LinearMap.IsSymmetric.stableSet_flow_neg_eq_positiveSpectralSubspace`: the stable set of zero
  for `exp (-t T)` is the positive spectral subspace of `T`.
* `LinearMap.IsSymmetric.unstableSet_flow_neg_eq_negativeSpectralSubspace`: the corresponding
...
theorem stableSet_flow_neg_eq_positiveSpectralSubspace
...
theorem unstableSet_flow_neg_eq_negativeSpectralSubspace
    (hT : (T : E →ₗ[ℝ] E).IsSymmetric) (hn : Module.finrank ℝ E = n) :
