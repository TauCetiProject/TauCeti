...
For a real `0 ≤ x ≤ 1/2`, the quantity `-log (1 - x)` is at most `x + 2 x ^ 2`. Together
with the lower bound supplied by Mathlib's `Real.log_le_sub_one_of_pos`, this compares the
logarithm of an Euler factor `(1 - x)⁻¹` with its local ratio `x`.
