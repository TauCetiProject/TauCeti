...
private theorem ae_bdd_abs_condExp_le_of_ae_bdd_abs {Ω : Type*} [MeasurableSpace Ω]
    {m m0 : MeasurableSpace Ω} {μ : Measure Ω} {R : NNReal} {f : Ω → ℝ}
    (hbdd : ∀ᵐ x ∂μ, |f x| ≤ R) :
    ∀ᵐ x ∂μ, |(μ[f | m]) x| ≤ R := by
  by_cases hnm : m ≤ m0
  swap
...
never contractability.  The two support lemmas stay `private`; the contraction-independence
    ...
    have h2 : ∫ y in B, g₂ y ∂(Measure.map W μ) = ∫ ω in W' ⁻¹' B, φ ω ∂μ := by
      ...
      calc ∫ y in B, g₂ y ∂(Measure.map W' μ)
        ...
        _ = ∫ ω in W' ⁻¹' B, μ₂ ω ∂μ :=
              setIntegral_congr_fun (hW' hB) fun ω _ => (congrFun hμ₂_eq ω).symm
        _ = ∫ ω in W' ⁻¹' B, φ ω ∂μ :=
              setIntegral_condExp hmW'_le hφ_int
                (MeasurableSpace.measurableSet_comap.mpr ⟨B, hB, rfl⟩)
...
theorem condExp_indicator_eq_of_law_eq_of_comap_le [IsFiniteMeasure μ]
  ...
  -- Mathlib's `ae_bdd_abs_condExp_of_ae_bdd_abs`; the helper is applied once per σ-algebra below.
  ...
  have condExp_abs_le : ∀ m' : MeasurableSpace Ω, ∀ᵐ ω ∂μ, |μ[φ | m'] ω| ≤ 1 := fun m' => by
    simpa using ae_bdd_abs_condExp_le_of_ae_bdd_abs (m := m') (R := 1) hφ_abs
