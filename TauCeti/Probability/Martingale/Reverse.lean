...
- `revCEFinite_martingale`: the reversed process is a forward martingale for `revFiltration`.
...
property, `μ[μ[f | 𝔽 (N - j)] | 𝔽 (N - i)] = μ[f | 𝔽 (N - i)]` whenever `i ≤ j`. -/
lemma revCEFinite_martingale [IsProbabilityMeasure μ]
