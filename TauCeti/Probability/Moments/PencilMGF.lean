...
/-- The origin lies in the domain of a pencil product, so a random variable whose exponential
moments are finite there has finite moments of every order and an analytic transform at the
origin. -/
...
private theorem cgf_eventuallyEq_sum_log
  ...
  filter_upwards [(isOpen_setOf_forall_mul_lt_one lam).mem_nhds
    (show (0 : ℝ) ∈ {t : ℝ | ∀ j, 2 * t * lam j < 1} from fun j => by norm_num)] with t ht
...
/-- The derivative of the cumulant-generating function of a pencil transform, on the domain of
the product. -/
private theorem hasDerivAt_sum_log {t : ℝ} (ht : ∀ j, 2 * t * lam j < 1) :
...
/-- The second derivative of the cumulant-generating function of a pencil transform at the
origin, where every factor of the product equals one. -/
private theorem hasDerivAt_sum_div (lam a : ι → ℝ) :
...
theorem integral_eq_of_mgf_eq_prod_rpow (hX : 0 ∈ interior (integrableExpSet X μ))
  ...
  rw [← hderiv, (cgf_eventuallyEq_sum_log hmgf).deriv_eq,
    (hasDerivAt_sum_log (a := a) (fun j => by norm_num)).deriv, Finset.mul_sum]
  refine Finset.sum_congr rfl fun j _ => ?_
  rw [show (1 : ℝ) - 2 * 0 * lam j = 1 by ring, div_one]
  ring
  ...
  have hsecond : deriv (deriv (cgf X μ)) 0 = ∑ j, -a j * -(4 * lam j ^ 2) := by
    have hdderiv : deriv (cgf X μ) =ᶠ[𝓝 0]
      ...
      filter_upwards [(cgf_eventuallyEq_sum_log hmgf).eventually_nhds,
        (isOpen_setOf_forall_mul_lt_one lam).mem_nhds
          (show (0 : ℝ) ∈ {t : ℝ | ∀ j, 2 * t * lam j < 1} from fun j => by norm_num)]
        with s hs hst
