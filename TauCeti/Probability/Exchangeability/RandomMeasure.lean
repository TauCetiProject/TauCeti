...
This is the first measure-valued factorization used in the hard direction of the Aldous--Hoover
representation.  It does not claim that the coordinate marginals determine `P`: the later
factorization must retain the higher finite-dimensional marginals in order to recover the cell
noise.

## Main definitions and results

* `TauCeti.Probability.coordinateMarginals` -- the path of one-coordinate marginals of a path law;
...
* `TauCeti.Probability.codedCoordinateMarginals` -- the same marginals in a standard Borel code;
...
theorem fullyExchangeable_coordinateMarginals_of_invariant
  ...
  have hfun : (fun P : ProbabilityMeasure (ℕ → α) => fun i => coordinateMarginals P (τ i)) =
  ...
  calc
    π.map (fun P => fun i => coordinateMarginals P (τ i)) =
        π.map (coordinateMarginals ∘ fun P => P.map (permReindex τ)) := by rw [hfun]
...
theorem exchangeable_coordinateMarginals_of_invariant
  ...
  (fullyExchangeable_coordinateMarginals_of_invariant π hπ).exchangeable
    fun _ => ((measurable_pi_apply _).comp measurable_coordinateMarginals).aemeasurable
...
def codedCoordinateMarginals [MeasurableSpace.CountablyGenerated α]
    ...
    ℕ → (TauCeti.MeasureTheory.probabilityMeasureCodeIndex α → ℝ≥0∞) :=
  fun i => TauCeti.MeasureTheory.probabilityMeasureCode (coordinateMarginals P i)
...
theorem codedCoordinateMarginals_apply [MeasurableSpace.CountablyGenerated α]
    ...
    codedCoordinateMarginals P i =
      TauCeti.MeasureTheory.probabilityMeasureCode (coordinateMarginals P i) :=
  (rfl)
...
theorem measurable_codedCoordinateMarginals [MeasurableSpace.CountablyGenerated α] :
  ...
  Measurable.of_eval fun i => Measurable.of_eval fun s => by
    have hmeas :=
      ((Measure.measurable_coe
        (TauCeti.MeasureTheory.measurableSet_probabilityMeasureCodeIndex s)).comp
          measurable_subtype_coe).comp
        ((measurable_pi_apply i).comp measurable_coordinateMarginals)
    convert hmeas using 1
    funext P
    rw [codedCoordinateMarginals_apply,
      TauCeti.MeasureTheory.probabilityMeasureCode_apply]
    rfl
...
theorem codedCoordinateMarginals_map_permReindex [MeasurableSpace.CountablyGenerated α]
  ...
  exact congrArg TauCeti.MeasureTheory.probabilityMeasureCode
  ...
  by
    simpa only [codedCoordinateMarginals_apply] using
      (exchangeable_coordinateMarginals_of_invariant π hπ).map_values
        TauCeti.MeasureTheory.measurable_probabilityMeasureCode
        fun _ => ((measurable_pi_apply _).comp measurable_coordinateMarginals).aemeasurable
...
theorem conditionallyIID_codedCoordinateMarginals_of_invariant
  ...
  conditionallyIID_of_exchangeable
    ...
    fun _ => ((measurable_pi_apply _).comp measurable_codedCoordinateMarginals).aemeasurable
...
theorem exists_pathLaw_codedCoordinateMarginals_eq_map_unitIntervalCoding
    ...
    ∃ Λ : ProbabilityMeasure
        (ProbabilityMeasure (TauCeti.MeasureTheory.probabilityMeasureCodeIndex α → ℝ≥0∞)),
      pathLaw π (fun i P => codedCoordinateMarginals P i) =
        ((Λ : Measure (ProbabilityMeasure
            (TauCeti.MeasureTheory.probabilityMeasureCodeIndex α → ℝ≥0∞))).prod
          (Measure.infinitePi fun _ : ℕ => (volume : Measure I))).map
            fun q i => unitIntervalCoding
              (TauCeti.MeasureTheory.probabilityMeasureCodeIndex α → ℝ≥0∞) q.1 (q.2 i) :=
