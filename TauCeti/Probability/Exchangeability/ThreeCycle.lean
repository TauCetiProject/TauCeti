...
Take the base space `Ω = ZMod 3` with its uniform probability law
`threeCycleMeasure = (3 : ℝ≥0∞)⁻¹ • Measure.count`, and the deterministic rotation process
`threeCycle n ω = ω + n`. Starting from a uniform state and stepping by the 3-cycle, the process
is stationary: its path law is invariant under the one-sided shift
...
`n`-fold rotation `ω + n`. -/
@[expose]
def threeCycle : ℕ → ZMod 3 → ZMod 3 := fun n ω => ω + (n : ZMod 3)
...
@[expose]
def threeCycleMeasure : Measure (ZMod 3) := (3 : ℝ≥0∞)⁻¹ • Measure.count

instance : threeCycleMeasure.IsAddRightInvariant := by
  change Measure.IsAddRightInvariant ((3 : ℝ≥0∞)⁻¹ • Measure.count)
  infer_instance

instance : IsProbabilityMeasure threeCycleMeasure where
  measure_univ := by
    have hc : (Measure.count : Measure (ZMod 3)) Set.univ = 3 := by
      simp [Measure.count_univ, ENat.card_eq_coe_fintype_card, ZMod.card]
    change ((3 : ℝ≥0∞)⁻¹ • Measure.count) Set.univ = 1
    rw [Measure.smul_apply, smul_eq_mul, hc, ENNReal.inv_mul_cancel] <;> norm_num

/-- The uniform law gives mass `3⁻¹` to each singleton. -/
theorem threeCycleMeasure_singleton (a : ZMod 3) : threeCycleMeasure {a} = 3⁻¹ := by
  rw [threeCycleMeasure, Measure.smul_apply, Measure.count_singleton, smul_eq_mul, mul_one]
...
/-- Swapping the two coordinates makes the event `{X₁ = 0, X₀ = 1}` unrealizable: it would need
`ω + 1 = 0` and `ω = 1` simultaneously. -/
private theorem threeCycle_swapped_event :
    {ω : ZMod 3 | ∀ i : Fin 2,
        threeCycle ((Equiv.swap 0 1 i).val) ω ∈ (![{0}, {1}] : Fin 2 → Set (ZMod 3)) i} = ∅ := by
  ext ω
  simp only [Set.mem_setOf_eq, Fin.forall_fin_two, Matrix.cons_val_zero, Matrix.cons_val_one,
...
/-- **The 3-cycle is not exchangeable.** Testing the two-dimensional law on the rectangle
`{X₀ = 0, X₁ = 1}`, the identity permutation gives mass `3⁻¹` while the transposition of the first
two coordinates gives mass `0`, so `ExchangeableAt` fails at `n = 2`. -/
theorem threeCycle_not_exchangeable : ¬ Exchangeable threeCycleMeasure threeCycle := by
  intro hE
  have h := hE 2 (Equiv.swap 0 1)
  ...
  rw [blockLaw_apply_rectangle threeCycleMeasure threeCycle
        (fun i : Fin 2 => (Equiv.swap 0 1 i).val)
      ...
      blockLaw_apply_rectangle threeCycleMeasure threeCycle (fun i : Fin 2 => (i : ℕ))
        (fun _ => Measurable.of_discrete.aemeasurable) _ (fun _ => MeasurableSet.of_discrete)]
    at hval
  rw [threeCycle_swapped_event, threeCycle_straight_event, measure_empty,
