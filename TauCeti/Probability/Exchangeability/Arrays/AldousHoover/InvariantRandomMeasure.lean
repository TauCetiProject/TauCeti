...
/-- The invariant law supplied for a directing random measure, rewritten as invariance of its
pushforward law on `ProbabilityMeasure (ℕ → α)`. -/
private theorem map_directing_invariant
    {μ : Measure Ω} {X : ℕ → Ω → (ℕ → α)} {ν : Ω → ProbabilityMeasure (ℕ → α)}
    (hν : ConditionallyIIDWith μ X ν)
    (hinv : ∀ τ : Equiv.Perm ℕ,
