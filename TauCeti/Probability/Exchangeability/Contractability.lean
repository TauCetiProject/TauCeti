...
/-- Internal path-law reindexing helper for the contractability preservation proof. -/
...
theorem Contractable.measurePreserving_reindex {μ : Measure Ω} {X : ℕ → Ω → α} [IsFiniteMeasure μ]
  ...
  intro n
  rw [Measure.map_map (measurable_prefixProj n)
      (measurable_pi_lambda _ fun k => measurable_pi_apply (φ k)),
    Function.comp_def,
    map_reindex_prefixProj_pathLaw μ hX_meas φ n,
