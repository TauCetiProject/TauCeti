...
`n ≥ 2` the equation lies in `𝔪²`, so the maximal ideal `(π, x, y)` of the two-dimensional local
ring still needs three generators. For `n = 1` its constant coefficient `-π` does not lie in
`𝔪_R²`, so `π = xy` is redundant and the maximal ideal is generated by `x` and `y`. For `n = 0`
