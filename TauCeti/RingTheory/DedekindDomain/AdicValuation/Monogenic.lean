...
finite and `K` has characteristic zero, the completed extension `L_w/K_v` is separable and the
ring of integers `𝒪_w` is generated by one element over `𝒪_v`.
...
/-- A canonical extension of adic completions in characteristic zero is separable. -/
...
/-- The completed integer ring `𝒪_w` is generated over `𝒪_v` by an element which is also an
integral generator of the completed field extension `L_w/K_v`. -/
theorem exists_adicCompletionIntegers_generator [CharZero K] :
