...
prime of the top ring above a height-one prime `v` of the base. When the residue fields are
finite and `K` has characteristic zero, the completed extension `L_w/K_v` is separable. More
generally, when the completed extension is separable, the ring of integers `𝒪_w` is generated by
one element over `𝒪_v`.
...
/-- When both residue fields are finite and the completed field extension `L_w/K_v` is
separable, the completed integer ring `𝒪_w` is generated over `𝒪_v` by an element which is also
an integral generator of the field extension. -/
theorem exists_adicCompletionIntegers_generator
  ...
  have hcoe : (pb.gen : w.adicCompletion L) =
      algebraMap (w.adicCompletionIntegers L) (w.adicCompletion L) pb.gen := rfl
  rw [hcoe, ← IntermediateField.adjoin_toSubalgebra, hfield]
  rfl
