...
theorem centerIdeal_ne_bot [IsFractionRing R K] [w.IsNontrivial]
  ...
  -- `R` maps into the field `K`, so it is nontrivial; that is all the argument needs of `R`.
...
-- `IsDedekindDomain.HeightOneSpectrum` takes `[IsDedekindDomain R]` as a phantom parameter (it is
-- `@[nolint unusedArguments]` upstream), and now that `centerIdeal_ne_bot` no longer needs
-- `IsDomain R`, that instance is used here only to name the return type.
