...
* `IsDedekindDomain.HeightOneSpectrum.adicCompletionIntegers_isLocalHom`: the canonical map
  `𝒪_v → 𝒪_w` is a local ring homomorphism.
...
theorem adicCompletionIntegers_isLocalHom :
...
scoped[AdicCompletionExtension] attribute [instance]
  IsDedekindDomain.HeightOneSpectrum.adicCompletionIntegers_isLocalHom
