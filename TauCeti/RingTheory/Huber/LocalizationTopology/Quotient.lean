...
* The Tau Ceti roadmap, `TauCetiRoadmap/AdicSpaces/README.md`, Layer 4.1.
...
variable {A : Type*} [CommRing A] {T : Finset A} {s : A} {k : ℕ} {t : Fin k → A}

-- When every numerator is `s` or some `tᵢ`, the unit-ideal condition on `(T, s)` is inherited by
-- `s` and the `tᵢ`.
private theorem span_insert_range_eq_top (hsplit : ∀ u ∈ T, u = s ∨ u ∈ Set.range t)
    (hspan : Ideal.span (insert s (T : Set A)) = ⊤) : Ideal.span (insert s (Set.range t)) = ⊤ :=
  eq_top_mono (Ideal.span_mono <| Set.insert_subset (Set.mem_insert _ _) hsplit) hspan
...
private theorem existsUnique_continuous_ringHom_completion_rationalQuotient :
  ...
  exact existsUnique_continuous_ringHom_completion_locTopology P T s S hden
    ...
    (isUnit_rationalRelationIdeal_quotientMk_weightedC (span_insert_range_eq_top hsplit hspan))
...
private theorem completionToRationalQuotient_comp_rationalQuotientToCompletion :
  ...
  · refine (isUnit_rationalRelationIdeal_quotientMk_weightedC
      (span_insert_range_eq_top hsplit hspan)).mul_left_cancel ?_
