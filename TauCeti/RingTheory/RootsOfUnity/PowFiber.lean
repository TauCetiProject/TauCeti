...
The action is free away from `0`, which is fixed by every root of unity. This is the algebraic
half of the local model `u ↦ u ^ m` for the quotient of a disc by a finite rotation group: the
orbit map of the rotation action is the power map, and the only point with a nontrivial
stabilizer is the centre.
...
variable {M : Type*} [CommGroupWithZero M] {m : ℕ}

/-- Multiplication by an `m`-th root of unity does not change the `m`-th power. -/
@[simp]
theorem rootsOfUnity_smul_pow (ζ : rootsOfUnity m M) (u : M) : (ζ • u) ^ m = u ^ m := by
  rw [Subgroup.smul_def, Units.smul_def, smul_eq_mul, mul_pow, ← Units.val_pow_eq_pow_val,
...
theorem pow_eq_pow_iff_exists_rootsOfUnity_smul (hm : m ≠ 0) {u v : M} :
  ...
  · have hv : v ≠ 0 := by
    ...
    · rw [mem_rootsOfUnity, Units.ext_iff, Units.val_pow_eq_pow_val, Units.val_mk0, div_pow, h,
        div_self (pow_ne_zero m hv), Units.val_one]
    · rw [Subgroup.smul_def, Units.smul_def, smul_eq_mul, Units.val_mk0, div_mul_cancel₀ v hu]
...
theorem orbitRel_rootsOfUnity_apply (hm : m ≠ 0) {u v : M} :
...
image under `u ↦ u ^ m`. -/
theorem preimage_image_pow_eq (hm : m ≠ 0) (s : SubMulAction (rootsOfUnity m M) M) :
    (· ^ m) ⁻¹' ((· ^ m) '' (s : Set M)) = s := by
...
theorem stabilizer_rootsOfUnity_of_ne_zero {u : M} (hu : u ≠ 0) :
    stabilizer (rootsOfUnity m M) u = ⊥ := by
  refine (Subgroup.eq_bot_iff_forall _).mpr fun ζ hζ ↦ ?_
  rw [mem_stabilizer_iff, Subgroup.smul_def, Units.smul_def, smul_eq_mul] at hζ
  exact Subtype.ext <| Units.ext <| by
    simpa using mul_right_cancel₀ hu (hζ.trans (one_mul u).symm)

/-- Every `m`-th root of unity fixes `0`. -/
@[simp]
theorem stabilizer_rootsOfUnity_zero : stabilizer (rootsOfUnity m M) (0 : M) = ⊤ :=
  Subgroup.eq_top_iff' _ |>.mpr fun _ ↦ mem_stabilizer_iff.mpr (mul_zero _)
