...
* `RingEquiv.isUnramifiedAt_iff_of_comp_eq`: a prime of the top ring is unramified over one base
  exactly when it is unramified over an isomorphic base.
* `RingEquiv.isUnramifiedIn_iff_of_eq_comap`: a prime of the base is unramified exactly when the
  corresponding prime of an isomorphic base is.
-/
...
/-- The structure-map compatibility of `e : R ≃+* R'` gives the one of `e.symm`. -/
private theorem comp_symm_eq (e : R ≃+* R')
    (he : (algebraMap R' S).comp (e : R →+* R') = algebraMap R S) :
    (algebraMap R S).comp (e.symm : R' →+* R) = algebraMap R' S := by
  rw [← he, RingHom.comp_assoc]
  simp

private theorem isUnramifiedAt_of_comp_eq (e : R ≃+* R')
    (he : (algebraMap R' S).comp (e : R →+* R') = algebraMap R S)
...
identifies the two structure maps to `S`, then a prime of `S` is unramified over `R` exactly when
it is unramified over `R'`. -/
theorem isUnramifiedAt_iff_of_comp_eq (e : R ≃+* R')
    (he : (algebraMap R' S).comp (e : R →+* R') = algebraMap R S)
    (P : Ideal S) [P.IsPrime] :
    Algebra.IsUnramifiedAt R P ↔ Algebra.IsUnramifiedAt R' P :=
  ⟨fun _ ↦ isUnramifiedAt_of_comp_eq e he P,
    fun _ ↦ isUnramifiedAt_of_comp_eq e.symm (comp_symm_eq e he) P⟩
...
private theorem isUnramifiedIn_of_eq_comap (e : R ≃+* R')
    (he : (algebraMap R' S).comp (e : R →+* R') = algebraMap R S)
  ...
  exact (isUnramifiedAt_iff_of_comp_eq e he P).mpr this
...
`S` exactly when the prime `p'` of `R'` it contracts from is. -/
theorem isUnramifiedIn_iff_of_eq_comap (e : R ≃+* R')
    (he : (algebraMap R' S).comp (e : R →+* R') = algebraMap R S)
    {p : Ideal R} {p' : Ideal R'} (hp : p = p'.comap (e : R →+* R')) :
    Algebra.IsUnramifiedIn S p ↔ Algebra.IsUnramifiedIn S p' := by
  refine ⟨isUnramifiedIn_of_eq_comap e.symm (comp_symm_eq e he) ?_,
    isUnramifiedIn_of_eq_comap e he hp⟩
