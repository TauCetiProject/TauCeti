...
theorem adjoin_eq_top_of_span_eval_eq_maximalIdeal {β : S} {g : R[X]}
  ...
  have hsup := TauCeti.Subalgebra.toSubmodule_sup_pow_restrictScalars_eq_top hgen hmem
...
extension is separable, then `S` is generated over `R` by a single element.

Pick a lift `a` of a primitive element of the residue extension and a polynomial `g : R[X]`
lifting its minimal polynomial.  Then `g(a)` lies in `𝓂(S)` and `g'(a)` is a unit, so the Newton
step replaces `a` by a `β` congruent to it at which `g` *generates* `𝓂(S)`, and
`TauCeti.IsLocalRing.adjoin_eq_top_of_span_eval_eq_maximalIdeal` applies. -/
