...
* `TauCeti.DenseGraphLimits.exists_graphon_unitInterval_cutDist_eq_zero` -- every graphon on a
  standard Borel carrier is at cut distance zero from one on the unit interval.

## References

* S. Janson, *Graphons, cut norm and distance, couplings and rearrangements*, NYJM Monographs 4
  (2013), Theorem 7.1 and Theorem A.9.
-/
...
interval** (Janson, Thm 7.1): the canonical carrier sees every graphon up to cut distance. -/
theorem exists_graphon_unitInterval_cutDist_eq_zero (ν : Measure α) [IsProbabilityMeasure ν]
    (V : Graphon α ν) : ∃ W : Graphon I (volume : Measure I), cutDist V W = 0 :=
