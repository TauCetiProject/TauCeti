...
The canonical graphon space `GraphonSpaceI` is a **compact** metric space
(the Lovász--Szegedy compactness theorem), and in particular complete.

Total boundedness is `totallyBounded_graphonSpaceI`; this file supplies completeness.  The
argument has three steps.

* **Realignment.** Cut distances are infima over couplings, so graphons that are close in cut
  distance need not be close in cut norm on their common carrier.  Given a sequence of graphons on a
  standard Borel carrier whose consecutive cut distances are controlled, choose near-optimal
  couplings of consecutive terms and glue them into one law on the path space `ℕ → Ω`
  (`TauCeti.Measure.chainMeasure`).  Read along the coordinate projections, the whole sequence lives
  on this one carrier, with consecutive terms close in cut norm
  (`exists_isProbabilityMeasure_cutNorm_comap_sub_lt`).
* **Limit.** The path space is countably generated, so the realigned sequence, now Cauchy in cut
  norm, has a cut-norm limit there (`exists_graphon_tendsto_cutNorm_of_cauchy_cutNorm`).
* **Transport.** That limit is at cut distance zero from a graphon on the unit interval
  (`exists_graphon_unitInterval_cutDist_eq_zero`), which is the limit of the original sequence.

## Main results

* `TauCeti.DenseGraphLimits.exists_isProbabilityMeasure_cutNorm_comap_sub_lt` -- a sequence of
  graphons with controlled consecutive cut distances is realised on a single path-space carrier
  with the same control on consecutive cut norms;
* `TauCeti.DenseGraphLimits.Graphon.instCompleteSpaceUnitInterval` -- strict graphons on the unit
  interval form a complete pseudometric space for the cut distance;
...
/-- **Graphons on the unit interval form a complete pseudometric space** for the cut distance.

A sequence whose consecutive cut distances decay geometrically is realigned onto the path space
(`exists_isProbabilityMeasure_cutNorm_comap_sub_lt`), where it is Cauchy in cut norm and so has a
cut-norm limit (`exists_graphon_tendsto_cutNorm_of_cauchy_cutNorm`); that limit is read back on the
unit interval at cut distance zero (`exists_graphon_unitInterval_cutDist_eq_zero`). -/
...
/-- **Lovász--Szegedy compactness.** The canonical graphon space over the unit interval is a
compact metric space: it is totally bounded (`totallyBounded_graphonSpaceI`) and complete, as the
separation quotient of the complete space of graphons on the unit interval. -/
