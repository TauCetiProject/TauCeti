...
theorem IsConnected.eulerChar_le_two {t : PermutationTriple n} (ht : t.IsConnected) :
    t.eulerChar ≤ 2 := by
  -- Minimal transposition factorizations of `σ0` and `σ1`, taken together, generate a transitive
  -- group, so Hurwitz's transposition bound applies to their product `σ1 * σ0 = σinf⁻¹`.
  ...
  -- The monodromy group is generated by two of the three components.
  have hmonodromy : t.monodromyGroup ≤ H := by
    rw [← t.closure_inv_pair_eq_monodromyGroup, Subgroup.closure_le]
    intro g hg
    simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hg
    rcases hg with rfl | rfl
    · exact inv_mem hσ0
    · exact inv_mem hσ1
  have htrans : MulAction.IsPretransitive H (Fin n) := by
    refine ⟨fun x y => ?_⟩
    obtain ⟨g, hg⟩ := ht.isPretransitive.exists_smul_eq x y
    exact ⟨⟨g, hmonodromy g.2⟩, hg⟩
  have hbound := card_add_orbitCount_le_length_add_two hL htrans
