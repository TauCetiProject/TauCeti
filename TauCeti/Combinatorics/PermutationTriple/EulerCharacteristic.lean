...
combinatorial Euler bound: minimal transposition factorizations of `σ0` and `σ1`, taken
together, generate a transitive group, so Hurwitz's transposition bound applies to their product
`σ1 * σ0 = σinf⁻¹`. -/
theorem IsConnected.eulerChar_le_two {t : PermutationTriple n} (ht : t.IsConnected) :
  ...
  let _ : Nonempty (Fin n) := ⟨⟨0, Nat.pos_of_ne_zero ht.ne_zero⟩⟩
