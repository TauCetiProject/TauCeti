...
A relabeling carrying one generating triple of `P` to another necessarily normalizes `P.G`, so the
isomorphism classes inside a passport are the orbits of the normalizer `N_{S_n}(P.G)` acting by
simultaneous conjugation on the generating triples, and the size of a passport is the number of
those orbits. This is the shape in which passport sizes are computed: generating triples of a
fixed subgroup with fixed cycle data are counted inside that subgroup, and the count is then
divided by the normalizer action rather than by the whole symmetric group. The normalizer is not
`P.G` itself: two generating triples of `P.G` can be conjugate in the symmetric group only through
an element normalizing `P.G`, and inner conjugation is in general a proper subgroup of that.
...
* `TauCeti.PassportSpec.exists_isGeneratingTriple_smul` and
  `TauCeti.PassportSpec.mem_normalizer_of_smul`: a connected triple in a passport is isomorphic to
  a generating triple of it, uniquely up to the normalizer.
* `TauCeti.PassportSpec.generatingTripleOrbitsEquivClasses`: the orbits of the normalizer on the
  generating triples of a passport are the isomorphism classes in that passport.
* `TauCeti.PassportSpec.passportSize_eq_card_generatingTripleOrbits`: the passport size is the
  number of those orbits.
...
theorem isGeneratingTriple_iff_hasPassport {s : ConnectedTriple n} :
...
/-- A relabeling carrying one generating triple of `P` to another normalizes the reference
...
theorem hasPassport_toConnectedTriple (g : P.GeneratingTriple) :
  ...
  (isGeneratingTriple_iff_hasPassport.mp g.2).2
...
theorem exists_toClass_eq {c : ConnectedIsoClass n} (hc : c.HasPassport P) :
  ...
  refine ⟨⟨τ • s.1, hτ⟩, ?_⟩
  rw [toClass_eq_mk, ConnectedIsoClass.mk_eq_mk_iff, orbitRel_apply, mem_orbit_iff]
  exact ⟨τ, Subtype.ext (rfl)⟩
...
/-- **The normalizer formulation.** The isomorphism classes of connected triples in a passport are
...
@[simp]
theorem generatingTripleOrbitsEquivClasses_apply_mk (g : P.GeneratingTriple) :
