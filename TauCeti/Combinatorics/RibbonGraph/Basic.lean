...
/-- The face count is the cardinality of the face type. -/
@[simp] theorem faceCount_eq_card_face : Γ.faceCount = Fintype.card Γ.Face := by
  rw [faceCount]
...
/-- The Euler characteristic is vertices plus faces minus edges. -/
theorem eulerChar_def : Γ.eulerChar =
    Fintype.card Γ.B + Fintype.card Γ.W + Γ.faceCount - Fintype.card Γ.E := by
  rw [eulerChar]
