...
private def edgeActionHom (f : Γ.Iso Δ) : Γ.E →ₑ[f.rotationGroupEquiv] Δ.E where
  toFun := f.edge
  map_smul' g e := by
    -- Subtype actions coerce to their underlying permutations, while `rotationGroupEquiv`
    -- coerces to conjugation by `f.edge`; these two coercions agree definitionally.
