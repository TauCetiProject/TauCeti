...
This file packages the subgroup of the diffeomorphism group that fixes the manifold boundary
pointwise.  The geometric-topology roadmap's layer on diffeomorphism groups asks for the relative
group `Diff(M, ∂M)`, the diffeomorphisms fixing the boundary pointwise, as soon as `Diff(M)` itself
is available as a group.  The generic group-action primitive already exists in Mathlib as
`fixingSubgroup`; the boundary-facing name below is just the roadmap-facing specialization.
...
* `toHomeomorphHom_mem_boundaryFixingSubgroup`: a boundary-fixing diffeomorphism maps to a
  boundary-fixing homeomorphism under the forgetful homomorphism.
-/
...
abbrev boundaryFixingSubgroup : Subgroup (M ≃ₘ^n⟮I, I⟯ M) :=
  fixingSubgroup (M ≃ₘ^n⟮I, I⟯ M) (I.boundary M)
...
theorem mem_boundaryFixingSubgroup_iff {f : M ≃ₘ^n⟮I, I⟯ M} :
  ...
  simp [boundaryFixingSubgroup, mem_fixingSubgroup_iff, smul_def]
