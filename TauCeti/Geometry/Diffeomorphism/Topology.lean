...
`ContMDiffMap.tendsto_manifoldWeakWhitney_iff`. On a compact source this is the `C^n` topology in
the usual sense, the weak and the strong Whitney topologies agreeing there. Only the forward map
is topologized; no continuity is claimed for inversion, which needs an inverse-function estimate
that is not developed here.
