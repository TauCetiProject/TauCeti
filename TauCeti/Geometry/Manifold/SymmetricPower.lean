...
* `TauCeti.mem_iff_pow_add_sum_symOpenPartialHomeomorph_mul_pow_eq_zero` and
  `TauCeti.exists_continuousLinearMap_ne_zero_mem_iff_symChartAt`: the unordered tuples through a
  fixed point `z` are cut out by one affine equation, with nonzero linear part, in every chart
  that they meet. For a basepoint `z` of a Heegaard surface this is the divisor
