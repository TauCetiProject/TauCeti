...
private theorem isOpenEmbedding_normalRay_restricted :
    IsOpenEmbedding (fun t : Ico (0 : ℝ) 1 => normalRay t.1) := by
  convert (isOpen_normalIio 1).isOpenEmbedding_subtypeVal.comp
    (homeomorphNormalIio 1).symm.isOpenEmbedding using 1
  funext t
  have ht : ((homeomorphNormalIio 1).symm t : EuclideanHalfSpace 1).1 0 = t.1 := by
    have h := congrArg (fun q : Ico (0 : ℝ) 1 => (q : ℝ))
      ((homeomorphNormalIio 1).apply_symm_apply t)
    simpa only [coe_homeomorphNormalIio] using h
  have hnormal : normalRay t.1 = ((homeomorphNormalIio 1).symm t : EuclideanHalfSpace 1) := by
    rw [← normalRay_normalCoord ((homeomorphNormalIio 1).symm t : EuclideanHalfSpace 1), ht]
  simpa only [Function.comp_apply] using hnormal
...
noncomputable def boundaryCollar (n : ℕ) :
  ...
  (collarDiffeomorph (k := ⊤) n) ∘
    Prod.map id (fun t : Ico (0 : ℝ) 1 => normalRay t.1)
...
theorem isCollar_boundaryParam (n : ℕ) :
  ...
  · exact (collarDiffeomorph (k := ⊤) n).toHomeomorph.isOpenEmbedding.comp
      (IsOpenEmbedding.id.prodMap isOpenEmbedding_normalRay_restricted)
  · intro x
...
@[simp]
theorem boundaryCollar_apply_zero_eq_boundaryParam (n : ℕ) (x : EuclideanSpace ℝ (Fin n)) :
...
@[simp]
theorem boundaryCollar_apply_zero (n : ℕ)
