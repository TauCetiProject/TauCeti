...
theorem isLagrangian_cotangent_graph_iff :
  ...
  constructor
  · intro h v w
    have hz := h.isIsotropic
    have hzero := (SymplecticForm.isIsotropic_iff.mp hz)
      (v, A v) (by simp) (w, A w) (by simp)
    simp only [cotangentSymplecticForm_apply] at hzero
  ...
  · intro hsym
    ...
    · apply (SymplecticForm.isCoisotropic_iff).2
      ...
      simp only [cotangentSymplecticForm_apply] at hv
...
theorem isLagrangian_strongDualCotangent_graph_iff :
  ...
  · intro h v w
    have hzero := (SymplecticForm.isIsotropic_iff.mp h.isIsotropic)
      (v, A v) (by simp) (w, A w) (by simp)
    simp only [strongDualCotangentSymplecticForm_apply] at hzero
    exact (sub_eq_zero.mp hzero).symm
  · intro hsym
    apply (SymplecticForm.isLagrangian_iff).2
    constructor
    ...
    · apply (SymplecticForm.isCoisotropic_iff).2
      ...
      intro v
      have hv := (SymplecticForm.mem_orthogonal_iff.mp hx) (v, A v) (by simp)
      simp only [strongDualCotangentSymplecticForm_apply] at hv
      have hsymv := hsym v x.1
      simpa using (sub_eq_zero.mp hv).trans hsymv
...
theorem ContDiffAt.fderiv_cotangentDifferentialGraph {f : V → ℝ} {x : V}
  ...
  exact ((hasFDerivAt_id x).prodMk
    ((hf.fderiv_right (m := 1) (by norm_num)).differentiableAt (by norm_num)).hasFDerivAt).fderiv
