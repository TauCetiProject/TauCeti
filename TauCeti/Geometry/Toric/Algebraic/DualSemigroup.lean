...
consists of the integral characters whose real extensions are nonnegative on the cone. It is the
additive monoid used to form the coordinate ring of the corresponding affine toric variety.
...
noncomputable def dualSemigroupMap (hi : IsIntegralLattice i) (hi' : IsIntegralLattice i')
  ...
  ((AddMonoidHom.compHom' f).domRestrict (dualSemigroup hi' τ)).codRestrict
    (dualSemigroup hi σ) fun m ↦ by
      have hm : AddMonoidHom.compHom' f (m : N' →+ ℤ) = (m : N' →+ ℤ).comp f := by
        ext n
        simp
      rw [AddMonoidHom.domRestrict_apply, hm, mem_dualSemigroup,
        hi.realCharacter_comp hi' f g hfg]
      intro x hx
      exact m.2 (hστ hx)
