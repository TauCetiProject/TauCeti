...
`dualSemigroup` is not exposed: `dualSemigroup_def` and `mem_dualSemigroup` characterise it
completely, so nothing downstream needs to unfold it. The former is proved by the parenthesised
`(rfl)`, which elaborates against the definition itself; a bare `rfl` in an exported theorem would
demand that the definition be `@[expose]`d.
...
theorem dualSemigroup_sup (hi : IsIntegralLattice i) (σ τ : PointedCone ℝ V) :
  ...
  refine le_antisymm
    (le_inf (dualSemigroup_anti hi le_sup_left) (dualSemigroup_anti hi le_sup_right))
    fun m hm ↦ ?_
  obtain ⟨hσ, hτ⟩ := AddSubmonoid.mem_inf.1 hm
  rw [mem_dualSemigroup] at hσ hτ ⊢
  intro x hx
  obtain ⟨y, hy, z, hz, rfl⟩ := Submodule.mem_sup.1 hx
  rw [map_add]
  exact add_nonneg (hσ hy) (hτ hz)
