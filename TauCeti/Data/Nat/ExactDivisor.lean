...
The classical notation is `Q ‖ N`, which this file does **not** introduce: in the same corner of
the library `p ^ r ‖ n` is used for exact `p`-adic divisibility — that `r` is the full exponent
of `p`, a statement about the pair `(p, r)` rather than about the single number `p ^ r` — and
`‖ ‖` is already norm notation. The predicate is spelled out instead.

## Main definitions

* `TauCeti.IsExactDivisor`: the predicate itself.

## Main results

* `TauCeti.IsExactDivisor.ne_zero`: an exact divisor is nonzero. There is no exact divisor `0`,
  because `Nat.Coprime 0 0` is false.
* `TauCeti.isExactDivisor_one`, `TauCeti.isExactDivisor_self`: `1` and (for `N ≠ 0`) `N` itself.
* `TauCeti.IsExactDivisor.mul`: coprime exact divisors multiply to an exact divisor. This is what
  makes the exact divisors of `N` generated by the maximal prime powers.
-/
...
divisor `N / Q`. Written `Q ‖ N` in the literature; see the module docstring for why no notation
is introduced. -/
...
/-- **Coprime exact divisors multiply.** Together with `isExactDivisor_one` this presents every
exact divisor of `N` as a product of maximal prime powers. -/
