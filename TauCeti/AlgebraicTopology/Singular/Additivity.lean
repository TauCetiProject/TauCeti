...
This is the chain-level additivity axiom of Eilenberg--Steenrod: every map in sight is induced by
one of the inclusions `X i ⟶ Σ i, X i`, so it is a statement about a cofan rather than an
unrelated degreewise decomposition.
...
lemma bijective_toSSet_map_sigmaι_app : Function.Bijective
  ...
  have key : Function.Bijective
    ...
    rwa [show ⇑(ContinuousMap.sigmaCodHomeomorph (StdSimplex ℝ (Fin (n.unop.len + 1)))
        (fun i ↦ ((X i : TopCat.{w}) : Type w))).symm =
      (fun p ↦ (ContinuousMap.sigmaMk p.1).comp p.2) from
        funext fun p ↦ ContinuousMap.sigmaCodHomeomorph_symm_apply _ _ p] at h
...
def isColimitCofanToSSetObj :
  ...
  ((Cofan.nonempty_isColimit_iff_bijective_fromSigma _).2
    (bijective_toSSet_map_sigmaι_app X n)).some

instance preservesColimit_discreteFunctor_toSSet :
