...
* `TauCeti.FundamentalGroup.map_range_eq_bot_of_subsingleton`: maps from a basepoint with
  trivial fundamental group have trivial fundamental-group range.
...
* `TauCeti.FundamentalGroup.map_range_le_mapOfEq_range_of_subsingleton`: a domain whose
  chosen-basepoint fundamental group is trivial satisfies the fundamental-group range condition
  against any `mapOfEq` range.
