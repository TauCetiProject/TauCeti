...
semilattice with measurable join is almost everywhere measurable, and so is the infimum into a
semilattice with measurable meet. In a linear order these are the maximum and the minimum, which
is what lets the two extremes of a finite family of random variables — and hence their range — be
...
almost-everywhere counterpart for both extrema. Both lemmas are registered with `fun_prop`, which
