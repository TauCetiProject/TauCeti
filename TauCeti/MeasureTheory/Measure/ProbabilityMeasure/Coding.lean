...
* `TauCeti.ProbabilityMeasure.codeIndex` -- the countable generating set algebra;
* `TauCeti.ProbabilityMeasure.code` -- evaluation on every member of that algebra;
* `TauCeti.ProbabilityMeasure.measurable_code` -- measurability of the code;
* `TauCeti.ProbabilityMeasure.code_injective` -- the code determines the measure.
-/
...
abbrev codeIndex (α : Type*) [MeasurableSpace α] [CountablyGenerated α] :=
  {s : Set α // s ∈ generateSetAlgebra (countableGeneratingSet α)}

instance instCountableCodeIndex : Countable (codeIndex α) :=
...
def code (P : ProbabilityMeasure α) : codeIndex α → ℝ≥0∞ :=
  fun s => (P : Measure α) s.1

/-- Evaluation of the probability-measure code. -/
@[simp]
theorem code_apply (P : ProbabilityMeasure α) (s : codeIndex α) :
    code P s = (P : Measure α) s.1 :=
  (rfl)

/-- Every member of the coding set algebra is measurable. -/
theorem measurableSet_codeIndex (s : codeIndex α) :
...
theorem measurable_code : Measurable (code (α := α)) :=
  Measurable.of_eval fun s =>
    (Measure.measurable_coe (measurableSet_codeIndex s)).comp
      measurable_subtype_coe

/-- The evaluation code determines a probability measure on a countably generated space. -/
theorem code_injective : Function.Injective (code (α := α)) := by
  intro P Q hPQ
