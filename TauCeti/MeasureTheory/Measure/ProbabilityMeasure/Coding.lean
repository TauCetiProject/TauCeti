...
instance instCountableProbabilityMeasureCodeIndex :
    Countable (probabilityMeasureCodeIndex α) :=
...
def probabilityMeasureCode (P : ProbabilityMeasure α) :
...
theorem measurable_probabilityMeasureCode :
    Measurable (probabilityMeasureCode (α := α)) :=
