...
noncomputable def baseChange : _root_.GroupLike R A →* _root_.GroupLike K (K ⊗[R] A) where
  toFun g := ⟨1 ⊗ₜ[R] g.val, {
...
theorem one_tmul_val_baseChangeEquiv_symm
  ...
  rw [← val_baseChange, ← baseChangeEquiv_apply hspan, MulEquiv.apply_symm_apply]
