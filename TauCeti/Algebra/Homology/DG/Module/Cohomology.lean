...
/-!
# The cohomology module of a differential graded module

Let `dM` be a differential on a module `ℳ` over the differential graded algebra `(𝒜, d)`, in the
sense of `TauCeti.IsDGModule`.  Its **cycles** are the kernel of `dM` and its **boundaries** are the
image of `dM`.  This file shows that the cycles are a module over the algebra of cycles
...
* `TauCeti.IsDGModule.cycles`: the kernel of the differential, as a module over the algebra of
  cycles of `A`.
* `TauCeti.IsDGModule.cyclesDeg`: the homogeneous cycles of a fixed degree.
* `TauCeti.IsDGModule.boundaries`: the image of the differential, as a submodule of the cycles.
* `TauCeti.IsDGModule.Cohomology`: the cohomology module, the cycles modulo the boundaries.

## Main results

* `TauCeti.IsDGModule.iSup_cyclesDeg_eq_top` and `TauCeti.IsDGModule.isInternal_cyclesDeg`: every
  cycle is a sum of homogeneous ones, and the homogeneous cycles form an internal direct sum.
* `TauCeti.IsDGModule.instGradedSMulCyclesDeg`: **the cycles of a differential graded module are a
  graded module over the graded algebra of cycles.**
* `TauCeti.IsDGModule.isTorsionBySet_boundaries`: the boundaries of `A` annihilate the cohomology
  module.
* `TauCeti.IsDGModule.instModuleCohomology`: **the cohomology of a differential graded module is a
  module over the cohomology algebra**, with `TauCeti.IsDGModule.quotientMk_smul` computing the
  action through a representing cycle.
...
/-- The **cycles** of a differential graded module: the kernel of the differential.  It is a
module over the algebra of cycles of `A` because a cycle acting on a cycle is a cycle. -/
def cycles (hM : IsDGModule h ℳ dM) : Submodule h.cycles M :=
  { (LinearMap.ker dM).toAddSubmonoid with
    smul_mem' := fun z _ hx => hM.map_smul_eq_zero_of_map_eq_zero (h.mem_cycles.mp z.2) hx }

@[simp]
lemma mem_cycles (hM : IsDGModule h ℳ dM) {x : M} : x ∈ hM.cycles ↔ dM x = 0 := Iff.rfl

/-- The differential of every element is a cycle, by the square-zero axiom. -/
theorem map_mem_cycles (hM : IsDGModule h ℳ dM) (x : M) : dM x ∈ hM.cycles :=
  hM.sq_zero x

/-- The **boundaries** of a differential graded module: the image of the differential, viewed
inside the cycles.  A cycle of `A` carries a boundary to a boundary, so this is a submodule over
the algebra of cycles. -/
def boundaries (hM : IsDGModule h ℳ dM) : Submodule h.cycles hM.cycles :=
...
theorem isTorsionBySet_boundaries (hM : IsDGModule h ℳ dM) :
...
/-- **The cohomology of a differential graded module is a module over the cohomology algebra.**
The action of a cycle of `A` on a cycle of `M` descends, because the boundaries of `A` annihilate
the cohomology module. -/
noncomputable instance instModuleCohomology (hM : IsDGModule h ℳ dM) :
...
cycle. -/
@[simp]
theorem quotientMk_smul (hM : IsDGModule h ℳ dM) (z : h.cycles) (x : hM.Cohomology) :
...
def cyclesDeg (hM : IsDGModule h ℳ dM) (p : ℤ) : Submodule R hM.cycles :=
  (ℳ p).comap ((hM.cycles.subtype).restrictScalars R)

@[simp]
lemma mem_cyclesDeg (hM : IsDGModule h ℳ dM) {p : ℤ} {z : hM.cycles} :
...
theorem iSup_cyclesDeg_eq_top (hM : IsDGModule h ℳ dM) : ⨆ p : ℤ, hM.cyclesDeg p = ⊤ := by
  classical
...
ambient module. -/
theorem iSupIndep_cyclesDeg (hM : IsDGModule h ℳ dM) : iSupIndep hM.cyclesDeg := by
  intro p
...
theorem isInternal_cyclesDeg (hM : IsDGModule h ℳ dM) : DirectSum.IsInternal hM.cyclesDeg :=
  DirectSum.isInternal_submodule_of_iSupIndep_of_iSup_eq_top hM.iSupIndep_cyclesDeg
    hM.iSup_cyclesDeg_eq_top

/-- The cycles inherit the grading of the ambient differential graded module. -/
noncomputable instance instDecompositionCyclesDeg (hM : IsDGModule h ℳ dM) :
    DirectSum.Decomposition hM.cyclesDeg :=
  hM.isInternal_cyclesDeg.chooseDecomposition

/-- **The cycles of a differential graded module are a graded module over the graded algebra of
cycles**: a homogeneous cycle of degree `p` carries a homogeneous cycle of degree `q` to one of
degree `p + q`. -/
instance instGradedSMulCyclesDeg (hM : IsDGModule h ℳ dM) :
