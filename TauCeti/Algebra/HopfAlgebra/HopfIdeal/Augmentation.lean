...
* `TauCeti.HopfIdeal.mem_augmentation`, `TauCeti.HopfIdeal.augmentation_toIdeal` and
  `TauCeti.HopfIdeal.augmentation_toIdeal_ne_top`: characteristic API.
* `TauCeti.HopfIdeal.le_augmentation`: every Hopf ideal is contained in the augmentation ideal.
* `TauCeti.HopfIdeal.centralAugmentation`: the central part of the augmentation ideal.
* `TauCeti.HopfIdeal.centralAugmentationIdeal`: the two-sided ideal generated by that central
  part.
...
homomorphism onto `R`. -/
theorem augmentation_toIdeal_ne_top [Nontrivial R] : (augmentation R H).toIdeal ≠ ⊤ := by
  rw [augmentation_toIdeal]
  exact RingHom.ker_ne_top _

/-- The central part of the augmentation ideal of a Hopf algebra. -/
noncomputable def centralAugmentation : Submodule R H :=
  (Subalgebra.center R H).toSubmodule ⊓
    (augmentation R H).toIdeal.restrictScalars R

/-- Membership in the central augmentation submodule means being both central and augmented. -/
@[simp]
theorem mem_centralAugmentation {z : H} :
    z ∈ centralAugmentation R H ↔
      z ∈ Subalgebra.center R H ∧ z ∈ (augmentation R H).toIdeal := by
...
theorem centralAugmentation_le_center :
    centralAugmentation R H ≤ (Subalgebra.center R H).toSubmodule :=
  inf_le_left

/-- The central augmentation submodule is contained in the augmentation ideal. -/
theorem centralAugmentation_le_augmentation :
    centralAugmentation R H ≤ (augmentation R H).toIdeal.restrictScalars R :=
  inf_le_right

/-- The ideal generated by the central part of the augmentation ideal. -/
noncomputable def centralAugmentationIdeal : Ideal H :=
  Ideal.span (centralAugmentation R H : Set H)

/-- The central augmentation ideal is the span of the central augmentation submodule. -/
theorem centralAugmentationIdeal_def :
    centralAugmentationIdeal R H = Ideal.span (centralAugmentation R H : Set H) := by
  rw [centralAugmentationIdeal]

/-- The universal property of the central augmentation ideal. -/
@[simp]
theorem centralAugmentationIdeal_le_iff {I : Ideal H} :
    centralAugmentationIdeal R H ≤ I ↔ (centralAugmentation R H : Set H) ⊆ I := by
  rw [centralAugmentationIdeal_def, Ideal.span_le]

/-- A central element of augmentation zero belongs to the ideal generated by all such elements. -/
theorem mem_centralAugmentationIdeal_of_mem {z : H} (hz : z ∈ centralAugmentation R H) :
    z ∈ centralAugmentationIdeal R H := by
  rw [centralAugmentationIdeal_def]
...
instance instIsTwoSidedCentralAugmentationIdeal :
    (centralAugmentationIdeal R H).IsTwoSided := by
  rw [centralAugmentationIdeal_def]
  exact Ideal.isTwoSided_span_of_subset_center fun _ hz ↦
    Semigroup.mem_center_iff.mpr
      (Subalgebra.mem_center_iff.mp ((mem_centralAugmentation R H).mp hz).1)

/-- The central augmentation ideal is contained in the augmentation ideal. -/
theorem centralAugmentationIdeal_le_augmentation :
    centralAugmentationIdeal R H ≤ (augmentation R H).toIdeal := by
  rw [centralAugmentationIdeal_def, Ideal.span_le]
  exact fun _ hz ↦ ((mem_centralAugmentation R H).mp hz).2

/-- Over a nontrivial base ring, the central augmentation ideal is proper. -/
theorem centralAugmentationIdeal_ne_top [Nontrivial R] :
    centralAugmentationIdeal R H ≠ ⊤ :=
  ne_top_of_le_ne_top (augmentation_toIdeal_ne_top R H)
    (centralAugmentationIdeal_le_augmentation R H)
...
theorem exists_finset_subset_centralAugmentation_span_eq [IsNoetherianRing H] :
    ∃ s : Finset H, (s : Set H) ⊆ centralAugmentation R H ∧
      Ideal.span (s : Set H) = centralAugmentationIdeal R H := by
  have hfg : (Ideal.span (centralAugmentation R H : Set H)).FG :=
    Ideal.fg_of_isNoetherianRing _
  obtain ⟨s, hs, hspan⟩ :=
    (Submodule.fg_span_iff_fg_span_finset_subset
      (R := H) (centralAugmentation R H : Set H)).mp hfg
