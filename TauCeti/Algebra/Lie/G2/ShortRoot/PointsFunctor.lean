...
/-!
# The points of the short-root type-G2 carrier, functorially

`TauCeti.G2ShortRoot.groupScheme` is the short-root type-`G₂` carrier over `ℤ`, built from the
seven-dimensional representation, and `TauCeti.G2ShortRoot.points A` realizes its `A`-valued
...
or an identification of the carrier with the pinned simply connected group scheme of type `G₂`.

## Main declarations

* `TauCeti.G2ShortRoot.pointsMap`: the map on carrier points induced by a ring homomorphism.
* `TauCeti.G2ShortRoot.pointsFunctor`: the group-valued functor of points.
* `TauCeti.G2ShortRoot.pointsMulEquiv`: the pointwise representing isomorphism.
* `TauCeti.G2ShortRoot.pointsFunctorNatIso`: the natural representing isomorphism.
...
/-- The map on the points of the short-root type-`G₂` carrier induced by a homomorphism of value
...
def pointsMap (f : A →+* B) : points A →* points B :=
  GeneralLinear.mapHopfIdealPointsSubgroupCongr 7 definingIdeal
    (points_def A) (points_def B) f.toIntAlgHom

/-- The induced map on carrier points is the entrywise map. -/
@[simp]
theorem coe_pointsMap (f : A →+* B) (g : points A) :
    (pointsMap f g : Matrix.GeneralLinearGroup (Fin 7) B) =
...
theorem coe_pointsMap_apply (f : A →+* B) (g : points A) (i j : Fin 7) :
    ((pointsMap f g : Matrix.GeneralLinearGroup (Fin 7) B) :
        Matrix (Fin 7) (Fin 7) B) i j =
      f (((g : Matrix.GeneralLinearGroup (Fin 7) A) : Matrix (Fin 7) (Fin 7) A) i j) := by
  rw [coe_pointsMap, Matrix.GeneralLinearGroup.map_apply]

/-- The identity homomorphism induces the identity on carrier points. -/
@[simp]
...
theorem pointsMap_comp {C : Type*} [CommRing C] (f : A →+* B) (g : B →+* C) :
  ...
  exact GeneralLinear.mapHopfIdealPointsSubgroupCongr_comp 7 definingIdeal
    (points_def A) (points_def B) (points_def C) f.toIntAlgHom g.toIntAlgHom

/-- An injective homomorphism of value rings induces an injective map on carrier points. -/
theorem pointsMap_injective {f : A →+* B} (hf : Function.Injective f) :
    Function.Injective (pointsMap f) :=
  GeneralLinear.mapHopfIdealPointsSubgroupCongr_injective 7 definingIdeal
...
rings. -/
@[simp]
theorem pointsMap_rootSubgroupPoints (f : A →+* B) (k : Fin 2 ⊕ Fin 2)
    (u : Multiplicative A) :
    pointsMap f (rootSubgroupPoints k A u) =
      rootSubgroupPoints k B (Multiplicative.ofAdd (f (Multiplicative.toAdd u))) := by
  apply Subtype.ext
  rw [coe_pointsMap, coe_rootSubgroupPoints_eq_kostantRootSubgroupMatrix,
    coe_rootSubgroupPoints_eq_kostantRootSubgroupMatrix,
...
@[simp]
theorem pointsMap_weightTorusPoints (f : A →+* B) (s : Fin 2 → Aˣ) :
  ...
  exact UniversalEnvelopingAlgebra.map_kostantTorusMatrix
    (M := lattice.toAddSubgroup) (b := latticeBasis) (wt := weight) f s

end Map

/-! ## The functor of points -/

/-- The group-valued functor of points of the short-root type-`G₂` carrier. -/
...
/-- The object part of the carrier's points functor is its named point group. -/
@[simp]
theorem pointsFunctor_obj (A : CommAlgCat.{v} ℤ) :
    pointsFunctor.obj A = GrpCat.of (points A) :=
  (rfl)

/-- The morphism part of the carrier's points functor is the induced entrywise map. -/
@[simp]
...
private theorem points_eq_hopfIdealPointsSubgroup (A : CommAlgCat.{v} ℤ) :
    points A = GeneralLinear.hopfIdealPointsSubgroup 7 definingIdeal A := by
  rw [points_def A]
  congr 1
  exact Subsingleton.elim _ _

/-- The points of the quotient coordinate Hopf algebra are the named carrier points. -/
def pointsMulEquiv (A : CommAlgCat.{v} ℤ) :
    HopfAlgebra.points
        (R := ℤ) (H := CommHopfAlgCat.quotient
          (GeneralLinear.coordinateHopfAlgebra ℤ 7) definingIdeal) A ≃*
      points A :=
  (GeneralLinear.hopfIdealPointsSubgroupMulEquiv 7 definingIdeal A).trans
...
theorem coe_pointsMulEquiv_apply (A : CommAlgCat.{v} ℤ)
    (q : HopfAlgebra.points
      (R := ℤ) (H := CommHopfAlgCat.quotient
        (GeneralLinear.coordinateHopfAlgebra ℤ 7) definingIdeal) A) :
    (pointsMulEquiv A q : Matrix.GeneralLinearGroup (Fin 7) A) =
      GeneralLinear.pointsMulEquiv 7
        (CommHopfAlgCat.quotientPointsHom
          (GeneralLinear.coordinateHopfAlgebra ℤ 7) definingIdeal A q) := by
  simp only [pointsMulEquiv, MulEquiv.trans_apply, MulEquiv.subgroupCongr_symm_apply]
  exact GeneralLinear.coe_hopfIdealPointsSubgroupMulEquiv_apply 7 definingIdeal A q
...
theorem quotientPointsHom_pointsMulEquiv_symm (A : CommAlgCat.{v} ℤ) (g : points A) :
    CommHopfAlgCat.quotientPointsHom
      ...
      (GeneralLinear.pointsMulEquiv (R := ℤ) 7).symm
        (g : Matrix.GeneralLinearGroup (Fin 7) A) := by
    ...
    pointsMulEquiv B
        (HopfAlgebra.mapPoints
          (H := CommHopfAlgCat.quotient
            (GeneralLinear.coordinateHopfAlgebra ℤ 7) definingIdeal) f q) =
      pointsMap f.hom (pointsMulEquiv A q) := by
  apply Subtype.ext
  rw [coe_pointsMap]
  simp only [pointsMulEquiv, MulEquiv.trans_apply, MulEquiv.subgroupCongr_symm_apply]
  exact (congrArg Subtype.val
      (GeneralLinear.hopfIdealPointsSubgroupMulEquiv_mapPoints 7 definingIdeal f q)).trans
    (GeneralLinear.coe_mapHopfIdealPointsSubgroup 7 definingIdeal f.hom _)

/-- The quotient coordinate Hopf algebra represents the points functor of the short-root
type-`G₂` carrier. -/
def pointsFunctorNatIso :
    HopfAlgebra.pointsFunctor
        (R := ℤ) (H := CommHopfAlgCat.quotient
          (GeneralLinear.coordinateHopfAlgebra ℤ 7) definingIdeal) ≅
      pointsFunctor :=
