...
* `TauCeti.UniversalEnvelopingAlgebra.centralEigenspace`: the eigenspace of a **central** element
  of `U(L)`, which centrality makes a Lie submodule.
...
* `TauCeti.UniversalEnvelopingAlgebra.mem_centralEigenspace_iff` and
  `TauCeti.UniversalEnvelopingAlgebra.representation_eq_smul_of_mem_center_of_lieSpan_eq_top`: the
  membership rule for that eigenspace, and the consequence that a central element acting by a
  scalar on a cyclic generator acts by that scalar everywhere.
...
/-- **The eigenspace of a central element of `U(L)`**, as a Lie submodule: the kernel of
`representation R L M u - c`, which is a submodule because that map is `R`-linear and a *Lie*
submodule because a central element commutes with the Lie action
(`TauCeti.UniversalEnvelopingAlgebra.representation_lie_of_mem_center`); centrality is what the Lie
structure needs, so the element is taken in `Subalgebra.center R U` rather than in `U`. By
`TauCeti.UniversalEnvelopingAlgebra.mem_centralEigenspace_iff` its underlying submodule is
Mathlib's `Module.End.eigenspace (representation R L M u) c`, whence the name. -/
noncomputable def centralEigenspace (u : Subalgebra.center R U) (c : R) :
    LieSubmodule R L M :=
  LieModuleHom.ker
    { toLinearMap := representation R L M (u : U) - c • LinearMap.id
      map_lie' := by
        intro x m
        simp [representation_lie_of_mem_center R L M u.2 x m] }

/-- Membership in `TauCeti.UniversalEnvelopingAlgebra.centralEigenspace` is acting by the
scalar. -/
@[simp]
theorem mem_centralEigenspace_iff {u : Subalgebra.center R U} {c : R} {m : M} :
    m ∈ centralEigenspace R L M u c ↔ representation R L M (u : U) m = c • m := by
  rw [centralEigenspace, LieModuleHom.mem_ker]
  simp [sub_eq_zero]

/-- A central element of the universal enveloping algebra that acts by a scalar on a cyclic
Lie-module generator acts by that scalar on the whole module: the locus where it acts by that
scalar is a Lie submodule containing the generator. -/
theorem representation_eq_smul_of_mem_center_of_lieSpan_eq_top
  ...
  have hvmem : v ∈ centralEigenspace R L M ⟨u, hu⟩ c :=
    (mem_centralEigenspace_iff R L M).mpr hv
  have htop : centralEigenspace R L M ⟨u, hu⟩ c = ⊤ := by
    rw [← top_le_iff, ← hgen]
    exact LieSubmodule.lieSpan_le.mpr (by simpa using hvmem)
  exact (mem_centralEigenspace_iff R L M).mp (by rw [htop]; exact LieSubmodule.mem_top m)
