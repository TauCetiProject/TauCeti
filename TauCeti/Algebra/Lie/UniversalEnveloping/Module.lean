...
* `TauCeti.UniversalEnvelopingAlgebra.centralEigenLieSubmodule`: the locus where a **central**
  element of `U(L)` acts by a fixed scalar, which centrality makes a Lie submodule.
...
* `TauCeti.UniversalEnvelopingAlgebra.mem_centralEigenLieSubmodule_iff` and
  `TauCeti.UniversalEnvelopingAlgebra.representation_eq_smul_of_mem_center_of_lieSpan_eq_top`: the
  membership rule for that locus, and the consequence that a central element acting by a scalar on
  a cyclic generator acts by that scalar everywhere.
...
/-- **The locus where a central element of `U(L)` acts by a fixed scalar**, as a Lie submodule. It
is the kernel of `representation R L M u - c`, which is a submodule because that map is `R`-linear
and a *Lie* submodule because a central element commutes with the Lie action
(`TauCeti.UniversalEnvelopingAlgebra.representation_lie_of_mem_center`); centrality is what the Lie
structure needs, so the element is taken in `Subalgebra.center R U` rather than in `U`. -/
...
/-- Membership in `TauCeti.UniversalEnvelopingAlgebra.centralEigenLieSubmodule` is acting by the
scalar. -/
@[simp]
theorem mem_centralEigenLieSubmodule_iff {u : Subalgebra.center R U} {c : R} {m : M} :
    m ∈ centralEigenLieSubmodule R L M u c ↔ representation R L M (u : U) m = c • m := by
  rw [centralEigenLieSubmodule, LieModuleHom.mem_ker]
  simp [sub_eq_zero]
...
theorem representation_eq_smul_of_mem_center_of_lieSpan_eq_top
  ...
  have hvmem : v ∈ centralEigenLieSubmodule R L M ⟨u, hu⟩ c :=
    (mem_centralEigenLieSubmodule_iff R L M).mpr hv
  have htop : centralEigenLieSubmodule R L M ⟨u, hu⟩ c = ⊤ := by
    rw [← top_le_iff, ← hgen]
    exact LieSubmodule.lieSpan_le.mpr (by simpa using hvmem)
  exact (mem_centralEigenLieSubmodule_iff R L M).mp (by rw [htop]; exact LieSubmodule.mem_top m)
