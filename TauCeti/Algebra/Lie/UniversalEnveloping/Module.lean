...
* `TauCeti.UniversalEnvelopingAlgebra.centralEigenLieSubmodule`: the vectors on which a **central**
  element of `U(L)` acts by a fixed scalar, a Lie submodule because centrality makes the action of
  that element equivariant.
...
* `TauCeti.UniversalEnvelopingAlgebra.representation_eq_smul_of_mem_center_of_lieSpan_eq_top`: a
  central element acting by a scalar on a cyclic generator acts by that scalar everywhere, proved
  through `TauCeti.UniversalEnvelopingAlgebra.centralEigenLieSubmodule`.
...
/-- The difference `ρ u - c` of the action of a central element and a scalar, as a homomorphism of
Lie modules. Centrality is exactly what makes it equivariant
(`TauCeti.UniversalEnvelopingAlgebra.representation_lie_of_mem_center`); for a non-central `u` the
map is only `R`-linear, which is why
`TauCeti.UniversalEnvelopingAlgebra.centralEigenLieSubmodule` below is a *Lie* submodule. -/
private noncomputable def representationSubSmulId (u : Subalgebra.center R U) (c : R) :
    M →ₗ⁅R,L⁆ M where
  toLinearMap := representation R L M (u : U) - c • LinearMap.id
  map_lie' := by
    intro x w
    simp [representation_lie_of_mem_center R L M u.2 x w]

private theorem mem_ker_representationSubSmulId_iff {u : Subalgebra.center R U} {c : R} {m : M} :
    m ∈ (representationSubSmulId R L M u c).ker ↔ representation R L M (u : U) m = c • m := by
  rw [LieModuleHom.mem_ker]
  simp [representationSubSmulId, sub_eq_zero]

/-- **The eigen-submodule of a central element of `U(L)`**: the vectors on which a central `u` acts
by the scalar `c`. Centrality makes this locus stable under the Lie action, so it is a Lie
submodule and not merely an `R`-submodule; it is the kernel of the equivariant map `ρ u - c`. -/
noncomputable def centralEigenLieSubmodule (u : Subalgebra.center R U) (c : R) :
    LieSubmodule R L M :=
  (representationSubSmulId R L M u c).ker

@[simp]
theorem mem_centralEigenLieSubmodule {u : Subalgebra.center R U} {c : R} {m : M} :
    m ∈ centralEigenLieSubmodule R L M u c ↔ representation R L M (u : U) m = c • m :=
  mem_ker_representationSubSmulId_iff R L M
...
theorem representation_eq_smul_of_mem_center_of_lieSpan_eq_top
  ...
  have htop : centralEigenLieSubmodule R L M ⟨u, hu⟩ c = ⊤ := by
    rw [← top_le_iff, ← hgen]
    exact LieSubmodule.lieSpan_le.mpr (by
      simpa using (mem_centralEigenLieSubmodule R L M (u := ⟨u, hu⟩) (c := c) (m := v)).mpr hv)
  exact (mem_centralEigenLieSubmodule R L M).mp (htop ▸ LieSubmodule.mem_top m)
