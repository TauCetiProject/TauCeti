...
* `TauCeti.prodRepresentation_kostantForm_apply_mem`: the product
...
/-- The product of two enveloping-algebra representations, acting componentwise on the product
of their carriers. -/
...
/-- A product representation preserves the product lattice when each factor preserves its
respective lattice. -/
theorem prodRepresentation_apply_mem
  ...
  exact ⟨hM u v.1 hv.1, hN u v.2 hv.2⟩

/-- The product of two lattices stable under a Kostant form is stable under the product
representation. This supplies the admissible lattice of a direct sum once its two summands are
known to be admissible. -/
theorem prodRepresentation_kostantForm_apply_mem (e : ι → L) (h : κ → L)
    (ρ : _root_.UniversalEnvelopingAlgebra ℚ L →ₐ[ℚ] Module.End ℚ V)
    (σ : _root_.UniversalEnvelopingAlgebra ℚ L →ₐ[ℚ] Module.End ℚ W)
    (M : Submodule ℤ V) (N : Submodule ℤ W)
    (hM : ∀ u ∈ UniversalEnvelopingAlgebra.kostantForm e h, ∀ v ∈ M, ρ u v ∈ M)
    (hN : ∀ u ∈ UniversalEnvelopingAlgebra.kostantForm e h, ∀ w ∈ N, σ u w ∈ N)
    (u : _root_.UniversalEnvelopingAlgebra ℚ L)
    (hu : u ∈ UniversalEnvelopingAlgebra.kostantForm e h)
    (v : V × W) (hv : v ∈ M.prod N) :
    prodRepresentation ρ σ u v ∈ M.prod N := by
  rw [Submodule.mem_prod] at hv ⊢
