...
Let `K` be a field of characteristic `p` and `L` a finite-dimensional Lie `K`-algebra.  A
*linearized polynomial*, or `p`-polynomial, in an element `u` of a `K`-algebra is a `K`-linear
...
* `LieAlgebra.ad K L x` lives in the finite-dimensional space `Module.End K L`, so the chain of
  subspaces spanned by the initial segments of the sequence `(LieAlgebra.ad K L x ^ p ^ i)`
  cannot grow forever.  The first repetition is a monic linearized relation.
...
⚠ Centrality is *not* natural for an arbitrary Lie homomorphism `f : L →ₗ⁅K⁆ L'`.  For the
abelian `L = K x` one may take the polynomial `ι x` itself, since `LieAlgebra.ad K L x = 0`; its
...
* `TauCeti.UniversalEnvelopingAlgebra.mem_center_of_ad_pPolynomial_eq_zero`: a monic linearized
  relation satisfied by `LieAlgebra.ad K L x` makes the corresponding polynomial in `ι x`
  central.
* `TauCeti.UniversalEnvelopingAlgebra.exists_pCentralPolynomial`: every element of a
  finite-dimensional Lie algebra over a field of characteristic `p` has such a polynomial.
...
* `TauCeti.UniversalEnvelopingAlgebra.exists_pow_ι_mem_center_of_isNilpotent_ad`: if
  `LieAlgebra.ad K L x` is nilpotent the polynomial may be taken to be a single Frobenius power
  `ι x ^ p ^ e`, and then `LieAlgebra.ad K U(L) (ι x)` is itself nilpotent.
...
/-- In a Noetherian module, some term of a sequence is a linear combination of its predecessors:
the spans of the initial segments form an increasing chain, which must stabilize. -/
private theorem exists_sum_smul_eq_of_isNoetherian {M : Type w} [AddCommGroup M] [Module K M]
    [IsNoetherian K M] (v : ℕ → M) :
    ∃ (n : ℕ) (c : Fin n → K), ∑ i : Fin n, c i • v i = v n := by
  obtain ⟨n, hn⟩ := monotone_stabilizes_iff_noetherian.mpr ‹IsNoetherian K M›
    ⟨fun m => Submodule.span K (v '' Set.Iio m),
      fun i j hij => Submodule.span_mono (Set.image_mono (Set.Iio_subset_Iio hij))⟩
  have hstep : Submodule.span K (v '' Set.Iio n) = Submodule.span K (v '' Set.Iio (n + 1)) :=
    hn (n + 1) (Nat.le_succ n)
  have hrange : (Set.range fun i : Fin n => v i) = v '' Set.Iio n := by
    ext z
    exact ⟨fun ⟨i, hi⟩ => ⟨i, i.isLt, hi⟩, fun ⟨i, hi, hiz⟩ => ⟨⟨i, hi⟩, hiz⟩⟩
  have hmem : v n ∈ Submodule.span K (Set.range fun i : Fin n => v i) := by
    rw [hrange, hstep]
...
Frobenius powers of `LieAlgebra.ad K L x` satisfy the monic relation with coefficients `a`, then
...
Lie generators is the polynomial in `LieAlgebra.ad K L x`.  A derivation of `U(L)` vanishing on
the Lie generators vanishes. -/
theorem mem_center_of_ad_pPolynomial_eq_zero (p : ℕ) [ExpChar K p] {e : ℕ} {a : Fin e → K} {x : L}
    (h : LieAlgebra.ad K L x ^ p ^ e +
        ∑ i : Fin e, a i • LieAlgebra.ad K L x ^ p ^ (i : ℕ) = 0) :
  ...
  have hlin : LieAlgebra.ad K U (_root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ e +
        ∑ i : Fin e, a i • _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ (i : ℕ)) =
      LieAlgebra.ad K U (_root_.UniversalEnvelopingAlgebra.ι K x) ^ p ^ e +
        ∑ i : Fin e,
          a i • LieAlgebra.ad K U (_root_.UniversalEnvelopingAlgebra.ι K x) ^ p ^ (i : ℕ) := by
    rw [map_add, map_sum]
    simp only [map_smul, LieAlgebra.ad_pow_expChar_pow]
  -- On the Lie generators it is the image of the polynomial in `LieAlgebra.ad K L x`.
  have happ : LieAlgebra.ad K U (_root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ e +
          ∑ i : Fin e, a i • _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ (i : ℕ))
        (_root_.UniversalEnvelopingAlgebra.ι K y) =
      _root_.UniversalEnvelopingAlgebra.ι K ((LieAlgebra.ad K L x ^ p ^ e +
        ∑ i : Fin e, a i • LieAlgebra.ad K L x ^ p ^ (i : ℕ)) y) := by
    rw [hlin]
    simp only [LinearMap.add_apply, LinearMap.sum_apply, LinearMap.smul_apply, ad_ι_pow_apply_ι,
      map_add, map_sum, map_smul]
  have hzero : _root_.UniversalEnvelopingAlgebra.ι K ((LieAlgebra.ad K L x ^ p ^ e +
      ∑ i : Fin e, a i • LieAlgebra.ad K L x ^ p ^ (i : ℕ)) y) = 0 := by
    rw [h, LinearMap.zero_apply, map_zero]
  exact happ.trans hzero

variable (K L)

/-- **Every element of a finite-dimensional Lie algebra has a central `p`-polynomial.**  For
`x : L` there are an exponent `e` and coefficients `a` making the monic linearized polynomial
...
Finite-dimensionality enters only through `Module.End K L`, where the Frobenius powers of
`LieAlgebra.ad K L x` cannot stay linearly independent. -/
theorem exists_pCentralPolynomial (p : ℕ) [Fact p.Prime] [CharP K p] [FiniteDimensional K L]
    ...
    ∃ (e : ℕ) (a : Fin e → K),
      _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ e +
        ...
        Subalgebra.center K U := by
  obtain ⟨e, c, hc⟩ :=
    exists_sum_smul_eq_of_isNoetherian K fun i : ℕ => LieAlgebra.ad K L x ^ p ^ i
  refine ⟨e, fun i => -c i, mem_center_of_ad_pPolynomial_eq_zero p ?_⟩
  have hneg : ∑ i : Fin e, (-c i) • LieAlgebra.ad K L x ^ p ^ (i : ℕ) =
      -∑ i : Fin e, c i • LieAlgebra.ad K L x ^ p ^ (i : ℕ) := by
    simp [neg_smul]
...
`TauCeti.UniversalEnvelopingAlgebra.exists_pCentralPolynomial` this places the central
`p`-polynomial of an element in `Z(U(L)) ∩ U⁺(L)`. -/
theorem pPolynomial_ι_mem_augmentation_toIdeal {p : ℕ} (hp : p ≠ 0) (e : ℕ) (a : Fin e → K)
    (x : L) :
    _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ e +
        ∑ i : Fin e, a i • _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ (i : ℕ) ∈
      (HopfIdeal.augmentation K U).toIdeal := by
  refine Ideal.add_mem _ (pow_ι_mem_augmentation_toIdeal K L x (pow_ne_zero e hp))
    (Submodule.sum_mem _ fun i _ => ?_)
  rw [Algebra.smul_def]
  exact Ideal.mul_mem_left _ _ (pow_ι_mem_augmentation_toIdeal K L x (pow_ne_zero _ hp))

variable {K L}

/-- **The adjoint-nilpotent specialization.**  When `LieAlgebra.ad K L x` is nilpotent the
...
quotient. -/
theorem exists_pow_ι_mem_center_of_isNilpotent_ad (p : ℕ) [ExpChar K p] (hp : p ≠ 1) {x : L}
    (h : IsNilpotent (LieAlgebra.ad K L x)) :
    ∃ e : ℕ, _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ e ∈ Subalgebra.center K U := by
  obtain ⟨k, hk⟩ := h
  have hp1 : 1 < p := by have := expChar_pos K p; omega
  have hzero : LieAlgebra.ad K L x ^ p ^ k = 0 := by
    rw [← Nat.sub_add_cancel (Nat.le_of_lt (Nat.lt_pow_self hp1)), pow_add, hk, mul_zero]
  refine ⟨k, ?_⟩
  simpa using
    mem_center_of_ad_pPolynomial_eq_zero (a := (0 : Fin k → K)) p (by simp [hzero])

/-- **Adjoint nilpotence passes to the enveloping algebra.**  In characteristic `p`, if
`LieAlgebra.ad K L x` is nilpotent on `L` then the inner derivation of `U(L)` attached to `ι x` is
...
/-- **Centrality of a `p`-polynomial is natural along surjections.**  If `f : L →ₗ⁅K⁆ L'` is
...
docstring. -/
theorem pPolynomial_map_mem_center_of_surjective {L' : Type w} [LieRing L'] [LieAlgebra K L']
    (f : L →ₗ⁅K⁆ L') (hf : Function.Surjective f) {p e : ℕ} {a : Fin e → K} {x : L}
    (h : _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ e +
        ∑ i : Fin e, a i • _root_.UniversalEnvelopingAlgebra.ι K x ^ p ^ (i : ℕ) ∈
      Subalgebra.center K U) :
    _root_.UniversalEnvelopingAlgebra.ι K (f x) ^ p ^ e +
        ∑ i : Fin e, a i • _root_.UniversalEnvelopingAlgebra.ι K (f x) ^ p ^ (i : ℕ) ∈
      Subalgebra.center K (_root_.UniversalEnvelopingAlgebra K L') := by
  have himg := Subalgebra.map_center_le_center (f := map K f) (map_surjective_of_surjective K f hf)
  refine himg (Subalgebra.mem_map.mpr ⟨_, h, ?_⟩)
  have hpow : ∀ n : ℕ, map K f (_root_.UniversalEnvelopingAlgebra.ι K x ^ n) =
      _root_.UniversalEnvelopingAlgebra.ι K (f x) ^ n := fun n => by rw [map_pow, map_ι]
