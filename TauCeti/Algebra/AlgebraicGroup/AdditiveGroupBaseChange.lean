...
the inverse map on scalar multiples of generators, and the one-dimensional additive group
`𝔾ₐ`. Naturality in the value algebra is not restated here: it is the composite of the
generic base-change naturality (`TauCeti.AlgHom.mapValue_baseChangePointsMulEquiv`) with the
naturality of the `k`-points calculation (`TauCeti.AdditiveGroup.pointsMulEquiv_mapValue`).
