...
theorem derivedDefiningIdeal_eq_ofPoints_commutator :
    derivedDefiningIdeal (R := k) H =
      HopfIdeal.ofPoints (commutator (WithConv (H →ₐ[k] k))) := by
  apply le_antisymm
  · exact (HopfIdeal.le_ofPoints_iff _ _).mpr
  ...
  · apply (le_derivedDefiningIdeal_iff H _).mpr
    ...
    rw [← AlgHom.comp_apply, HopfAlgebra.productMap_comp_commutatorAlgHom
      (toConv g) (toConv h)]
    exact (HopfIdeal.mem_ofPoints _ x).mp hx
      ⟨⁅toConv g, toConv h⁆,
...
theorem mem_derivedDefiningIdeal_iff_forall_commutator (x : H) :
    x ∈ derivedDefiningIdeal (R := k) H ↔
      ∀ g : commutator (WithConv (H →ₐ[k] k)), g.val.ofConv x = 0 := by
  rw [derivedDefiningIdeal_eq_ofPoints_commutator, HopfIdeal.mem_ofPoints]
