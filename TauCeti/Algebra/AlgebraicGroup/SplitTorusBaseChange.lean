...
coordinate characters `1 ⊗ single (ofAdd (single i 1)) 1` and records the inverse
evaluation. Naturality in the value algebra is not restated here: it is the composite of the
generic base-change naturality (`TauCeti.AlgHom.mapValue_baseChangePointsMulEquiv`) with the
naturality of the `k`-points calculation
(`TauCeti.DiagonalizableGroup.pointsMulEquiv_mapValue` and `freeAbelianCharEquiv_comp`).
