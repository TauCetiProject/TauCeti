...
standard generator `1 ⊗ single (ofAdd 1) 1` and how the inverse point evaluates on scalar
multiples of that generator. Naturality in the value algebra is not restated here: it is the
composite of the generic base-change naturality
(`TauCeti.AlgHom.mapValue_baseChangePointsMulEquiv`) with the naturality of the `k`-points
calculation (`TauCeti.RootsOfUnityGroup.pointsMulEquiv_mapValue`).
