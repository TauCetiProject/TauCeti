...
/-- Simp-normal form of `quotientKerPointsMulEquiv_apply`, with the kernel quotient spelled
using the ring-hom kernel. -/
@[simp]
theorem quotientKerPointsMulEquiv_apply_ringHomKer (f : H →ₐc[R] K)
    ...
    quotientKerPointsMulEquiv f hf A g =
      AlgHom.mapDomain (A := A)
        ((kerLiftBialgEquiv f hf).symm : K →ₐc[R] H ⧸ (ker f hf).toIdeal) g := by
  simpa [ker_toIdeal] using quotientKerPointsMulEquiv_apply f hf A g
...
/-- Simp-normal form of `quotientKerPointsMulEquiv_symm_apply`, with the kernel quotient
spelled using the ring-hom kernel. -/
@[simp]
theorem quotientKerPointsMulEquiv_symm_apply_ringHomKer (f : H →ₐc[R] K)
    ...
    (quotientKerPointsMulEquiv f hf A).symm g =
      AlgHom.mapDomain (A := A)
        (kerLiftBialgEquiv f hf : H ⧸ (ker f hf).toIdeal →ₐc[R] K) g := by
  simpa [ker_toIdeal] using quotientKerPointsMulEquiv_symm_apply f hf A g
...
/-- Simp-normal form of
`quotientPointsHom_quotientKerPointsMulEquiv_symm_apply`, with the kernel quotient spelled
using the ring-hom kernel. -/
@[simp]
theorem quotientPointsHom_quotientKerPointsMulEquiv_symm_apply_ringHomKer
    (f : H →ₐc[R] K) (hf : Function.Surjective f) (A : CommAlgCat.{x} R)
    (g : HopfAlgebra.points (R := R) (H := K) A) :
    CommHopfAlgCat.quotientPointsHom (_root_.CommHopfAlgCat.of R H) (ker f hf) A
        (((quotientKerPointsMulEquiv f hf A).symm g :
      ...
      AlgHom.mapDomain f g := by
  simpa [ker_toIdeal] using quotientPointsHom_quotientKerPointsMulEquiv_symm_apply f hf A g
...
/-- Simp-normal form of `quotientPointsHom_quotientKerPointsMulEquiv_apply`, with the
kernel quotient spelled using the ring-hom kernel. -/
@[simp]
theorem quotientPointsHom_quotientKerPointsMulEquiv_apply_ringHomKer (f : H →ₐc[R] K)
    (hf : Function.Surjective f) (A : CommAlgCat.{x} R)
    (g : WithConv (H ⧸ RingHom.ker (f : H →ₐ[R] K) →ₐ[R] A)) :
    CommHopfAlgCat.quotientPointsHom (_root_.CommHopfAlgCat.of R H) (ker f hf) A g =
      AlgHom.mapDomain f (quotientKerPointsMulEquiv f hf A g) := by
  simpa [ker_toIdeal] using quotientPointsHom_quotientKerPointsMulEquiv_apply f hf A g
