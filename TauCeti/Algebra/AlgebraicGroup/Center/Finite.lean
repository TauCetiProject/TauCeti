...
Let `H` be a finite-type commutative Hopf algebra over an algebraically closed field. This file
shows that its center is finite once its reduced center is finite. The quotient map from the
center to its reduction has nilradical kernel; finite type makes that kernel finitely generated,
so finiteness lifts through it.
...
* `moduleFinite_centerCoordinate_of_reducedCenter_identityComponent_eq_augmentation`:
  a center is finite when its reduction has trivial identity component.
...
/-- A finite-type affine group's center is finite when the identity component of its reduced
center is the trivial subgroup scheme. -/
theorem
    moduleFinite_centerCoordinate_of_reducedCenter_identityComponent_eq_augmentation
    [IsAlgClosed k]
