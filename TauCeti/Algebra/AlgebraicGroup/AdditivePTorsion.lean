...
/-!
# The additive `p`-torsion group scheme `αₚ`
...
`R[x] / (xᵖ)`, and for every commutative `R`-algebra `A` its `A`-points are the `p`-torsion of
the additive group: the elements `a ∈ A` with `aᵖ = 0`.

This file builds `αₚ` as the quotient of the additive-group Hopf algebra by the Hopf ideal
generated by `xᵖ`, and identifies its functor of points with this `p`-torsion subgroup.
...
`TauCeti.AdditiveGroup.gaPointsMulEquiv`. The image consists of exactly the `p`-torsion
elements, and the pre-composition map is injective because the quotient map is surjective, so
`αₚ(A)` is the additive group of `p`-torsion of `A`. This exhibits `αₚ` as a non-smooth,
...
* `TauCeti.AlphaP.pointsHom`: the injective homomorphism from the convolution group of points of
  `αₚ` to the additive group `Multiplicative A`, landing in the `p`-torsion.
* `TauCeti.AlphaP.mem_range_pointsHom_iff`: a point value `a` comes from `αₚ` iff `aᵖ = 0`, so
  the functor of points of `αₚ` is the `p`-torsion of the additive group.
...
antipode-stable. Its quotient is the coordinate ring of the additive `p`-torsion group scheme
`αₚ`. -/
...
/-- **The functor of points of `αₚ` is the `p`-torsion of the additive group.** An element `a` of
`A` is the value of a point of `αₚ` iff `aᵖ = 0`. -/
