...
theorem tensorCoeffEquiv_apply (t : V ⊗[R] MonoidAlgebra R G) (g : G) :
    tensorCoeffEquiv R G V t g =
      TauCeti.LinearMap.tensorComponent (R := R) (M := V) (monoidCoeff R G g) t := by
  classical
  have h : (Finsupp.lapply g).comp (tensorCoeffEquiv R G V).toLinearMap =
      TauCeti.LinearMap.tensorComponent (R := R) (M := V) (monoidCoeff R G g) :=
...
theorem weightProj_weightProj_of_ne {h g : G} (hne : h ≠ g) (v : V) :
  ...
  simpa only [LinearMap.coe_comp, Function.comp_apply, TauCeti.LinearMap.tensorComponent_zero,
...
theorem map_weightProj (f : Hom R (MonoidAlgebra R G) V W) (g : G) (v : V) :
  ...
  calc
    f (TauCeti.LinearMap.tensorComponent (monoidCoeff R G g)
        (coact (R := R) (C := MonoidAlgebra R G) v)) =
      TauCeti.LinearMap.tensorComponent (monoidCoeff R G g)
        (TensorProduct.map f.toLinearMap LinearMap.id
          (coact (R := R) (C := MonoidAlgebra R G) v)) := by
            ...
            simpa only [LinearMap.comp_id] using
              (TauCeti.LinearMap.tensorComponent_map (monoidCoeff R G g) f.toLinearMap
                LinearMap.id (coact (R := R) (C := MonoidAlgebra R G) v)).symm
    _ = _ := congrArg
      (TauCeti.LinearMap.tensorComponent (R := R) (M := W) (monoidCoeff R G g))
      (f.map_coact_apply v)
