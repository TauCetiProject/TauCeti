...
theorem tensorCoeffEquiv_apply (t : V ⊗[R] MonoidAlgebra R G) (g : G) :
    tensorCoeffEquiv R G V t g =
      TensorProduct.tensorComponent (R := R) (M := V) (monoidCoeff R G g) t := by
  classical
  have h : (Finsupp.lapply g).comp (tensorCoeffEquiv R G V).toLinearMap =
      TensorProduct.tensorComponent (R := R) (M := V) (monoidCoeff R G g) :=
...
theorem weightProj_weightProj_of_ne {h g : G} (hne : h ≠ g) (v : V) :
  ...
  simpa only [LinearMap.coe_comp, Function.comp_apply, TensorProduct.tensorComponent_zero,
