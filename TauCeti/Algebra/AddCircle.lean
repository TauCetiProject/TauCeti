...
cyclic of order `n` with a distinguished generator, the one of invariant `1 / n`. The instance
`Rat.fact_zero_lt_one` is supplied so that the positivity hypothesis of this API is available
for `AddCircle (1 : ℚ)`.
...
/-- The period of the rational circle `ℚ/ℤ = AddCircle (1 : ℚ)` is positive.
...
theorem torsionBy_le_torsionBy_iff (hm : 0 < m) (hn : 0 < n) :
  ...
  refine ⟨fun h ↦ ?_, fun ⟨c, hc⟩ u hu ↦ torsionBy.nsmul_iff.mpr ?_⟩
  ...
  · rw [hc, mul_nsmul, torsionBy.nsmul_iff.mp hu, smul_zero]
