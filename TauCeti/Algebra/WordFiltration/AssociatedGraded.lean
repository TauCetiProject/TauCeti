...
* `TauCeti.Algebra.wordFiltration.gradedPiece_mk_eq_zero_iff`: a class in a graded piece vanishes
  exactly when its representative lies in the preceding filtration step.
...
theorem gradedPiece_mk_eq_zero_iff (f : M →ₗ[R] A) {k : ℕ} (x : wordFiltration f k) :
