...
public import TauCeti.NumberTheory.Modular.Acyclicity
...
/-- For `n ≠ 0`, an integral matrix `g` with `g A = c A` for a determinant-`n` matrix `A` is the
scalar `c`: multiply on the right by the adjugate of `A`. -/
private lemma eq_smul_one_of_mul_eq_smul (hn : n ≠ 0) {g : Matrix (Fin 2) (Fin 2) ℤ}
    {A : TraceFormulaMatrix n} {c : ℤ} (h : g * A.1 = c • A.1) : g = c • 1 := by
  have h' := congrArg (· * adjugate A.1) h
  simp only [Matrix.mul_assoc, Matrix.smul_mul, Matrix.mul_adjugate, A.2, Matrix.mul_smul,
    Matrix.mul_one, smul_comm c n] at h'
  exact smul_right_injective _ hn h'

/-- For `n ≠ 0`, only `±1` in `SL(2, ℤ)` fixes an element of `ℳₙ`. -/
theorem eq_one_or_eq_neg_one_of_smul_eq (hn : n ≠ 0) {g : SL(2, ℤ)}
  ...
  · refine Or.inl (Subtype.ext ?_)
    simpa using eq_smul_one_of_mul_eq_smul hn (c := 1)
      (by simpa [FixedDetMatrices.smul_coe] using congrArg Subtype.val h)
...
theorem isCancelSMul (hn : n ≠ 0) : IsCancelSMul PSL(2, ℤ) (TraceFormulaMatrixModule n) := by
  ...
  exact eq_one_or_eq_neg_one_of_smul_eq hn h
...
theorem ker_one_add_S_inf_ker_one_add_T_mul_S_add_sq_eq_bot {k : Type*} [Ring k] (hn : n ≠ 0) :
  ...
  have hρ (g : SL(2, ℤ)) :
    ...
    simp [coe_smul]
  rw [hρ, hρ]
  exact ModularGroup.ker_one_add_S_inf_ker_one_add_T_mul_S_add_sq_eq_bot
