...
* `TauCeti.TraceFormulaMatrixModule.commute_ofMulAction`: the left and right representations
  commute, so `ℛₙ` is a bimodule.
...
/-- The left and right permutation representations on `k[ℳₙ]` commute. -/
theorem commute_ofMulAction (g : PSL(2, ℤ)) (h : PSL(2, ℤ)ᵐᵒᵖ) :
    Commute (Representation.ofMulAction k PSL(2, ℤ) (TraceFormulaMatrixModule n) g)
      (Representation.ofMulAction k PSL(2, ℤ)ᵐᵒᵖ (TraceFormulaMatrixModule n) h) := by
  ext
  simp [smul_comm g h]
