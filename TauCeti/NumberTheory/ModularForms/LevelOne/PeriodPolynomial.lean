...
For odd `w` the central element `S² = -1` acts by `-1`, so `W_w = 0` as soon as `R` has no
additive torsion; only even `w`, that is even weight, carries period polynomials.

The Eisenstein polynomial `X^w - Y^w` is an even period polynomial for every even `w`; up to a
nonzero scalar it is the extended even period polynomial of the Eisenstein series of weight
`w + 2`.
...
* `TauCeti.binaryFormRep_parity_mem_periodPolynomials`: for even `w`, the parity involution
  preserves `W_w`.
* `TauCeti.evenPeriodPolynomials_sup_oddPeriodPolynomials` and
  ...
  `w`, when `2` is invertible (spanning) and `R` has no additive torsion (disjointness).
* `TauCeti.periodPolynomials_eq_bot_of_odd`: `W_w = 0` for odd `w` when `R` has no additive
  torsion.
* `TauCeti.eisensteinPeriodPolynomial_mem_evenPeriodPolynomials`: `X^w - Y^w ∈ W_w^+` for even
  `w`.
...
polynomials. -/
theorem binaryFormRep_parity_mem_periodPolynomials (hw : Even w)
...
theorem evenPeriodPolynomials_sup_oddPeriodPolynomials [Invertible (2 : R)] (hw : Even w) :
  ...
  have hεP := binaryFormRep_parity_mem_periodPolynomials hw hP
...
private lemma eq_zero_of_add_self_eq_zero [IsAddTorsionFree R]
  ...
  rw [← two_nsmul, ← nsmul_zero 2] at hm
  simpa using nsmul_right_injective two_ne_zero hm

/-- A period polynomial cannot be both even and odd when `R` has no additive torsion. -/
theorem disjoint_evenPeriodPolynomials_oddPeriodPolynomials [IsAddTorsionFree R] :
    Disjoint (evenPeriodPolynomials R w) (oddPeriodPolynomials R w) :=
  Submodule.disjoint_def.2 fun _ hP hP' ↦ eq_zero_of_add_self_eq_zero <| eq_neg_iff_add_eq_zero.1
    ((mem_evenPeriodPolynomials_iff.1 hP).2.symm.trans (mem_oddPeriodPolynomials_iff.1 hP').2)

/-- For odd `w` there are no nonzero period polynomials when `R` has no additive torsion: the
central element `S² = -1` acts by `-1` but fixes every `P` with `P ∣ S = -P`. -/
theorem periodPolynomials_eq_bot_of_odd [IsAddTorsionFree R] (hw : Odd w) :
    periodPolynomials R w = ⊥ := by
  refine (Submodule.eq_bot_iff _).2 fun P hP ↦ eq_zero_of_add_self_eq_zero ?_
  have hSP := eq_neg_of_add_eq_zero_right (mem_periodPolynomials_iff.1 hP).1
  have hSS := binaryFormRep_op_mul_apply (R := R) (w := w) (S : Matrix (Fin 2) (Fin 2) ℤ)
    (S : Matrix (Fin 2) (Fin 2) ℤ) P
  rw [S_mul_S_eq, binaryFormRep_op_neg, hw.neg_one_pow, hSP, map_neg, hSP, neg_neg, op_one,
    map_one, LinearMap.smul_apply, Module.End.one_apply, neg_one_smul] at hSS
  exact eq_neg_iff_add_eq_zero.1 hSS.symm

/-! ### The Eisenstein polynomial -/

variable (R w)

/-- The binary form `X^w - Y^w`. For even `w` it is an even period polynomial, and up to a nonzero
scalar the extended even period polynomial of the Eisenstein series of weight `w + 2`. -/
...
theorem eisensteinPeriodPolynomial_mem_evenPeriodPolynomials (hw : Even w) :
