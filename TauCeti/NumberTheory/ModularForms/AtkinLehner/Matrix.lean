...
(`IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_eq` and its right-handed twin), and a form on which
`Γ₀(N)` acts trivially cannot tell them apart. The two-sided version of that statement is that
...
The family is multiplicative in the exact divisor: for coprime `Q` and `R`, a product of an
Atkin–Lehner matrix for `Q` and one for `R` is an Atkin–Lehner matrix for `Q * R`
(`IsAtkinLehnerMatrix.mul`). Since every exact divisor is a product of the maximal prime powers
it contains (`TauCeti.IsExactDivisor.mul`), the family is generated by the prime-power members,
and squaring stays inside `Γ₀(N)` up to the scalar `Q` (`exists_mem_Gamma0_mul_self`) — the
matrix-level source of the involution `𝒲_Q ^ 2 = 1` in even weight.
...
* `TauCeti.IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_eq`,
  `TauCeti.IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_eq_mul`: any two members for the same `Q`
  differ by an element of `Γ₀(N)`, on the left and on the right respectively.
* `TauCeti.IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_comm`: `W` normalizes `Γ₀(N)`.
* `TauCeti.IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_self`: `W ^ 2 = Q • γ` with `γ ∈ Γ₀(N)`.
* `TauCeti.IsAtkinLehnerMatrix.mul`: the coprime multiplicativity of the family.
...
theorem isAtkinLehnerMatrix_atkinLehnerMatrix (h : IsExactDivisor Q N) :
...
theorem IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_eq (hQ : Q ≠ 0) (hQN : Q ∣ N)
...
`IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_eq` with witness `W⁻¹ W'`. -/
...
theorem IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_comm (hQ : Q ≠ 0) (hQN : Q ∣ N)
    (h : IsAtkinLehnerMatrix N Q M) {γ : SL(2, ℤ)} (hγ : γ ∈ Gamma0 N) :
    ∃ δ : SL(2, ℤ), δ ∈ Gamma0 N ∧
      M * (γ : Matrix (Fin 2) (Fin 2) ℤ) = (δ : Matrix (Fin 2) (Fin 2) ℤ) * M :=
  h.exists_mem_Gamma0_mul_eq hQ hQN (h.mul_right hQN hγ)
...
theorem IsAtkinLehnerMatrix.exists_mem_Gamma0_mul_comm' (hQ : Q ≠ 0) (hQN : Q ∣ N)
  ...
  h.exists_mem_Gamma0_mul_eq_mul hQ hQN (h.mul_left hQN hγ)
...
/-- **Coprime multiplicativity of the family.** With `Q` and `R` coprime divisors of `N`, an
Atkin–Lehner matrix for `Q` times one for `R` is an Atkin–Lehner matrix for `Q * R` — which is
again an exact divisor when `Q` and `R` are (`TauCeti.IsExactDivisor.mul`). -/
theorem IsAtkinLehnerMatrix.mul (hQN : Q ∣ N) (hRN : R ∣ N) (hQR : Nat.Coprime Q R)
    (h : IsAtkinLehnerMatrix N Q M) (h' : IsAtkinLehnerMatrix N R M') :
    IsAtkinLehnerMatrix N (Q * R) (M * M') := by
  have hQRN : ((Q * R : ℕ) : ℤ) ∣ (N : ℤ) :=
    Int.natCast_dvd_natCast.mpr (Nat.Coprime.mul_dvd_of_dvd_of_dvd hQR hQN hRN)
