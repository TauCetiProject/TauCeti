...
the two form spaces. The group is abstractly `(ℤ/2) ^ ω(N)`, generated by the maximal prime-power
exact divisors. The action may have a kernel: nothing here claims it is injective.
