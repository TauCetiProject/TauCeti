...
The exact divisors of `N` form a Boolean algebra under the subsets of `N.primeFactors`, and the
normalized operators **act** through it: for exact divisors `Q` and `R`,
...
In particular all of them **commute**, and the family is generated by the members at the maximal
prime powers `p ^ (N.factorization p)`, on which the abstract group `(ℤ/2) ^ ω(N)` acts. The
action may have a kernel: nothing here claims it is injective.
...
theorem normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp (hQ : Q ∥ N)
  ...
  have hrq : Nat.Coprime (R / Nat.gcd Q R) (Q / Nat.gcd Q R) :=
    hrQ.coprime_dvd_right (Nat.div_dvd_of_dvd (Nat.gcd_dvd_left Q R))
  have hQex : ∀ x, hq.normalizedAtkinLehnerOperatorCusp k
      (hg.normalizedAtkinLehnerOperatorCusp k x) =
      hQ.normalizedAtkinLehnerOperatorCusp k x := fun x ↦ by
    rw [hg.normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp_of_coprime hq hgq]
    exact normalizedAtkinLehnerOperatorCusp_congr _ hQ
      (Nat.mul_div_cancel' (Nat.gcd_dvd_left Q R)) k x
  have hRex : ∀ x, hr.normalizedAtkinLehnerOperatorCusp k
      (hg.normalizedAtkinLehnerOperatorCusp k x) =
      hR.normalizedAtkinLehnerOperatorCusp k x := fun x ↦ by
    rw [hg.normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp_of_coprime hr hgr]
    exact normalizedAtkinLehnerOperatorCusp_congr _ hR
      (Nat.mul_div_cancel' (Nat.gcd_dvd_right Q R)) k x
  have hswap : ∀ y, hg.normalizedAtkinLehnerOperatorCusp k
      (hr.normalizedAtkinLehnerOperatorCusp k y) =
      hr.normalizedAtkinLehnerOperatorCusp k
        (hg.normalizedAtkinLehnerOperatorCusp k y) := fun y ↦ by
    rw [hr.normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp_of_coprime hg
        hgr.symm,
      hg.normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp_of_coprime hr hgr]
    exact normalizedAtkinLehnerOperatorCusp_congr _ _ (Nat.mul_comm _ _) k y
  rw [← hRex f, ← hQex (hr.normalizedAtkinLehnerOperatorCusp k
      (hg.normalizedAtkinLehnerOperatorCusp k f)), hswap,
    hg.normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp_self,
    hr.normalizedAtkinLehnerOperatorCusp_normalizedAtkinLehnerOperatorCusp_of_coprime hq hrq]
  refine normalizedAtkinLehnerOperatorCusp_congr _ _ ?_ k f
  rw [mul_div_gcd_sq_eq]
  exact Nat.mul_comm _ _
