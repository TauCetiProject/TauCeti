...
The argument is a strong induction on the level. At level `N` the cusp forms split into the old
and the new subspace. The newforms of level `N` span the new subspace. The old subspace is
spanned by the level-raises `V_d f` of forms `f` of proper divisor levels `M`; by induction, `f`
is a combination of level-raised newforms of levels dividing `M`, and `V_d ∘ V_e = V_{de}`. So
the old subspace is generated by the level-raised *newspaces* of the proper divisor levels.
...
* `HeckeRing.GL2.Newform.mem_span_levelRaise_of_forall_heckeTCuspNat_eq_smul`: a simultaneous
  ...
  eigenvalues.
* `HeckeRing.GL2.Newform.exists_eigenvalue_eq_of_forall_heckeTCuspNat_eq_smul` and
...
prime `p ∤ N`. -/
theorem exists_eigenvalue_eq_of_forall_heckeTCuspNat_eq_smul [NeZero N]
...
theorem EigenformAwayFromLevel.exists_newform_eigenvalue_eq [NeZero N]
  ...
  obtain ⟨M, _, hM, g, hg⟩ := Newform.exists_eigenvalue_eq_of_forall_heckeTCuspNat_eq_smul
    f.ne_zero (a := fun p ↦
