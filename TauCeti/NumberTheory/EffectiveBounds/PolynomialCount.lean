...
elements. Such a polynomial is determined by its `D + 1` coefficients of index `0, ..., D`;
each coefficient lies in the integer interval `[-C, C]`, which has size `2 * C + 1`.

This is the elementary counting step that the Layer 2 summit of the effective-bounds roadmap
needs. Mathlib's qualitative Hermite-Minkowski theorem (`NumberField.finite_of_discr_bdd`)
already bounds the degree and the coefficients of a generating polynomial of each number field
of bounded discriminant, but it only concludes finiteness of the resulting polynomial family.
The explicit count below turns that finiteness into an explicit upper bound input.
...
* `TauCeti.NumberField.card_intPolynomial_degree_height_le`: the explicit count
  `#{p : Z[X] | p.natDegree <= D and forall i, |p.coeff i| <= C} <=
  (2 * C + 1) ^ (D + 1)`.
...
/-- Sending a polynomial to its coefficient vector is injective on `intPolynomialBox D C`. -/
...
`(2 * C + 1) ^ (D + 1)` integer polynomials of degree at most `D` whose coefficients are all
