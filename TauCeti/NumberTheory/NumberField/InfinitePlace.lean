...
* `NumberField.isTotallyReal_of_sq_ratCast_of_pos`: a positive square generating the field forces
  total reality.
...
omit [NumberField K] in
/-- **A field generated by a positive rational square root is totally real.** If `K = ℚ(x)`,
`x² = r`, and `r > 0`, then every complex embedding of `K` is real. Each embedding sends `x` to a
complex number whose square is the positive real number `r`, so its imaginary part vanishes; the
generator hypothesis determines the embedding on all of `K`. -/
theorem isTotallyReal_of_sq_ratCast_of_pos [Algebra ℚ K] {x : K} {r : ℚ}
