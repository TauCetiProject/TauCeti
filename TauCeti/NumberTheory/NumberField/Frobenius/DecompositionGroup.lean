...
decomposition group of a nonzero prime `Q`, the subgroup generated by an arithmetic Frobenius
`σ` at `Q` together with the inertia subgroup is everything: the residue automorphism of any
element of `D(Q)` is a power of the residue Frobenius, so the element differs from the
corresponding power of `σ` by an element of the kernel of the residue action, which is the
inertia subgroup. No unramifiedness is needed. -/
theorem zpowers_sup_inertia_eq_top_of_isArithFrobAt (Q : Ideal (𝓞 L)) [Q.IsPrime] (hQ : Q ≠ ⊥)
  ...
  rw [show τ = ⟨σ, hσ.mem_stabilizer⟩ ^ (n : ℕ) * ((⟨σ, hσ.mem_stabilizer⟩ ^ (n : ℕ))⁻¹ * τ) by
    group]
  exact Subgroup.mul_mem_sup (Subgroup.npow_mem_zpowers _ _) hker

omit [IsGalois K L] in
/-- **The decomposition group is generated by a Frobenius and the inertia subgroup.** For a
nonzero prime `Q` and an arithmetic Frobenius `σ` at `Q`, the decomposition group of `Q` in
...
theorem zpowers_sup_inertia_eq_stabilizer_of_isArithFrobAt (Q : Ideal (𝓞 L)) [Q.IsPrime]
  ...
  have h := congrArg (Subgroup.map (MulAction.stabilizer (L ≃ₐ[K] L) Q).subtype)
    (zpowers_sup_inertia_eq_top_of_isArithFrobAt Q hQ hσ)
...
omit [IsGalois K L] in
/-- **Orbits of the decomposition group are orbits of a Frobenius, once inertia acts trivially.**
Let `Gal(L/K)` act on a set on which the inertia subgroup of a nonzero prime `Q` acts trivially.
Then the orbits of the decomposition group of `Q` are the orbits of the cyclic group generated by
any arithmetic Frobenius at `Q`. This is how a Frobenius at a possibly ramified prime of a
Galois closure still determines the splitting in an unramified subfield. -/
theorem orbit_stabilizer_eq_orbit_zpowers_of_isArithFrobAt {X : Type*} [MulAction (L ≃ₐ[K] L) X]
    (Q : Ideal (𝓞 L)) [Q.IsPrime] (hQ : Q ≠ ⊥) {σ : L ≃ₐ[K] L} (hσ : IsArithFrobAt (𝓞 K) σ Q)
    (hI : ∀ τ ∈ Q.inertia (L ≃ₐ[K] L), ∀ x : X, τ • x = x) (x : X) :
  ...
  · rintro _ ⟨d, rfl⟩
    ...
    beta_reduce
    rw [MulAction.subgroup_smul_def, MulAction.subgroup_smul_def, Subgroup.coe_mul, mul_smul,
      hI _ (Ideal.coe_mem_inertia.mpr hz) x, Subgroup.coe_zpow]
  · rintro _ ⟨g, rfl⟩
