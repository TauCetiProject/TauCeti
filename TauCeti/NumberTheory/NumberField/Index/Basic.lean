...
* `NumberField.RingOfIntegers.minpoly_rat_coe`: the minimal polynomial over `ℚ` of an algebraic
  integer is its minimal polynomial over `ℤ` with the coefficients cast to `ℚ`.
...
theorem _root_.NumberField.RingOfIntegers.minpoly_rat_coe (x : 𝓞 K) :
    minpoly ℚ (x : K) = (minpoly ℤ x).map (algebraMap ℤ ℚ) := by
  rw [minpoly.isIntegrallyClosed_eq_field_fractions' ℚ x.isIntegral_coe,
