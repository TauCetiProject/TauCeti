...
Units of norm `-1` are exactly the solutions of the negative Pell equation `b² - d a² = -1`, which
supplies the hypothesis concretely (`exists_norm_eq_neg_one_of_sq_sub_mul_sq_eq_neg_one`): for
`d = 2` the unit `1 + √2` qualifies.

This is the classical criterion `h⁺ = h ↔ N(ε) = -1` for the fundamental unit `ε`, and it is the
Layer 3 input that decides, for a real quadratic field, whether the ordinary class group inherits
the narrow `2`-rank `t - 1` of the multiquadratic roadmap's genus-theory formula.
...
values have the same sign and hence that `x` or `-x` is totally positive. The proof feeds
`x / σ x = x² / N(x)`, a product of totally positive elements, to
...
theorem isTotallyPositive_or_isTotallyPositive_neg_of_norm_pos
    ...
    (hx : x ≠ 0) (hnorm : 0 < Algebra.norm ℚ x) :
...
theorem radicand_pos_of_norm_eq_neg_one (hmin : minpoly ℤ θ = X ^ 2 - C d)
  ...
  · have := norm_pos_of_radicand_neg hmin hgen hd hune
    rw [hu] at this
    norm_num at this
  · exact absurd (show IsSquare (((d : ℤ) : ℚ)) from ⟨0, by rw [hd]; norm_num⟩)
      (not_isSquare_radicand hmin)
  · exact hd
...
theorem exists_units_isTotallyPositive_smul_gen_of_norm_eq_neg_one
...
/-- A nonzero element of positive norm has a totally positive multiple by `±1`. -/
private theorem exists_units_isTotallyPositive_smul (hmin : minpoly ℤ θ = X ^ 2 - C d)
    (hgen : Algebra.adjoin ℚ {(θ : K)} = ⊤) {y : K} (hy : y ≠ 0)
    (hnorm : 0 < Algebra.norm ℚ y) :
    ∃ ε : (𝓞 K)ˣ, IsTotallyPositive (ε • y) := by
  rcases isTotallyPositive_or_isTotallyPositive_neg_of_norm_pos hmin hgen hy hnorm with h | h
...
theorem toClassGroup_injective_of_norm_eq_neg_one (hmin : minpoly ℤ θ = X ^ 2 - C d)
  ...
  obtain ⟨v, hv⟩ := exists_units_isTotallyPositive_smul_gen_of_norm_eq_neg_one hmin hgen hu
  ...
  · -- Negative norm: correct it with `θ`, whose norm `-d` is negative too.
    have hy : (θ : K) * (x : K) ≠ 0 := mul_ne_zero (coe_gen_ne_zero hmin) hx
    have hypos : 0 < Algebra.norm ℚ ((θ : K) * (x : K)) := by
      rw [map_mul, norm_gen_eq_neg_radicand hmin hgen]
      nlinarith
    obtain ⟨ε, hε⟩ := exists_units_isTotallyPositive_smul hmin hgen hy hypos
    refine ⟨v * ε, ?_⟩
  ...
  · exact exists_units_isTotallyPositive_smul hmin hgen hx hgt
...
The narrow class number is the class number times the order of the kernel of `Cl⁺(K) → Cl(K)`
(`card_eq_card_classGroup_mul_card_ker`), so the two agree exactly when that kernel is trivial.
This is the classical criterion `h⁺ = h ↔ N(ε) = -1`. -/
theorem card_eq_card_classGroup_iff_exists_norm_eq_neg_one (hmin : minpoly ℤ θ = X ^ 2 - C d)
  ...
  rw [← toClassGroup_injective_iff_exists_norm_eq_neg_one hmin hgen hd,
    ← MonoidHom.ker_eq_bot_iff, Subgroup.eq_bot_iff_card, card_eq_card_classGroup_mul_card_ker]
  have hcg : 0 < Nat.card (ClassGroup (𝓞 K)) := Nat.card_pos
  constructor
  · intro h
    nth_rewrite 2 [← mul_one (Nat.card (ClassGroup (𝓞 K)))] at h
    exact Nat.eq_of_mul_eq_mul_left hcg h
  · intro h
    rw [h, mul_one]
