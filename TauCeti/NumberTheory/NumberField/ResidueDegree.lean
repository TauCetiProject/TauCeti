...
* `TauCeti.absNorm_dvd_rationalPrimeBelow_pow_finrank`: the absolute norm of `𝔭` divides
  `p ^ [K : ℚ]`, so the residue degree is at most the degree of the field.
...
* `TauCeti.asIdeal_dvd_span_singleton_algebraMap_iff`: a height-one prime divides the ideal
  generated by an integer exactly when the rational prime below it does.
...
theorem asIdeal_dvd_span_singleton_rationalPrimeBelow (𝔭 : HeightOneSpectrum (𝓞 K)) :
...
theorem absNorm_dvd_rationalPrimeBelow_pow_finrank (𝔭 : HeightOneSpectrum (𝓞 K)) :
    Ideal.absNorm 𝔭.asIdeal ∣ rationalPrimeBelow 𝔭 ^ Module.finrank ℚ K := by
  have h := map_dvd Ideal.absNorm (asIdeal_dvd_span_singleton_rationalPrimeBelow 𝔭)
...
theorem asIdeal_eq_span_singleton_of_absNorm_eq_pow_finrank {𝔭 : HeightOneSpectrum (𝓞 K)}
    ...
    𝔭.asIdeal = Ideal.span {((rationalPrimeBelow 𝔭 : ℕ) : 𝓞 K)} := by
  obtain ⟨J, hJ⟩ := asIdeal_dvd_span_singleton_rationalPrimeBelow 𝔭
...
/-- A height-one prime divides the ideal generated by an integer exactly when the rational prime
below it does. -/
theorem asIdeal_dvd_span_singleton_algebraMap_iff (𝔭 : HeightOneSpectrum (𝓞 K)) (n : ℤ) :
    𝔭.asIdeal ∣ Ideal.span {algebraMap ℤ (𝓞 K) n} ↔ (rationalPrimeBelow 𝔭 : ℤ) ∣ n := by
  rw [Ideal.dvd_span_singleton, ← Ideal.mem_comap, ← Ideal.under_def,
...
private theorem finite_primesDividing (K : Type*) [Field K] [NumberField K] {n : ℤ} (hn : n ≠ 0) :
  ...
  exact (asIdeal_dvd_span_singleton_algebraMap_iff 𝔭 n).mpr h𝔭
