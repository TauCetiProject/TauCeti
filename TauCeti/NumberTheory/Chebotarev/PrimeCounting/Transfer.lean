...
`ψ → ϑ → π` part from that analytic input and express its unweighted count using the generic
prime-counting API.
...
* `primeCount_asymptotic_of_tendsto_frobeniusPsi_div` packages the resulting asymptotic in the
  logarithmic-integral normalization.
...
theorem primeCount_asymptotic_of_tendsto_frobeniusPsi_div
    (C : ConjClasses (L ≃ₐ[K] L)) {δ : ℝ} (hδ : δ ≠ 0)
    (hψ : Tendsto (fun x : ℝ ↦ frobeniusPsi K L C x / x) atTop (𝓝 δ)) :
    primeCount K (frobeniusPrimeSet K L C) ~[atTop]
