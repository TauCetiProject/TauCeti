...
/-- Every automorphism of a number field generated by square roots of integers is an
involution, since it sends each generator to `±` itself. -/
private theorem pow_two_eq_one (hr : ∀ i, r i ^ 2 = algebraMap ℤ K (d i))
    (htop : IntermediateField.adjoin ℚ (Set.range r) = ⊤) (σ : K ≃ₐ[ℚ] K) : σ ^ 2 = 1 := by
