...
omit [Finite ι] in
/-- A generator is not equal to its own negation when the radicand is nonzero. This is the
`gen`-level specialisation of the general `TauCeti.ne_neg_of_ne_zero`. -/
theorem gen_ne_neg [NeZero (2 : K)] (hroot : ∀ i, root i ^ 2 = algebraMap K L (d i))
  ...
  exact TauCeti.ne_neg_of_ne_zero h2L hr0 hcoe
...
/-- **A field generated by square roots of base elements is Galois over the base.** If the
square roots `root i` of the `d i` generate `L` over `K` (`adjoin K (Set.range root) = ⊤`) and
`2 ≠ 0` in `K`, then `L / K` is Galois. This is `TauCeti.Multiquadratic.isGalois` transported
along `htop`. -/
theorem isGalois_of_adjoin_eq_top [NeZero (2 : K)]
    (hroot : ∀ i, root i ^ 2 = algebraMap K L (d i))
    (htop : adjoin K (Set.range root) = ⊤) : IsGalois K L := by
