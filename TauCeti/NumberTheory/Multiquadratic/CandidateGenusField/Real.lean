...
* `TauCeti.Multiquadratic.isAbelianGalois_candidateGenusFieldReal`: the real candidate is abelian
  and Galois over `ℚ`.
* `TauCeti.Multiquadratic.candidateGenusFieldRealBaseEquiv`: the quadratic base inside the real
  candidate is canonically equivalent to `candidateGenusFieldBase hd`.
...
/-- **The two embedded models of the quadratic base are canonically equivalent.** The subfield
generated by `candidateGenusFieldRealBaseRoot hd hpos` inside the real candidate lifts to the
subfield generated by `candidateGenusFieldBaseRoot hd` inside the full prime-discriminant
compositum, namely `candidateGenusFieldBase hd`. -/
noncomputable def candidateGenusFieldRealBaseEquiv (hd : Squarefree d) (hpos : 0 < d) :
    (adjoin ℚ {candidateGenusFieldRealBaseRoot hd hpos} :
  ...
  (IntermediateField.liftAlgEquiv _).trans
    (IntermediateField.equivOfEq (by
      rw [IntermediateField.lift_adjoin_simple, coe_candidateGenusFieldRealBaseRoot,
        ← candidateGenusFieldBase_def]))

/-- The canonical equivalence between the two models of the quadratic base preserves their
values in the full prime-discriminant compositum. -/
@[simp] theorem coe_candidateGenusFieldRealBaseEquiv_apply (hd : Squarefree d) (hpos : 0 < d)
...
@[simp] theorem coe_candidateGenusFieldRealBaseEquiv_symm_apply (hd : Squarefree d)
    (hpos : 0 < d) (x : candidateGenusFieldBase hd) :
    ((↑((candidateGenusFieldRealBaseEquiv hd hpos).symm x) :
        candidateGenusFieldReal hd) : candidateGenusField hd) =
      (x : candidateGenusField hd) := by
  simpa using
    (coe_candidateGenusFieldRealBaseEquiv_apply hd hpos
      ((candidateGenusFieldRealBaseEquiv hd hpos).symm x)).symm

/-- **The real genus-field candidate is unramified over its quadratic base at every finite
place.** Unramifiedness of the full prime-discriminant compositum descends to its maximal totally
real subfield. The canonical equivalence `candidateGenusFieldRealBaseEquiv` then identifies the
base used in that tower with the subfield generated by the chosen root inside the real candidate.
-/
theorem isUnramifiedIn_candidateGenusFieldReal (hd : Squarefree d)
    (hnsq : ¬ IsSquare ((d : ℤ) : ℚ)) (hpos : 0 < d)
    (p : Ideal (𝓞 (adjoin ℚ {candidateGenusFieldRealBaseRoot hd hpos} :
      IntermediateField ℚ (candidateGenusFieldReal hd)))) :
    Algebra.IsUnramifiedIn (𝓞 (candidateGenusFieldReal hd)) p := by
  let F : IntermediateField ℚ (candidateGenusFieldReal hd) :=
    adjoin ℚ {candidateGenusFieldRealBaseRoot hd hpos}
  let e : F ≃ₐ[ℚ] candidateGenusFieldBase hd :=
    candidateGenusFieldRealBaseEquiv hd hpos
  let f : candidateGenusFieldBase hd →ₐ[ℚ] candidateGenusFieldReal hd :=
    IntermediateField.inclusion (candidateGenusFieldBase_le_candidateGenusFieldReal hd hpos)
  let _ : Algebra (candidateGenusFieldBase hd) (candidateGenusFieldReal hd) :=
    f.toRingHom.toAlgebra
  let _ : IsScalarTower (candidateGenusFieldBase hd) (candidateGenusFieldReal hd)
      (candidateGenusField hd) :=
    -- Both sides are the inclusion of the same nested subfields into the full compositum.
    IsScalarTower.of_algebraMap_eq' rfl
  intro q _ hq
  have hfull : Algebra.IsUnramifiedIn (𝓞 (candidateGenusField hd))
      (q.under (𝓞 (candidateGenusFieldBase hd))) :=
    isUnramifiedIn_candidateGenusField hd hnsq _
  have hbase : Algebra.IsUnramifiedAt (𝓞 (candidateGenusFieldBase hd)) q :=
    TauCeti.RamificationInertia.isUnramifiedAt_of_isUnramifiedIn hfull q
  let eᵢ : 𝓞 (candidateGenusFieldBase hd) ≃+* 𝓞 F :=
    RingOfIntegers.mapRingEquiv e.symm.toRingEquiv
  let _ : Algebra (𝓞 (candidateGenusFieldBase hd)) (𝓞 F) :=
    eᵢ.toRingHom.toAlgebra
  let _ : IsScalarTower (𝓞 (candidateGenusFieldBase hd)) (𝓞 F)
      (𝓞 (candidateGenusFieldReal hd)) :=
    IsScalarTower.of_algebraMap_eq' (by
      apply RingHom.ext
      intro x
      apply RingOfIntegers.ext
      apply Subtype.ext
      exact coe_candidateGenusFieldRealBaseEquiv_symm_apply hd hpos (x : _))
  have : Algebra.IsUnramifiedAt (𝓞 (candidateGenusFieldBase hd)) q := hbase
  exact Algebra.FormallyUnramified.of_restrictScalars
    (𝓞 (candidateGenusFieldBase hd)) (𝓞 F) (Localization.AtPrime q)
