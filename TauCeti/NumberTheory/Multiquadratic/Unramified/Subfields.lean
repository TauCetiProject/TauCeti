...
* `mem_ramifiedPrimes_adjoin_singleton_iff`: the same criterion, also covering the square
  radicand `1` and its trivial quadratic factor.
...
omit [IsGalois ℚ M] in
/-- **The quadratic ramification criterion, including the trivial factor.** For squarefree `a`
...
`ℚ(x) = ℚ` is unramified. -/
theorem mem_ramifiedPrimes_adjoin_singleton_iff {a : ℤ} (hsf : Squarefree a) {x : M}
