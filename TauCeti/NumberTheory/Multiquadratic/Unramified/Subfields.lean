...
* `mem_ramifiedPrimes_adjoin_iff_dvd_fundamentalDiscriminant`: the ramified primes of the quadratic
  subfield generated by a square root of a squarefree integer, including the square radicand `1`.
...
theorem mem_ramifiedPrimes_adjoin_iff_dvd_fundamentalDiscriminant {a : ℤ} (hsf : Squarefree a)
    {x : M} (hx : x ^ 2 = algebraMap ℤ M a) {p : ℕ} (hp : p.Prime) :
  ...
  · let _ : NumberField (adjoin ℚ {x} : IntermediateField ℚ M) :=
      NumberField.of_intermediateField _
    have hd : NumberField.discr (adjoin ℚ {x} : IntermediateField ℚ M) =
