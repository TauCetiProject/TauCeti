...
theorem aut_mul_self_eq_one (hroot : ∀ i, root i ^ 2 = algebraMap K L (d i))
  ...
  ext y
  have hAlg :
      ((σ * σ : IntermediateField.adjoin K (Set.range root) ≃ₐ[K]
          IntermediateField.adjoin K (Set.range root)) :
        IntermediateField.adjoin K (Set.range root) →ₐ[K]
          IntermediateField.adjoin K (Set.range root)) =
        ((1 : IntermediateField.adjoin K (Set.range root) ≃ₐ[K]
          IntermediateField.adjoin K (Set.range root)) :
        IntermediateField.adjoin K (Set.range root) →ₐ[K]
    ...
    rintro x ⟨i, rfl⟩
    -- The extensionality goal `↑(σ * σ) ⟨root i, _⟩ = ⟨root i, _⟩` is
    -- definitionally the generator equation below: `gen root i := ⟨root i, _⟩`,
    -- and `↑(σ * σ) ·` reduces to `σ (σ ·)`. We convert to that readable form
    -- so the `aut_gen_eq_self_or_eq_neg` case split can fire on `gen root i`.
    change σ (σ (gen root i)) = gen root i
    rcases aut_gen_eq_self_or_eq_neg hroot σ i with h | h <;> simp [h, map_neg]
  exact congr_arg Subtype.val (AlgHom.congr_fun hAlg y)
