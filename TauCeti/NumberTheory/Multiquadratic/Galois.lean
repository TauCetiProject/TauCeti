...
theorem aut_mul_self_eq_one (hroot : ∀ i, root i ^ 2 = algebraMap K L (d i))
  ...
  apply AlgEquiv.ext
  intro y
  have hhom : (σ * σ).toAlgHom = (1 : IntermediateField.adjoin K (Set.range root) ≃ₐ[K]
      IntermediateField.adjoin K (Set.range root)).toAlgHom := by
    refine IntermediateField.algHom_ext_of_eq_adjoin (F := K)
      (S := IntermediateField.adjoin K (Set.range root)) (s := Set.range root) rfl ?_
    rintro x ⟨i, rfl⟩
    -- The extensionality goal is definitionally the generator equation below:
    -- `gen root i := ⟨root i, _⟩`, and `↑(σ * σ) ·` reduces to `σ (σ ·)`.
    -- We convert to that readable form so the case split can fire on `gen root i`.
    change σ (σ (gen root i)) = gen root i
    rcases aut_gen_eq_self_or_eq_neg hroot σ i with h | h <;> simp [h, map_neg]
  exact congr_arg (fun f : IntermediateField.adjoin K (Set.range root) →ₐ[K]
    IntermediateField.adjoin K (Set.range root) => f y) hhom
