...
* `HeckeRing.GL2.heckeTDiag_p_ppow_eq`: `T(p, pᵏ) = T(p,p) · T(1, p^(k−1))`.
* `HeckeRing.GL2.heckeT_ppow_recurrence`: the prime-power recurrence.
* `HeckeRing.GL2.heckeT_ppow_mul`: the product formula
...
divisor pair. -/
lemma heckeTDiag_p_ppow_eq (k : ℕ) (hk : 0 < k) :
    heckeTDiag p (p ^ k) = heckeTScalar p * heckeTDiag 1 (p ^ (k - 1)) := by
  have h0 := heckeTScalar_mul_heckeTDiag_ppow p hp 0 (k - 1) (Nat.zero_le _)
  rw [show k - 1 + 1 = k from Nat.succ_pred_eq_of_pos hk] at h0
  simpa [pow_zero, zero_add, pow_one] using h0.symm

include hp in
/-- `T(p⁰) = 1`. -/
private lemma heckeT_ppow_zero : heckeT ⟨p ^ 0, pow_pos hp.pos 0⟩ = 1 := by
  rw [show (⟨p ^ 0, pow_pos hp.pos 0⟩ : ℕ+) = 1 from Subtype.ext (pow_zero p)]
  exact heckeT_one

omit hp in
/-- `T(1, p⁰) = 1`. -/
private lemma heckeTDiag_one_ppow_zero : heckeTDiag 1 (p ^ 0) = 1 := by
  rw [pow_zero]
  exact heckeTDiag_one_one

omit hp in
/-- `T(1, p¹) = T(1, p)`. -/
private lemma heckeTDiag_one_ppow_one : heckeTDiag 1 (p ^ 1) = heckeTDiag 1 p := by
  rw [pow_one]

include hp in
/-- `T(p¹) = T(p)`: normalizing the exponent in the index. -/
private lemma heckeT_ppow_one : heckeT ⟨p ^ 1, pow_pos hp.pos 1⟩ = heckeT ⟨p, hp.pos⟩ := by
  congr 1
...
private lemma heckeT_ppow_recurrence_step (k : ℕ) (hk_pos : 0 < k)
    (ih : ∀ j : ℕ, j < k + 2 → 0 < j →
  ...
  rw [heckeTDiag_p_ppow_eq p hp (k + 2) (by omega)] at h5
  have h2 := heckeTDiag_one_ppow_eq p hp (k + 2 + 1) (by omega)
  conv at h2 => rhs; rw [show (k + 2 + 1) - 2 = k + 1 by omega]
  rw [h2] at h5
  simp only [show k + 2 ≠ 1 by omega, ite_false, show k + 2 - 1 = k + 1 by omega] at h5
  rw [heckeTDiag_one_ppow_eq p hp (k + 2) (by omega)] at h5
  ...
  have hdist₁ : heckeT ⟨p, hp.pos⟩ *
    ...
    mul_sub _ _ _
  rw [hdist₁] at h5
  have h2k1 := heckeTDiag_one_ppow_eq p hp (k + 1) (by omega)
...
theorem heckeT_ppow_recurrence : ∀ k : ℕ, 0 < k →
    heckeT ⟨p ^ (k + 1), pow_pos hp.pos (k + 1)⟩ =
  ...
  rw [heckeTDiag_p_ppow_eq p hp k hk] at h5
  have h2 := heckeTDiag_one_ppow_eq p hp (k + 1) (by omega)
  ...
  · simp only [show (1 : ℕ) - 1 = 0 from rfl, ite_true] at h5 ⊢
    rw [heckeT_ppow_zero p hp, heckeTDiag_one_ppow_zero, mul_one] at h5
    rw [heckeT_ppow_zero p hp, mul_one, heckeT_ppow_one p hp]
    rw [heckeTDiag_one_ppow_one, heckeT_prime p hp] at h5
    rw [heckeT_prime p hp]
  ...
  · simp only [show (2 : ℕ) ≠ 1 by omega, ite_false, show (2 : ℕ) - 1 = 1 by omega] at h5 ⊢
    rw [heckeTDiag_one_ppow_eq p hp 2 (by omega)] at h5
    have hdist : heckeT ⟨p, hp.pos⟩ *
    ...
    rw [heckeT_ppow_zero p hp, mul_one, heckeTDiag_one_ppow_one, heckeT_prime p hp] at h5
    rw [heckeT_ppow_one p hp] at h5 ⊢
    ...
    linear_combination (norm := module) -h5
  · exact heckeT_ppow_recurrence_step p hp (k + 1) (by omega) ih
...
private lemma heckeTScalar_pow_comm_heckeT_ppow (i k : ℕ) :
...
private lemma heckeT_prime_comm_heckeTScalar_pow (i : ℕ) :
  ...
  rw [← heckeT_ppow_one p hp]
  exact (heckeTScalar_pow_comm_heckeT_ppow p hp i 1).symm
...
private lemma heckeT_ppow_mul_summand_split (r s i : ℕ) (hi : i ≤ r) (hrs : r ≤ s) :
  ...
  have h_rec := heckeT_ppow_recurrence p hp (r + 1 + s - 2 * i) (by omega)
...
private lemma heckeT_ppow_mul_last_two_terms (r s : ℕ) (hrs : r + 2 ≤ s) :
  ...
  have h_rec := heckeT_ppow_recurrence p hp (s - r - 1) (by omega)
...
`s`, it follows at `r + 2`. -/
private lemma heckeT_ppow_mul_step (r s : ℕ) (hrs : r + 2 ≤ s)
  ...
  have h_rec := heckeT_ppow_recurrence p hp (r + 1) (by omega)
  ...
  have h_sum_split : ∑ i ∈ Finset.range (r + 1), (p : ℤ) ^ i • (Tpp ^ i *
    ...
    exact heckeT_ppow_mul_summand_split p hp r s i (by omega) (by omega)
  ...
  congr 1
  exact heckeT_ppow_mul_last_two_terms p hp r s hrs

include hp in
/-- **Shimura, Theorem 3.24(4)** — the prime-power product formula:
`T(pʳ) · T(pˢ) = ∑_{i ≤ r} pⁱ · T(p,p)ⁱ · T(p^(r+s−2i))` for `r ≤ s`. -/
theorem heckeT_ppow_mul : ∀ r s : ℕ, r ≤ s →
  ...
  intro s hrs
  rcases r with _ | _ | r
  · rw [Finset.sum_range_one, heckeT_ppow_zero p hp]
    simp
  · rw [Finset.sum_range_succ, Finset.sum_range_one]
    ...
    rw [heckeT_ppow_one p hp]
    exact (eq_sub_iff_add_eq.mp (heckeT_ppow_recurrence p hp s (by omega))).symm
  · exact heckeT_ppow_mul_step p hp r s (by omega) (ih (r + 1) (by omega) s (by omega))
