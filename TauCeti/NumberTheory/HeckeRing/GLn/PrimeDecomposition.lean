...
the `p`-power operators generate the `p`-local Hecke subsemiring `R_p` (Shimura's `R_p`).
...
* `HeckeRing.GLn.pLocalSubring`: the `p`-local Hecke subsemiring `R_p`.
...
theorem diagElem_split_prime (a : Fin n → ℕ) (ha_pos : ∀ i, 0 < a i) (p : ℕ)
  ...
  exact (diagElem_mul_of_coprime n _ _ (ppowDiag_pos n p hp _) (removePrime_pos n p a ha_pos)
    (coprime_prod_ppowDiag_removePrime n p hp a ha_pos)).symm

/-- The `p`-local Hecke subsemiring `R_p`: generated by the diagonal Hecke operators with
