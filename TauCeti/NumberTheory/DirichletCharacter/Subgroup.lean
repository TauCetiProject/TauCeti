...
conductors of its members. It is therefore the least positive level through which every character
in the group factors.
...
noncomputable def dirichletConductor (Y : Subgroup (DirichletCharacter R n)) : ℕ := by
...
theorem conductor_dvd_dirichletConductor (Y : Subgroup (DirichletCharacter R n))
    {χ : DirichletCharacter R n} (hχ : χ ∈ Y) : conductor χ ∣ Y.dirichletConductor := by
...
dividing `m`. -/
theorem dirichletConductor_dvd_iff (Y : Subgroup (DirichletCharacter R n)) {m : ℕ} :
    Y.dirichletConductor ∣ m ↔ ∀ χ ∈ Y, conductor χ ∣ m := by
  let _ : Fintype Y := Fintype.ofFinite Y
  rw [dirichletConductor]
  constructor
  · intro h χ hχ
    exact (Finset.dvd_lcm (f := fun ψ : Y ↦ conductor ψ.1)
      (Finset.mem_univ ⟨χ, hχ⟩)).trans h
  · intro h
    exact Finset.lcm_dvd fun χ _ ↦ h χ.1 χ.2

/-- The conductor of a subgroup of level-`n` characters divides `n`. -/
theorem dirichletConductor_dvd_level [NeZero n]
    (Y : Subgroup (DirichletCharacter R n)) : Y.dirichletConductor ∣ n :=
...
theorem dirichletConductor_ne_zero [NeZero n]
    (Y : Subgroup (DirichletCharacter R n)) : Y.dirichletConductor ≠ 0 := by
  intro h
...
theorem dirichletConductor_dvd_iff_forall_factorsThrough [NeZero n]
    (Y : Subgroup (DirichletCharacter R n)) {m : ℕ} (hmn : m ∣ n) :
...
theorem dirichletConductor_dvd_of_le {Y Z : Subgroup (DirichletCharacter R n)} (hYZ : Y ≤ Z) :
...
@[simp]
theorem dirichletConductor_closure_singleton [NeZero n] (χ : DirichletCharacter R n) :
...
variable {R R' : Type*} [CommRing R] [CommRing R'] {n : ℕ}
  [Finite (DirichletCharacter R n)] [Finite (DirichletCharacter R' n)] [NeZero n]
