...
theorem mem_unitFiltration_iff_valuation_le {π : 𝒪[K]} (hπ : Irreducible π) (i : ℕ) {x : Kˣ} :
  ...
  have key : ∀ y : 𝒪[K], y ∈ 𝓂[K] ^ i ↔ valuation K (y : K) ≤ valuation K (π : K) ^ i :=
    Set.ext_iff.mp (hπ.maximalIdeal_pow_eq_setOfPred_le_v_coe_pow (v := valuation K) i)
...
/-- The depth-zero step of the filtration is the unit group of `𝒪[K]`. This is deliberately not
a `simp` lemma: `mem_unitFiltration_zero` below is the `simp` normal form for depth zero, and it
states the criterion in the canonical `valuation K` spelling rather than the valuation of
`(valuation K).valuationSubring` that rewriting by this equation would produce. -/
...
/-- Membership of `U(K, 0)`: a unit of `K` lies in the depth-zero step exactly when it has
valuation `1`, that is, when it is a unit of `𝒪[K]`. -/
@[simp]
...
theorem unitFiltration_one :
  ...
  · have h' : valuation K ((x : K) - 1) < 1 :=
    ...
    rw [← SetLike.mem_coe, hπ.maximalIdeal_pow_eq_setOfPred_le_v_coe_pow (v := valuation K)] at hy
    simpa using hy
...
theorem iInf_unitFiltration : ⨅ i : ℕ, unitFiltration K i = ⊥ := by
  ...
  have hzero : valuation K ((x : K) - 1) = 0 := by
    by_contra h
    obtain ⟨i, hi⟩ := exists_pow_lt₀ (Valuation.integer.v_irreducible_lt_one hπ) (Units.mk0 _ h)
...
theorem hasBasis_nhds_one_unitFiltration :
  ...
  · rintro γ -
    obtain ⟨i, hi⟩ := exists_pow_lt₀ hπ1 γ
