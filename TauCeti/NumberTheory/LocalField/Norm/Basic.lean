...
* `TauCeti.map_span_singleton_norm_eq_span_singleton`: the norm image of a principal ideal is the
  principal ideal generated by the norm of its generator.
...
omit [TopologicalSpace K] [IsNonarchimedeanLocalField K] [TopologicalSpace L]
  [IsNonarchimedeanLocalField L] [FiniteDimensional K L] in
variable (K L) in
...
theorem map_span_singleton_norm_eq_span_singleton (x : 𝒪[L]) :
  ...
  have key : ((Algebra.norm 𝒪[K]) '' (↑(Ideal.span {x} : Ideal 𝒪[L]) : Set 𝒪[L]))
    ...
    obtain ⟨a, ha⟩ : ∃ a : 𝒪[L], a * x = c :=
      Ideal.mem_span_singleton'.mp hc
    subst ha
    rw [map_mul]
