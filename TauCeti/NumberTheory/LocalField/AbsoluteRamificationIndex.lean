...
`K`. -/
@[simp]
theorem absoluteRamificationIndex_eq_natCastValuation (hp : (p : K) ≠ 0) :
    absoluteRamificationIndex K p = natCastValuation K p hp := by
...
theorem absoluteRamificationIndex_padic : absoluteRamificationIndex ℚ_[p] p = 1 := by
  rw [absoluteRamificationIndex_eq_natCastValuation
    (hp := Nat.cast_ne_zero.mpr (Fact.out : p.Prime).ne_zero), Padic.natCastValuation_self]
