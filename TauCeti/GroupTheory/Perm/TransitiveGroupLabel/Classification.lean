...
* `TauCeti.referenceSubgroup_five_two_eq_normalizer`: the reference subgroup of `5T3` is the
  normalizer of that of `5T1`.
...
theorem exists_sylow_eq_referenceSubgroup_five_zero :
  ...
  have hCi : (referenceSubgroup 5 ⟨0, by simp⟩).index = 24 := by
    ...
    rw [hC, natCard_perm_fin_five] at h
    omega
...
theorem referenceSubgroup_five_two_eq_normalizer :
  ...
  have hle : referenceSubgroup 5 ⟨2, by simp⟩ ≤
    ...
    · rw [SetLike.mem_coe, Subgroup.mem_normalizer_iff_map_conj_eq,
      ...
      have hconj : ((MulAut.conj ([0, 1, 3, 2].formPerm : Perm (Fin 5)) :
          Perm (Fin 5) →* Perm (Fin 5))) (finRotate 5) = finRotate 5 ^ 2 := by decide
      rw [hconj]
      refine Subgroup.eq_of_le_of_card_ge
        (Subgroup.zpowers_le.2 (pow_mem (Subgroup.mem_zpowers _) _)) ?_
      rw [Nat.card_zpowers, Nat.card_zpowers, orderOf_eq_prime (p := 5) (by decide) (by decide),
        orderOf_eq_prime (p := 5) (by decide) (by decide)]
  refine Subgroup.eq_of_le_of_card_ge hle ?_
...
theorem natCard_referenceSubgroup_five_two :
  ...
  obtain ⟨P, hP⟩ := exists_sylow_eq_referenceSubgroup_five_zero
  rw [referenceSubgroup_five_two_eq_normalizer, ← hP]
...
theorem referenceSubgroup_five_one_le_referenceSubgroup_five_two :
  ...
  have hsq : ([0, 1, 3, 2].formPerm : Perm (Fin 5)) ^ 2 = swap 0 3 * swap 1 2 := by decide
...
theorem natCard_referenceSubgroup_five_one :
  ...
  have h5 : 5 ∣ Nat.card (referenceSubgroup 5 ⟨1, by simp⟩) := by
    have h := Subgroup.card_dvd_of_le (H := referenceSubgroup 5 ⟨0, by simp⟩)
      (K := referenceSubgroup 5 ⟨1, by simp⟩) (by
...
theorem natCard_referenceSubgroup_five_four :
  ...
  rw [referenceSubgroup_five_four, Subgroup.card_top, natCard_perm_fin_five]
...
`5T1` and `5T3` carries a label: it is one of `5T1`, `5T2` and `5T3`. -/
private theorem exists_transitiveGroupLabel_five_of_le_of_le {H : Subgroup (Perm (Fin 5))}
  ...
  · -- `H` has index two in `5T3`, so it contains the square `swap 0 3 * swap 1 2` of the
    ...
    have hindex : (H.subgroupOf (referenceSubgroup 5 ⟨2, by simp⟩)).index = 2 := by
      ...
      rw [natCard_referenceSubgroup_five_two, natCard_perm_fin_five] at hFi
      rw [hH, natCard_perm_fin_five] at hHi
    ...
    have hsq : swap (0 : Fin 5) 3 * swap 1 2 ∈ H := by
      ...
      have hsq : ([0, 1, 3, 2].formPerm : Perm (Fin 5)) ^ 2 = swap 0 3 * swap 1 2 := by decide
      rwa [Subgroup.mem_subgroupOf, Subgroup.coe_pow, Subgroup.coe_mk, hsq] at h
...
label: by Sylow's theorem it has a unique Sylow `5`-subgroup, and a conjugation carrying that
onto the reference subgroup of `5T1` carries the subgroup into its normalizer, the reference
subgroup of `5T3`. -/
private theorem exists_transitiveGroupLabel_five_of_natCard_dvd_twenty
  ...
  rcases card_sylow_five_eq_one_or_six (α := Fin 5) (by simp) G with h1 | h6
  · obtain ⟨P, hPG, hGN⟩ := exists_sylow_le_le_normalizer_of_card_sylow_five_eq_one
      (α := Fin 5) (by simp) G h5 h1
    ...
    obtain ⟨j, hj⟩ := exists_transitiveGroupLabel_five_of_le_of_le
      (H := G.map (MulAut.conj τ).toMonoidHom) (hPQ ▸ Subgroup.map_mono hPG) (by
        rw [referenceSubgroup_five_two_eq_normalizer, ← hPQ,
          ← Subgroup.map_equiv_normalizer_eq]
        exact Subgroup.map_mono hGN)
    exact ⟨j, (transitiveGroupLabel_map_conj_iff G τ).1 hj⟩
  · -- Six Sylow `5`-subgroups would force `30` to divide the order.
    have h30 := Sylow.mul_card_sylow_dvd_card (p := 5) h5
    rw [h6] at h30
    have := Nat.le_of_dvd (Nat.card_pos (α := G)) h30
...
theorem exists_transitiveGroupLabel_five (G : Subgroup (Perm (Fin 5)))
  ...
  rw [natCard_perm_fin_five] at hmul
