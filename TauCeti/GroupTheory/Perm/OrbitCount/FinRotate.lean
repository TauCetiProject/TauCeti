...
where the rotation is the identity. This count includes fixed points. The formula is useful when
a traversal permutation is identified, up to conjugacy, with cyclic rotation: it reduces the
