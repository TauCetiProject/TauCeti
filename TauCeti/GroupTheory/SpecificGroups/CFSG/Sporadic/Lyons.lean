...
maximum number of cosets defined equal to the index, and a separate enumeration of the z-free
relators alone closes at index `3906` over the subgroup generated by `a`, `b`, `c`, again with
maximum cosets equal to the index. This is provenance rather than a
...
* `TauCeti.Sporadic.Lyons.relatorsOne` to `TauCeti.Sporadic.Lyons.relatorsFour`: the four
  blocks of relators, and `TauCeti.Sporadic.Lyons.relatorList` their concatenation.
...
theorem length_relatorsOne : relatorsOne.length = 5 := rfl

/-- The second block has eighteen relators. -/
theorem length_relatorsTwo : relatorsTwo.length = 18 := rfl

/-- The third block has sixteen relators. -/
theorem length_relatorsThree : relatorsThree.length = 16 := rfl

/-- The fourth block has fourteen relators. -/
theorem length_relatorsFour : relatorsFour.length = 14 := rfl

/-- The row has fifty-three relators. -/
theorem length_relatorList : relatorList.length = 53 := rfl
