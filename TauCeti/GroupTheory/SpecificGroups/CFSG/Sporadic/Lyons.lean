...
## Source-to-Lean read-through and corrections

The relators were read against a 600 dpi rendering of the journal PDF of the article, whose
SHA-256 digest is `f3d02899c47345df12cc093a7f099ab582f11616bd8ef3a51b6cd504d23de803`, pages
335--336, where the overbars that denote inverses are unambiguous. The paper writes `x̄` for
`x⁻¹`, `x^y` for `y⁻¹xy`, which is `TauCeti.Relator.conj`, and `[x,y]` for `x⁻¹y⁻¹xy`,
transcribed as `TauCeti.Relator.comm (.inv x) (.inv y)`; a displayed equation `r = s` is the
relator `r s⁻¹`, which is `TauCeti.Relator.div`.

An earlier transcription of this row dropped or displaced overbars in four relators, and the
presented group collapsed: the sixteen relators of `R_H2 ∪ R_H1` forced `d` into the subgroup
generated by `a`, `b`, `c`, and the whole presented group was trivial. Against the print, and in
one relator against the group itself:

* `R_H1`, fifth relator: the right-hand side is `a⁻¹ b a⁻¹ b⁻¹ a⁻¹ b⁻² c a⁻¹ b⁻¹ c⁻¹ b a`; the
  earlier transcription had `a` for the third `a⁻¹` and `c` for `c⁻¹`;
* `R_G`, third relator: the right-hand side is
  `c⁻¹ b c² a c⁻¹ b d⁻¹ c a c⁻¹ b c⁻¹ b⁻² c a⁻¹ c d⁻¹ c b² c d a d⁻¹ c⁻¹ d⁻¹ c b a⁻¹ b`; the earlier
  transcription had `c` for the `c⁻¹` before `b⁻²`, `a` for the `a⁻¹` after it, and `b⁻²` for
  `b²`;
* `R_G`, fourth relator: the second entry of the commutator is
  `d⁻¹ c⁻¹ b a b⁻¹ d c⁻¹ d c⁻¹ d c⁻¹ b⁻¹ a b⁻¹ c d⁻¹ c d`. Here the print itself is wrong: it has
  `d c d` for `d c⁻¹ d` and `b a` for `b⁻¹ a`, and the earlier transcription further had `c d c d`
  for the final `c d⁻¹ c d`. The word used here is the one that holds in `Ly`, as described below;
* `R_G`, fifth relator: the right-hand side is
  `a⁻¹ d⁻¹ c⁻¹ b c⁻² a b⁻¹ c a⁻¹ b² c b⁻¹ c a⁻¹ c⁻¹ d b⁻¹ a⁻¹`; the earlier transcription had
  `a b⁻²` for `a⁻¹ b²` and `a` for the `a⁻¹` before `c⁻¹ d`.

The other twenty-one relators agree with the print letter for letter. The reduced block lengths
are unchanged by these corrections and remain the published `80`, `160`, and `309`, total `549`.

The corrected row was checked against a permutation model of `Ly` built from an independent
presentation on the same generators: Sims' presentation as amended by Havas and Sims, *A
presentation for the Lyons simple group*, Progress in Mathematics 173 (1999), 241--249, whose
enumeration over the subgroup generated by `a`, `b`, `c`, `d` with ACE gives index `8835156`. On
that coset table all twenty-five relators of this row evaluate to the identity, while relators
`14`, `19`, `20`, and `21` of the earlier transcription do not, and the fourth `R_G` relator as
printed does not; among all words within two letter edits of the printed one, only the word above
holds. The nine `R_H2` relators present a group of order `1500000`, and the sixteen relators of
`R_H2 ∪ R_H1` present a group in which the subgroup generated by `a`, `b`, `c` has index `3906`
with coset image simple of order `5859000000`, as expected for `G₂(5)`. Consequently the group
presented by this row maps onto `Ly`; that the map is an isomorphism is the paper's double-coset
enumeration, not reproduced here. None of this is a Lean theorem, and this file asserts no order,
finiteness, simplicity, or identification result.
...
private def lyH1Relators {α : Type*} (a b c d : Relator α) : List (Relator α) :=
  ...
  [ Relator.div (Relator.conj (a ⬝ bInv ⬝ a) d) (a ⬝ bInv ⬝ .pow a 5),
    ...
    Relator.div
      ...
      (aInv ⬝ b ⬝ aInv ⬝ bInv ⬝ aInv ⬝ .pow bInv 2 ⬝ c ⬝ aInv ⬝ bInv ⬝ cInv ⬝
        b ⬝ a),
    Relator.div
  ...
  [ Relator.div (Relator.conj a z) (.pow aInv 3),
    ...
    Relator.div
      (Relator.conj
        ...
        (z ⬝ d ⬝ z))
      (cInv ⬝ b ⬝ .pow c 2 ⬝ a ⬝ cInv ⬝ b ⬝ dInv ⬝ c ⬝ a ⬝ cInv ⬝ b ⬝ cInv ⬝
        .pow bInv 2 ⬝ c ⬝ aInv ⬝ c ⬝ dInv ⬝ c ⬝ .pow b 2 ⬝ c ⬝ d ⬝ a ⬝
        dInv ⬝ cInv ⬝ dInv ⬝ c ⬝ b ⬝ aInv ⬝ b),
    Relator.div
      (sourceComm z
        (dInv ⬝ cInv ⬝ b ⬝ a ⬝ bInv ⬝ d ⬝ cInv ⬝ d ⬝ cInv ⬝ d ⬝ cInv ⬝ bInv ⬝
          a ⬝ bInv ⬝ c ⬝ dInv ⬝ c ⬝ d))
      (b ⬝ c ⬝ bInv ⬝ cInv),
    Relator.div (Relator.conj a (z ⬝ d ⬝ bInv ⬝ z))
      (aInv ⬝ dInv ⬝ cInv ⬝ b ⬝ .pow cInv 2 ⬝ a ⬝ bInv ⬝ c ⬝ aInv ⬝
        .pow b 2 ⬝ c ⬝ bInv ⬝ c ⬝ aInv ⬝ cInv ⬝ d ⬝ bInv ⬝ aInv),
    Relator.div
      (Relator.conj
...
theorem lyPresentation_reducedH1Lengths :
  ...
  · change (FreeGroup.reduce (Relator.toWord
      ((Relator.conj (b.pow 2 ⬝ a ⬝ c ⬝ b ⬝ a) (d ⬝ c ⬝ a.inv ⬝ b ⬝ c ⬝ d)).div
        (a.inv ⬝ b ⬝ a.inv ⬝ b.inv ⬝ a.inv ⬝ b.inv.pow 2 ⬝ c ⬝ a.inv ⬝ b.inv ⬝ c.inv ⬝ b ⬝
          a)))).length = 31
    simp [Relator.toWord_conj, Relator.toWord_div, Relator.toWord_pow,
      FreeGroup.reduce, FreeGroup.invRev]
  constructor
...
theorem lyPresentation_reducedExtensionLengths :
  ...
  · change (FreeGroup.reduce (Relator.toWord
      ((Relator.conj
        ...
        (z ⬝ d ⬝ z)).div
        (cInv ⬝ b ⬝ c.pow 2 ⬝ a ⬝ cInv ⬝ b ⬝ dInv ⬝ c ⬝ a ⬝ cInv ⬝ b ⬝ cInv ⬝
          bInv.pow 2 ⬝ c ⬝ aInv ⬝ c ⬝ dInv ⬝ c ⬝ b.pow 2 ⬝ c ⬝ d ⬝ a ⬝
  ...
  · change (FreeGroup.reduce (Relator.toWord
      ((Relator.conj a (z ⬝ d ⬝ bInv ⬝ z)).div
        (aInv ⬝ dInv ⬝ cInv ⬝ b ⬝ cInv.pow 2 ⬝ a ⬝ bInv ⬝ c ⬝ aInv ⬝
          b.pow 2 ⬝ c ⬝ bInv ⬝ c ⬝ aInv ⬝ cInv ⬝ d ⬝ bInv ⬝ aInv)))).length = 29
    simp [aInv, bInv, cInv, dInv, Relator.toWord_conj, Relator.toWord_div,
      FreeGroup.reduce, FreeGroup.invRev]
  constructor
  · change (FreeGroup.reduce (Relator.toWord
