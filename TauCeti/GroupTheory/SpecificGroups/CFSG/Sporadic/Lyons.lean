...
generated by `a`, `b`, `c`, and the whole presented group was trivial. Against the print:
...
* `R_G`, third relator: the right-hand side is
  ...
  `b²`;
* `R_G`, fourth relator: the commutator's second entry ends `b a b⁻¹ c d⁻¹ c d`; the earlier
  transcription had `d` for that `d⁻¹`;
...
Coset enumeration outside Lean supports the corrected data at the first two stages: the nine
`R_H2` relators present a group of order `1500000` whose only perfect quotient is trivial, and the
sixteen relators of `R_H2 ∪ R_H1` present a group in which the subgroup generated by `a`, `b`, `c`
has index `3906`, the coset image being simple of order `5859000000`, as expected for `G₂(5)`.
The paper's enumeration of the `8835156` cosets of `G₂(5)` in `Ly` used a double-coset method,
and has not been reproduced here. None of this is a Lean theorem, and this file asserts no order,
