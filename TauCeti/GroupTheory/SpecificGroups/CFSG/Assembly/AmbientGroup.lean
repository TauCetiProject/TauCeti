...
Frobenius; the uniform Steinberg endomorphism is not assembled here.
...
* `TauCeti.ValidLieTypeIndex.frobenius_simpleRootSubgroup`: the Frobenius raises the parameter of
  every simple root subgroup to the `q`-th power, uniformly in the seventeen constructors.
* `TauCeti.ValidLieTypeIndex.simpleRootSubgroup_A`, ...,
  `TauCeti.ValidLieTypeIndex.simpleRootSubgroup_tits` and `TauCeti.ValidLieTypeIndex.frobenius_A`,
  ..., `TauCeti.ValidLieTypeIndex.frobenius_tits`: on each constructor the simple root subgroups
  and the Frobenius are those of the graph-twisted assembly or of the half-Frobenius family.
