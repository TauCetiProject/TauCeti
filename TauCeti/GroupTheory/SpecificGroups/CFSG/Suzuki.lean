...
The group below is named but not identified. Nothing is proved finite, perfect or simple, and
Mathlib's separate `suzukiGroup` is not mentioned, so no comparison with it is claimed. The fixed
points of an odd half-Frobenius power are not the `ℱ_q` points of the carrier, which is why this
family is not an instance of the Frobenius machinery the untwisted ones use, and no description of
them by matrix entries is offered.
...
This is the classification's recipe run on the rank-two type-`C` carrier. Because the Steinberg map
is an odd power of the half-Frobenius rather than a Frobenius, its fixed points are a proper
subgroup of the `ℱ_q` points of the carrier, and the quotient is `²B₂(2 ^ (2m+1))` rather than a
