...
last: every integer power of an involution is itself or the identity, so that subgroup is
`{1, sr i}`, and it is nontrivial, and proper except in `DihedralGroup 1`.
...
* `TauCeti.mem_zpowers_sr_iff`: the subgroup generated by a reflection is the two-element
  subgroup `{1, sr i}`, with `TauCeti.zpowers_sr_ne_bot` and `TauCeti.zpowers_sr_ne_top` reading
  off that it is nontrivial, and proper except in `DihedralGroup 1`.
...
theorem sr_ne_one (i : ZMod n) : DihedralGroup.sr i ≠ 1 := fun h =>
  sr_notMem_dihedralRotations i (by rw [h]; exact one_mem _)

/-- **The subgroup generated by a reflection is `{1, sr i}`.** A reflection is an involution, so an
even power of it is the identity and an odd power is the reflection again. -/
theorem mem_zpowers_sr_iff {i : ZMod n} {g : DihedralGroup n} :
    g ∈ Subgroup.zpowers (DihedralGroup.sr i) ↔ g = 1 ∨ g = DihedralGroup.sr i := by
  have h2 : DihedralGroup.sr i ^ (2 : ℤ) = 1 := by
    rw [show (2 : ℤ) = ((2 : ℕ) : ℤ) from rfl, zpow_natCast, pow_two, DihedralGroup.sr_mul_self]
  constructor
  · intro hg
    obtain ⟨k, hk⟩ := Subgroup.mem_zpowers_iff.mp hg
    rcases Int.even_or_odd k with ⟨m, rfl⟩ | ⟨m, rfl⟩
    · exact Or.inl (by rw [← hk, show (m + m : ℤ) = 2 * m by ring, zpow_mul, h2, one_zpow])
    · exact Or.inr (by rw [← hk, zpow_add, zpow_mul, h2, one_zpow, one_mul, zpow_one])
  · rintro (rfl | rfl)
    · exact one_mem _
    · exact Subgroup.mem_zpowers _

/-- The subgroup generated by a reflection is nontrivial. -/
theorem zpowers_sr_ne_bot (i : ZMod n) :
...
theorem zpowers_sr_ne_top (hn : n ≠ 1) (i : ZMod n) :
    Subgroup.zpowers (DihedralGroup.sr i) ≠ ⊤ := by
  intro htop
...
theorem eq_one_of_mem_dihedralRotations_of_mem_zpowers_sr {i : ZMod n} {g : DihedralGroup n}
  ...
  rcases mem_zpowers_sr_iff.mp hgs with h | h
  · exact h
