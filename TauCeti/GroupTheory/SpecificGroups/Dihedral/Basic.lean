...
/-- **The subgroup generated by a reflection is `{1, sr i}`.** A reflection has order `2`, so an
integer power of it is determined by the exponent modulo `2`, and only the residues `0` and `1`
occur. -/
@[simp]
theorem mem_zpowers_sr_iff {i : ZMod n} {g : DihedralGroup n} :
  ...
  · obtain ⟨k, rfl⟩ := Subgroup.mem_zpowers_iff.mp hg
    rw [← zpow_mod_orderOf (DihedralGroup.sr i) k, DihedralGroup.orderOf_sr, Nat.cast_ofNat]
