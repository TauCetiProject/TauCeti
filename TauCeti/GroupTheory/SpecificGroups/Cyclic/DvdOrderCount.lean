...
/-!
# How many elements of a cyclic group have order a multiple of `f`

Let `α` be a cyclic group of order `h` and let `f` divide `h`. The elements of `α` whose order is
a multiple of `f` are counted exactly:

`#{τ | f ∣ orderOf τ} = h * ∏ p ∣ f, (1 - p ^ -(v_p h - v_p f + 1))`,

the product running over the primes of `f`. Writing `τ = g ^ k` for a generator `g`, the order of
`τ` is `h / gcd h k`, and `f` divides it exactly when `p ^ (v_p h - v_p f + 1)` fails to divide
`k` for every prime `p` of `f`. Those conditions constrain `k` modulo coprime prime powers, so
they are independent and the count is a product.
...
`IsCyclic.card_filter_dvd_orderOf_eq_sum_totient` turns the same count into a sum of totients over
the divisors of `h` that `f` divides; the results here evaluate it in closed form instead.

## Main results

* `TauCeti.dvd_orderOf_pow_iff`: `f ∣ orderOf (g ^ k)` as non-divisibility of `k` by a prime power
  at each prime of `f`.
* `IsCyclic.card_filter_dvd_orderOf_mul_prod_primeFactors`: the count, in `ℕ`.
* `IsCyclic.card_filter_dvd_orderOf_eq_mul_prod_primeFactors`: the count, as the displayed product
  over `ℚ`.
* `IsCyclic.le_card_filter_dvd_orderOf`: the uniform lower bound `(1 - 2 ^ -r) ^ #f.primeFactors`
  for the proportion, when `f ^ r ∣ h`.
-/
...
powers. -/
theorem dvd_orderOf_pow_iff {G : Type*} [Monoid G] {g : G} (hg : IsOfFinOrder g) {f : ℕ}
    (hf : f ∣ orderOf g) (k : ℕ) :
    f ∣ orderOf (g ^ k) ↔
...
variable {α : Type*} [Group α] [Fintype α] [IsCyclic α] {f : ℕ}
...
private theorem card_filter_dvd_orderOf_eq_card_zmod [NeZero (Fintype.card α)]
  ...
  have hcrit : ∀ k : ℕ, f ∣ orderOf (g ^ k) ↔ ∀ p ∈ f.primeFactors,
    ...
    intro k
    have h := TauCeti.dvd_orderOf_pow_iff (isOfFinOrder_of_finite g) (hg ▸ hf) k
    rwa [hg] at h
