...
Attached to an element `x` and a prime `p` is the join of the cyclic group generated by the
`p`-free part `s` of `x` with a Sylow `p`-subgroup of the centraliser of `s`: it contains `x`, it
is `p`-elementary, and its index in that centraliser is prime to `p`. The cyclic subgroups are
already elementary, so it is this larger subgroup, not `⟨x⟩`, that carries content.
...
subgroup attached to an element: for `x : G` with `p`-free part `s`, the join of `⟨s⟩` with a Sylow
`p`-subgroup of the centraliser of `s`. It contains `x`, it is `p`-elementary, and its relative
index in that centraliser is prime to `p`, which is the arithmetic the induction theorem runs on.
-/
...
theorem isPElementary_sup_of_commute {C Q : Subgroup G} (hC : IsCyclic C)
  ...
  · have := hC
    exact isCyclic_of_surjective (subgroupOfEquivOfLe hCle).symm
...
with `P`. It is `p`-elementary by `TauCeti.isPElementary_pElementaryOfSylow`, and its relative
index in the centraliser is prime to `p` by
...
theorem not_dvd_relIndex_pElementaryOfSylow [Fact p.Prime] [Finite G] :
