...
These standard examples model the hypersurface `S[X]/(X^(i+j))` in the homotopy category
...
of `X^(i+j)`, obtained from `rankOne`. Taking `j = n - i` when `i ≤ n`
gives the standard factorization of `X^n`. -/
