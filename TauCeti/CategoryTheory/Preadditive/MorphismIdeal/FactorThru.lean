...
* `TauCeti.MorphismIdeal.FactorsThrough P f`: the morphism `f` factors through an object
  satisfying `P`.
* `TauCeti.MorphismIdeal.factorIdeal P`: the two-sided ideal additively generated by such maps.

## Main results

* `TauCeti.MorphismIdeal.factorIdeal_le_iff`: the universal property of the generated ideal.
* `TauCeti.MorphismIdeal.mem_factorIdeal_iff`: under the stated closure hypotheses, membership in
  the generated ideal is equivalent to a single factorization through a `P`-object.
...
/-- The witness description of factorization through a `P`-object. -/
...
theorem zero (P : ObjectProperty C) [P.Nonempty] (X Y : C) :
...
theorem factorIdeal_le_iff {I : MorphismIdeal C} :
  ...
  · intro h X Y Z hZ i p
    exact le_def.1 h (i ≫ p) (comp_mem_factorIdeal hZ i p)
  · intro h X Y f hf
    induction hf using AddSubgroup.closure_induction with
    | mem f hf =>
