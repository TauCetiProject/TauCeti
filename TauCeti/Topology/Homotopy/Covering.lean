...
through `p`: the source has trivial `π₁`, so the image of `FundamentalGroup.map f a₀` is trivial
and lands in any subgroup. This is the hypothesis of Mathlib's covering-space lifting criterion
