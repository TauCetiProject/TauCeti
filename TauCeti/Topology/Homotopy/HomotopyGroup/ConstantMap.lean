...
@[simp]
theorem map_const_continuousMap_apply (y : Y) (a : HomotopyGroup N X x) :
...
homotopy groups. -/
@[simp]
theorem mapHom_const_continuousMap [DecidableEq N] [Nonempty N] (y : Y) :
    mapHom (N := N) (x := x) (ContinuousMap.const X y) rfl = 1 := by
  ext a
  rw [mapHom_apply, map_const_continuousMap_apply]
...
theorem mapHom_eq_one_of_subsingleton [DecidableEq N] [Nonempty N] [Subsingleton Y]
  ...
  have h : f = ContinuousMap.const X y := Subsingleton.elim _ _
  subst f
  exact mapHom_const_continuousMap (x := x) y
