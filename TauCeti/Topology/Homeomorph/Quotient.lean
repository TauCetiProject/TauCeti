...
`Homeomorph.Quotient.congrRight`, the homeomorphisms between quotient spaces induced by a
relation-preserving homeomorphism and by an equivalence of relations, act on equivalence classes.

## Main results

* `Homeomorph.Quotient.congr_mk`: `congr e` sends the class of `x` to the class of `e x`.
...
/-- `Homeomorph.Quotient.congr e` sends the class of `x` to the class of `e x`. This is the
homeomorphism counterpart of Mathlib's `Quotient.congr_mk`. -/
@[simp]
theorem congr_mk {Y : Type*} [TopologicalSpace Y] {rX : Setoid X} {rY : Setoid Y} (e : X ≃ₜ Y)
