...
/-- **A local homeomorphism is a covering map over an open set on which it is proper.**  If
`f : E → X` is a local homeomorphism between Hausdorff spaces with `X` locally compact, and every
compact subset of the open set `s` has compact preimage, then `f` is a covering map over `s`. -/
theorem IsCoveringMapOn.of_isLocalHomeomorph_of_isCompact_preimage [T2Space E] [T2Space X]
    [LocallyCompactSpace X] (hs : IsOpen s) (hf : IsLocalHomeomorph f)
    (hK : ∀ K ⊆ s, IsCompact K → IsCompact (f ⁻¹' K)) : IsCoveringMapOn f s := by
  have hcont := hf.continuous
  have hfs : IsOpen (f ⁻¹' s) := hs.preimage hcont
  ...
  have hloc : IsLocalHomeomorph (s.restrictPreimage f) :=
    IsLocalHomeomorph.of_comp (g := Subtype.val)
      (hf.comp hfs.isOpenEmbedding_subtypeVal.isLocalHomeomorph)
      hs.isOpenEmbedding_subtypeVal.isLocalHomeomorph hcont.restrictPreimage
  -- ... and it is proper, hence a closed map with compact fibres.
  have hproper : IsProperMap (s.restrictPreimage f) := by
    refine isProperMap_iff_isCompact_preimage.mpr ⟨hcont.restrictPreimage, fun K hK' => ?_⟩
