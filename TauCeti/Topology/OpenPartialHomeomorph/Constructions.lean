...
/-!
# Restricting an open partial homeomorphism to a parametrized slice

An open partial homeomorphism restricts to a chart on a subtype when membership in the subtype is
detected by a parametrized coordinate slice. This file packages the common topological construction
used by boundary charts, subgroup slice charts, and regular-level-set charts.
...
Only continuity of `π` on the target is needed. Outside the target, the inverse uses the canonical
choice supplied by `Nonempty s`; its value there is irrelevant to an open partial homeomorphism. -/
noncomputable def subtypeCoord (e : OpenPartialHomeomorph X Y) (s : Set X) (hs : Nonempty s)
    ...
    (hπc : ContinuousOn π e.target) : OpenPartialHomeomorph s Z where
  toFun x := π (e x.1)
...
theorem subtypeCoord_apply (e : OpenPartialHomeomorph X Y) (s : Set X) (hs : Nonempty s)
    ...
    (hslice : ∀ {x}, x ∈ e.source → x ∈ s → ι (π (e x)) = e x)
    (hπι : Set.LeftInvOn π ι (ι ⁻¹' e.target)) (hιc : Continuous ι)
    (hπc : ContinuousOn π e.target) (x : s) :
