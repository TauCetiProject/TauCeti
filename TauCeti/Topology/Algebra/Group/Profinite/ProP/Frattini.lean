...
intersection of its open normal subgroups of index `p`. For a prime `p` it is the kernel of the
largest elementary abelian `p`-quotient of `G`, and for a pro-`p` group it is the Frattini
subgroup in the usual sense, the intersection of the maximal open subgroups; that last
identification needs the finite `p`-group theory of maximal subgroups and is not established
here.
...
and `proPFrattini p G` is trivial exactly when `G` itself already is one.
...
normal subgroups of `G` of index `p`. For a prime `p` its quotient is the largest elementary
abelian `p`-quotient of `G`. -/
