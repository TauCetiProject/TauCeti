...
intersection of its open normal subgroups of index `p`. It is the kernel of the largest
elementary abelian `p`-quotient of `G`, and for a pro-`p` group it is the Frattini subgroup in
the usual sense, the intersection of the maximal open subgroups; that last identification needs
the finite `p`-group theory of maximal subgroups and is not established here.
...
Two consequences are recorded because they are what consumers use: the Frattini quotient
`G ⧸ proPFrattini p G` is commutative of exponent dividing `p` — an `𝔽_p`-vector space — and
`proPFrattini p G` is trivial exactly when `G` itself already is one.
...
* `TauCeti.isMulCommutative_quotient_proPFrattini` and
  `TauCeti.exponent_quotient_proPFrattini_dvd`: the Frattini quotient is elementary abelian.
* `TauCeti.proPFrattini_eq_bot_iff`: the pro-`p` Frattini subgroup is trivial exactly when `G` is
  commutative of exponent dividing `p`.
* `TauCeti.map_proPFrattini_eq`: the pro-`p` Frattini subgroup is characteristic under continuous
  automorphisms.
...
normal subgroups of `G` of index `p`. Its quotient is the largest elementary abelian
`p`-quotient of `G`. -/
...
theorem mem_proPFrattini_iff {x : G} :
  ...
  rw [proPFrattini, Subgroup.mem_iInf]
...
/-- The Frattini quotient has exponent dividing `p`. Together with
...
`p`. -/
theorem proPFrattini_le_comap (f : G →* H) (hf : Continuous f)
...
theorem map_proPFrattini_le (f : G →* H) (hf : Continuous f) (hsurj : Function.Surjective f) :
  ...
  Subgroup.map_le_iff_le_comap.mpr (proPFrattini_le_comap f hf hsurj)
...
theorem map_proPFrattini_eq (e : G ≃ₜ* H) :
  ...
  refine le_antisymm (map_proPFrattini_le _ e.continuous e.surjective) fun x hx ↦ ?_
  have hsymm : e.symm x ∈ proPFrattini p G :=
    map_proPFrattini_le e.symm.toMulEquiv.toMonoidHom e.symm.continuous e.symm.surjective
...
theorem map_proPFrattini_eq_of_surjective (hp : p.Prime) (f : G →* H) (hf : Continuous f)
  ...
  refine le_antisymm (map_proPFrattini_le f hf hsurj) ?_
