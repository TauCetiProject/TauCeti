...
ordered pair of columns. `TauCeti.Matrix.pairMinor` names it, so that a family of such minors can be
indexed by pairs rather than by `Fin 2`-valued reindexing functions, and
`TauCeti.Matrix.pairMinor_mul` is the Cauchy--Binet expansion of a minor of a product whose middle
...
* `TauCeti.Matrix.pairMinor`: the `2 × 2` minor on an ordered row pair and an ordered column pair.

## Main results

* `TauCeti.Matrix.pairMinor_eq`: the minor written out as a difference of two products.
* `TauCeti.Matrix.pairMinor_map`: a ring morphism carries a minor to the minor of the mapped
  matrix.
* `TauCeti.Matrix.pairMinor_mul`: Cauchy--Binet, expanding a minor of a product over the six index
...
def pairMinor (g : _root_.Matrix m n R) (p : m × m) (q : n × n) : R :=
  (g.submatrix ![p.1, p.2] ![q.1, q.2]).det

/-- The `2 × 2` minor written out. -/
theorem pairMinor_eq (g : _root_.Matrix m n R) (p : m × m) (q : n × n) :
    pairMinor g p q = g p.1 q.1 * g p.2 q.2 - g p.1 q.2 * g p.2 q.1 := by
  rw [pairMinor, _root_.Matrix.det_fin_two]
  simp

/-- A ring morphism carries a minor to the minor of the mapped matrix. -/
@[simp]
theorem pairMinor_map {S : Type*} [CommRing S] (f : R →+* S) (g : _root_.Matrix m n R)
...
theorem pairMinor_mul (g : _root_.Matrix m (Fin 4) R) (h : _root_.Matrix (Fin 4) n R)
    (p : m × m) (q : n × n) :
    pairMinor (g * h) p q =
  ...
  simp only [pairMinor_eq, _root_.Matrix.mul_apply, Fin.sum_univ_four]
  ring
