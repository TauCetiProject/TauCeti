...
theorem coe_leviHom (A : GL (Fin m) R) :
  ...
  simpa only [coe_mulEquivGLSymplectic, coe_reindexGL, Equiv.symm_symm, Matrix.submatrix_apply,
...
theorem map_leviHom {S : Type*} [CommRing S] (f : R →+* S) (A : GL (Fin m) R) :
  ...
  have hmap :
    ...
    simp only [coe_mulEquivGLSymplectic, GLSymplecticFin.coe_map,
      GLSymplectic.coe_map, coe_reindexGL, Equiv.symm_symm, Matrix.GeneralLinearGroup.map_apply,
