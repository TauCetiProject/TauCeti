...
The file first records the rank-one system of `GL₂`, assembled from the rank-one Bruhat
decomposition, generation by `B` and `w = !![0, 1; 1, 0]`, the equality `B ∩ N = T`, and the
permutation quotient `N / T ≃ S₂`.

It then builds the system in every rank. `B` and `N` generate `GLₙ₊₁(k)` because every
transvection is either upper triangular or the conjugate of one by a permutation matrix, and the
Weyl quotient `N ⧸ (B ∩ N)` is the symmetric group, generated by the adjacent transpositions. The
Tits multiplication axiom `B s B w B ⊆ B s w B ∪ B w B` for `s = (a b)` with `b = a + 1` comes from
writing an element of `B` as `y x_{ab}(c)` with `y_{ab} = 0`, so that `s y s ∈ B`. When the
permutation of `w` keeps `a` before `b`, the transvection `x_{ab}(c)` moves past `w` into `B`.
Otherwise the rank-one identity `x_{ba}(c) ∈ B s x_{ab}(c⁻¹)` for `c ≠ 0` moves the product into
`B w B`. As a consequence, every element of `GLₙ₊₁(k)` lies in a Bruhat cell `B τ B` of a
permutation matrix.
...
* `TauCeti.transvectionUnit_mem_upperTriangularGroup`: an upper transvection is upper triangular.
...
* `TauCeti.glTitsSystem_mem_intersection`: its subgroup `B ∩ N` is the diagonal torus inside `N`.
...
variable {R : Type u} [CommRing R] {ι : Type*} [Fintype ι] [LinearOrder ι]

/-- A transvection `x_{ij}(c)` with `i < j` is upper triangular. -/
theorem transvectionUnit_mem_upperTriangularGroup {i j : ι} (hij : i < j) (c : R) :
    transvectionUnit hij.ne c ∈ upperTriangularGroup ι R := by
...
/-- In the standard `GLₙ₊₁` Tits system, membership in `B ∩ N` is exactly membership in the
diagonal torus. -/
theorem glTitsSystem_mem_intersection (w : (glTitsSystem k n).subgroupN) :
    w ∈ (glTitsSystem k n).intersection ↔ (w : GL (Fin (n + 1)) k) ∈ diagonalTorus k (n + 1) := by
  have hwN : (w : GL (Fin (n + 1)) k) ∈ GLDiagonalNormalizer k (n + 1) := w.property
  rw [TitsSystem.mem_intersection]
  constructor
  · intro hw
    rw [glTitsSystem_subgroupB] at hw
    exact UpperTriangularGroup.mem_diagonalTorus_of_mem ⟨_, hwN⟩ hw
  · intro hw
    rw [glTitsSystem_subgroupB]
    exact UpperTriangularGroup.diagonalTorus_le hw
