...
diagonalized over `k` with unit eigenvalues in `Q`, these eigenspaces span the vector space over
`k`.
...
def rationalEigenspace {n : Type*} [Fintype n] (M : Matrix n n Q) (s : Qˣ) :
    Submodule k (n → k) :=
...
@[simp]
theorem mem_rationalEigenspace {n : Type*} [Fintype n] {M : Matrix n n Q} {s : Qˣ}
    {v : n → k} :
...
/-- If `P₀` diagonalizes `M` with unit eigenvalues, its rational eigenspaces span because they
contain the columns of `P₀`. -/
theorem iSup_rationalEigenspace_eq_top {n : Type*} [Fintype n] [DecidableEq n]
