...
/-!
# Operators of order two and three: the Choie–Zagier criterion, Popa–Zagier's projection, exactness
...
(their (5)). This file records the linear algebra behind their Lemmas 1 and 3 and behind the
exactness step in the proof of their §3 Theorem 2, for endomorphisms `σ` and `υ` of a module `M`
with `σ ^ 2 = 1` and `υ ^ 3 = 1`, which are the left multiplications by `S` and `U`; the right
multiplications by `S` and `U` are further such endomorphisms `σ'` and `υ'`, commuting with `σ`
and `υ`. The ideal `𝓘` becomes the submodule `range (1 + σ) ⊔ range (1 + υ + υ ^ 2)`: when `2`
...
* `TauCeti.End.mem_sup_range_one_add_mul_one_sub`: **exactness in Popa–Zagier's §3 Theorem 2**:
  ...
  `ℬ` whose right coset sums vanish lies in `𝒥 = π_S ℛ (1 - π_S) + π_U ℛ (1 - π_U)`.
...
/-- **Exactness in Popa–Zagier's §3 Theorem 2**: let `σ, σ'` be of order two and `υ, υ'` of order
three, with `σ, υ` commuting with `σ', υ'`, `2` and `3` invertible and `ker (1 + σ')`,
...
and that `ξ` lies in `ℬ`, and the conclusion is `ξ ∈ 𝒥 = π_S ℛ (1 - π_S) + π_U ℛ (1 - π_U)`. -/
theorem mem_sup_range_one_add_mul_one_sub [Invertible (2 : R)] [Invertible (3 : R)]
  ...
  have hc : ∀ {a b : End R M}, Commute a b → Commute (1 - a) (1 + b) ∧
      Commute (1 - a) (1 + b + b ^ 2) := fun h ↦
    have h' := (Commute.one_left _).sub_left h
    ⟨(Commute.one_right _).add_right h', ((Commute.one_right _).add_right h').add_right
      (h'.pow_right 2)⟩
  obtain ⟨-, ⟨⟨a, rfl⟩, hy⟩, -, ⟨⟨b, rfl⟩, hz⟩, rfl⟩ := Submodule.mem_sup.1 <|
    mem_inf_ker_sup_inf_ker_of_mem_sup (hc hσσ').1 (hc hσυ').2 (hc hυσ').1 (hc hυυ').2 hacyc
  ...
  rw [SetLike.mem_coe, mem_ker, sub_apply, End.one_apply, sub_eq_zero, eq_comm] at hy hz
  -- `(1 - σ') a` is fixed by `σ`, so it is `(1 + σ) (1 - σ') (a / 2)`; `(1 - υ') b` is fixed by
  -- `υ`, so it is `(1 + υ + υ²) (1 - υ') (b / 3)`
  have h3 : (1 + υ + υ ^ 2 : End R M) ((1 - υ') b) = (3 : R) • (1 - υ') b := by
    generalize (1 - υ') b = z at hz ⊢
    simp [sq, hz, ← two_add_one_eq_three, add_smul, two_smul]
  refine Submodule.add_mem_sup ⟨(⅟2 : R) • a, ?_⟩ ⟨(⅟3 : R) • b, ?_⟩ <;>
    rw [End.mul_apply, map_smul, map_smul]
  · rw [add_apply, End.one_apply, hy, ← two_smul R, invOf_smul_smul]
  · rw [h3, invOf_smul_smul]
