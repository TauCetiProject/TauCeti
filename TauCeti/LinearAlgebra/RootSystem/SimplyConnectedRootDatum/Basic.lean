...
* `TauCeti.sub_mem_of_consecutive_sub_mem_fin` and `TauCeti.add_mem_of_consecutive_sub_mem_fin`:
  the same telescoping for a `Fin`-indexed family, and its companion for sums, which also uses
  twice the last member.
...
theorem sub_mem_of_consecutive_sub_mem_fin {m : ℕ} (S : AddSubmonoid M) (v : Fin m → M)
...
theorem add_mem_of_consecutive_sub_mem_fin {m : ℕ} (S : AddSubmonoid M) (v : Fin m → M)
  ...
  · exact sub_mem_of_consecutive_sub_mem_fin S v hS (Fin.le_def.2 (by simp [c]; omega))
  · exact sub_mem_of_consecutive_sub_mem_fin S v hS (Fin.le_def.2 (by simp [c]; omega))
