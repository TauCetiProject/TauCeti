...
coordinates are nonnegative. This fixes the Bourbaki sign convention used by the compatible Borel:
the positive nilradical is generated by the root bivectors whose type-D coordinates have this sign.
-/
theorem mem_posRoots_typeDSimplyConnectedBase_iff_nonneg (hn : 4 ≤ n)
  ...
  let e : Fin n ≃ (typeDSimplyConnectedBase n hn).support :=
  ...
  have he_apply (i : Fin n) : (e i : Fin (2 * n * (n - 1))) = typeDSimpleIndex n hn i := by
    rfl
  have hcoeff : ∑ j ∈ (typeDSimplyConnectedBase n hn).support, coeff j =
      ∑ i : Fin n, typeDSimpleRootCoordinates n hn (typeDRootEquiv n hn k) i := by
    calc
      _ = ∑ j : (typeDSimplyConnectedBase n hn).support, coeff j := by
        simpa using Finset.sum_subtype (typeDSimplyConnectedBase n hn).support
          ...
          (fun j => coeff j)
      _ = ∑ i : Fin n, coeff (e i : Fin (2 * n * (n - 1))) :=
        (e.sum_comp (fun j => coeff (j : Fin (2 * n * (n - 1))))).symm
      _ = _ := by
  ...
  have hroot : (typeDSimplyConnectedRootDatum n hn).root k =
    ...
    calc
      _ = ∑ i : Fin n, typeDSimpleRootCoordinates n hn
          (typeDRootEquiv n hn k) i •
            (typeDSimplyConnectedRootDatum n hn).root (typeDSimpleIndex n hn i) :=
        (sum_smul_root_typeDSimpleIndex hn k).symm
      _ = ∑ i : Fin n, coeff (e i : Fin (2 * n * (n - 1))) •
          (typeDSimplyConnectedRootDatum n hn).root (e i : Fin (2 * n * (n - 1))) := by
        apply Finset.sum_congr rfl
        intro i hi
        rw [he_apply]
        simp [coeff]
      _ = ∑ j : (typeDSimplyConnectedBase n hn).support,
          coeff j • (typeDSimplyConnectedRootDatum n hn).root j :=
        e.sum_comp (fun j => coeff (j : Fin (2 * n * (n - 1))) •
          (typeDSimplyConnectedRootDatum n hn).root (j : Fin (2 * n * (n - 1))))
      _ = _ := by
        simpa using (Finset.sum_subtype (typeDSimplyConnectedBase n hn).support
          (p := fun j => j ∈ (typeDSimplyConnectedBase n hn).support) (by simp)
          (fun j => coeff j • (typeDSimplyConnectedRootDatum n hn).root j)).symm
  have hne_sum : (∑ i : Fin n, typeDSimpleRootCoordinates n hn
      (typeDRootEquiv n hn k) i) ≠ 0 := by
    intro hz
    apply (typeDSimplyConnectedBase n hn).height_ne_zero k
    rw [(typeDSimplyConnectedBase n hn).height_eq_sum hroot, hcoeff, hz]
  rw [mem_posRoots, RootPairing.Base.isPos_iff, (typeDSimplyConnectedBase n hn).height_eq_sum hroot,
    hcoeff]
  constructor
  · intro hk
  ...
  · intro hnonneg
    obtain hnonneg' | hnonpos :=
      typeDSimpleRootCoordinates_nonneg_or_nonpos hn (typeDRootEquiv n hn k)
    · exact lt_of_le_of_ne (Finset.sum_nonneg fun i _ => hnonneg' i) (Ne.symm hne_sum)
    · exfalso
      have hsum_nonpos : (∑ i : Fin n, typeDSimpleRootCoordinates n hn
          (typeDRootEquiv n hn k) i) ≤ 0 := Finset.sum_nonpos fun i _ => hnonpos i
      exact hne_sum (le_antisymm hsum_nonpos
