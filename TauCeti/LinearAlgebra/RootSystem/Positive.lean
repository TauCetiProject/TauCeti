...
omit [CharZero R] in
/-- The positive root cone is the additive submonoid generated by the simple roots. -/
theorem posRootCone_def : posRootCone P b = AddSubmonoid.closure (P.root '' b.support) :=
  (rfl)
...
/-- **A root is positive exactly when it lies in the positive root cone.** The forward direction
is `TauCeti.root_mem_posRootCone_of_mem_posRoots`; conversely, a nonnegative integer combination of
the simple roots has nonnegative height. -/
