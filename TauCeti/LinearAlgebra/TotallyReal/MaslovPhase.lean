...
* `Submodule.maslovPhase`: the Maslov phase `ρ(L₀, L)`.
* `TauCeti.IsMaximalTotallyReal.maslovPhase_map`: `ρ(L₀, A L₀) = det A / conj (det A)`.
* `Submodule.norm_maslovPhase`: the Maslov phase has modulus one.
...
open Classical in
/-- The **Maslov phase** `ρ(L₀, L)` of a real subspace `L` of a complex module `E` relative to a
real subspace `L₀`. When `L₀` is maximal totally real and `L = A L₀` for a complex-linear
automorphism `A` of `E`, it is `det A / conj (det A) = det A ^ 2 / |det A| ^ 2`, a complex number
of modulus one which does not depend on the choice of `A`
(`TauCeti.IsMaximalTotallyReal.maslovPhase_map`). Every maximal totally real `L` is of this form
(`TauCeti.IsMaximalTotallyReal.exists_linearEquiv_map_eq`). If no such `A` exists, the phase is
`1` by convention. -/
noncomputable def maslovPhase (L₀ L : Submodule ℝ E) : ℂ :=
  if h : ∃ A : E ≃ₗ[ℂ] E, L₀.map ((A : E →ₗ[ℂ] E).restrictScalars ℝ) = L then
    LinearMap.det (h.choose : E →ₗ[ℂ] E) / conj (LinearMap.det (h.choose : E →ₗ[ℂ] E))
  else 1

/-- The Maslov phase has modulus one. -/
@[simp]
theorem norm_maslovPhase (L₀ L : Submodule ℝ E) : ‖L₀.maslovPhase L‖ = 1 := by
  unfold maslovPhase
  split_ifs with h
  · have h0 : LinearMap.det (h.choose : E →ₗ[ℂ] E) ≠ 0 := by
      rw [← LinearEquiv.coe_det]
      exact (LinearEquiv.det h.choose).ne_zero
    rw [norm_div, Complex.norm_conj, div_self (norm_ne_zero_iff.2 h0)]
  · exact norm_one

/-- The Maslov phase is nonzero. -/
theorem maslovPhase_ne_zero (L₀ L : Submodule ℝ E) : L₀.maslovPhase L ≠ 0 :=
...
theorem maslovPhase_map (A : E ≃ₗ[ℂ] E) :
    L.maslovPhase (L.map ((A : E →ₗ[ℂ] E).restrictScalars ℝ)) =
      LinearMap.det (A : E →ₗ[ℂ] E) / conj (LinearMap.det (A : E →ₗ[ℂ] E)) := by
  unfold Submodule.maslovPhase
  split_ifs with h
  · exact hL.det_div_conj_eq_of_map_eq h.choose_spec
  · exact (h ⟨A, rfl⟩).elim

/-- A maximal totally real subspace has Maslov phase one relative to itself. -/
theorem maslovPhase_self : L.maslovPhase L = 1 := by
...
theorem maslovPhase_mul_maslovPhase {L₁ L₂ : Submodule ℝ E}
    (hL₁ : IsMaximalTotallyReal ((LinearMap.lsmul ℂ E Complex.I).restrictScalars ℝ) L₁)
    (hL₂ : IsMaximalTotallyReal ((LinearMap.lsmul ℂ E Complex.I).restrictScalars ℝ) L₂) :
    L.maslovPhase L₁ * L₁.maslovPhase L₂ = L.maslovPhase L₂ := by
  ...
  rw [hL.maslovPhase_map, hL₁.maslovPhase_map, hAB, hL.maslovPhase_map, LinearEquiv.coe_trans,
    LinearMap.det_comp, map_mul, mul_div_mul_comm, mul_comm]

/-- Exchanging the two subspaces inverts the Maslov phase. -/
theorem maslovPhase_symm {L₁ : Submodule ℝ E}
    (hL₁ : IsMaximalTotallyReal ((LinearMap.lsmul ℂ E Complex.I).restrictScalars ℝ) L₁) :
    L₁.maslovPhase L = (L.maslovPhase L₁)⁻¹ := by
...
theorem maslovPhase_map_map {L₁ : Submodule ℝ E}
    (hL₁ : IsMaximalTotallyReal ((LinearMap.lsmul ℂ E Complex.I).restrictScalars ℝ) L₁)
    (B : E ≃ₗ[ℂ] E) :
    (L.map ((B : E →ₗ[ℂ] E).restrictScalars ℝ)).maslovPhase
        (L₁.map ((B : E →ₗ[ℂ] E).restrictScalars ℝ)) = L.maslovPhase L₁ := by
  obtain ⟨A, rfl⟩ := hL.exists_linearEquiv_map_eq hL₁
  ...
  rw [hconj, (hL.map_linearEquiv B).maslovPhase_map, hdet, hL.maslovPhase_map]
...
theorem maslovPhase_map_lsmul {z : ℂ} (hz : z ≠ 0) :
    L.maslovPhase (L.map ((LinearMap.lsmul ℂ E z).restrictScalars ℝ)) =
  ...
  have hsmul : LinearMap.lsmul ℂ E z = z • LinearMap.id := by
    ext x
    simp
  have h := hL.maslovPhase_map (LinearEquiv.smulOfNeZero ℂ E z hz)
  rw [hA, hsmul, LinearMap.det_smul, LinearMap.det_id, mul_one, map_pow, ← div_pow] at h
  rw [hsmul, h]
...
theorem maslovPhase_map_I :
    L.maslovPhase (L.map ((LinearMap.lsmul ℂ E Complex.I).restrictScalars ℝ)) =
      (-1) ^ finrank ℂ E := by
