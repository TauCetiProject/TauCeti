...
/-- A permutation step is a diagonal chain, since it is generated by binary steps. -/
theorem permutation {w w' : Fin n → Rˣ} (h : PermutationStep w w') : DiagonalChain w w' := by
  change Relation.ReflTransGen DiagonalStep w w'
  exact (Relation.ReflTransGen.mono (r := BinaryStep) (p := DiagonalStep)
    (fun _ _ hb => Or.inr hb)) w w' h.to_reflTransGen_binaryStep
...
theorem induction_on {w w' : Fin n → Rˣ} (h : DiagonalChain w w')
  ...
  induction h with
  | refl => exact hw
  | tail _ hstep' ih => exact hstep hstep' ih
