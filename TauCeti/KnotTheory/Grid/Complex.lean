...
* `TauCeti.GridChain.relabelEquiv`: the linear automorphism of grid chains relabeling each
  generator along an equivalence of grid states.
...
/-- The linear automorphism of grid chains induced by relabeling each generator along an
equivalence `e` of grid states.

The diagonal-reflection and half-turn-rotation relabelings `transposeEquiv` and `rotateEquiv` are
the special cases where `e` is the involution `GridState.transpose` or `GridState.rotate`. -/
noncomputable def relabelEquiv {R : Type*} [Semiring R] {n : ℕ} (e : GridState n ≃ GridState n) :
    GridChain R n ≃ₗ[R] GridChain R n :=
  Finsupp.domLCongr e

/-- The relabeling along `e` sends the generator `x` to the generator `e x`. -/
theorem relabelEquiv_single {R : Type*} [Semiring R] {n : ℕ} (e : GridState n ≃ GridState n)
    (x : GridState n) (a : R) :
    relabelEquiv e (Finsupp.single x a) = Finsupp.single (e x) a := by
  unfold relabelEquiv
  rw [Finsupp.domLCongr_single]

/-- The `y`-coefficient of a chain relabeled along `e` is the `e.symm y`-coefficient of the
original chain. -/
theorem relabelEquiv_apply {R : Type*} [Semiring R] {n : ℕ} (e : GridState n ≃ GridState n)
...
theorem transposeEquiv_single {R : Type*} [Semiring R] {n : ℕ} (x : GridState n) (a : R) :
  ...
  exact relabelEquiv_single _ x a
...
theorem transposeEquiv_apply {R : Type*} [Semiring R] {n : ℕ} (f : GridChain R n)
  ...
  unfold transposeEquiv
  exact relabelEquiv_apply _ f y
...
theorem transposeEquiv_symm {R : Type*} [Semiring R] {n : ℕ} :
  ...
  unfold transposeEquiv relabelEquiv
...
theorem rotateEquiv_symm {R : Type*} [Semiring R] {n : ℕ} :
  ...
  unfold rotateEquiv relabelEquiv
  rw [Finsupp.domLCongr_symm]
  rfl
