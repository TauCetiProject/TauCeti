...
@[simp]
theorem relabelEquiv_symm {R : Type*} [Semiring R] {n : ℕ} (e : GridState n ≃ GridState n) :
    (relabelEquiv (R := R) e).symm = relabelEquiv e.symm := by
  rw [relabelEquiv, Finsupp.domLCongr_symm]
  rfl
...
theorem transposeEquiv_symm {R : Type*} [Semiring R] {n : ℕ} :
    (transposeEquiv R n).symm = transposeEquiv R n :=
  relabelEquiv_symm _
...
theorem rotateEquiv_symm {R : Type*} [Semiring R] {n : ℕ} :
    (rotateEquiv R n).symm = rotateEquiv R n :=
  relabelEquiv_symm _
