...
Each variable `V_c` has Alexander degree `-1` on `GC⁻`, hence on `GH⁻`. On the grid homology of a
knot every variable acts as `U` (`TauCeti.GridDiagram.IsKnot.X_smul_unblockedHomology`), so `U`
lowers the Alexander grading by one. This is the graded `R[U]`-module structure in which the
concordance invariant `τ` is minus the maximal Alexander grading of a homogeneous non-torsion
element (`TauCeti.InternalGrading.supNonTorsionDegree`).
...
variable {n : ℕ} (G : OddComponentGridDiagram n) (R : Type*) [CommRing R]
...
noncomputable def alexanderHomologyGrading :
    InternalGrading R (G.1.unblockedDifferential R).homology :=
    ...
    y ∈ (G.alexanderHomologyGrading R).piece a ↔
      ∃ z : LinearMap.ker (G.1.unblockedDifferential R),
        (z : GridChainMinus R n) ∈ G.alexanderChainMinusPiece R a ∧
          (G.1.unblockedDifferential R).homologyπ z = y := by
...
theorem X_smul_mem_alexanderHomologyGrading_piece (i : Fin n) {a : ℤ}
  ...
  rw [sub_eq_add_neg]
  refine (G.alexanderChainMinusGrading R).smul_mem_homology_piece _ (fun p x hx ↦ ?_) hy
