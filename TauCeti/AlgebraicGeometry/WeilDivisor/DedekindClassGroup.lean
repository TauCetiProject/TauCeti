...
`TauCeti.AlgebraicGeometry.WeilDivisor.Dedekind` packages the height-one spectrum of `R` as the
points of an affine curve and the order of vanishing of a rational function as the order system
`OrderSystem.ofDedekindDomain R K`; its class group
`(OrderSystem.ofDedekindDomain R K).ClassGroup` is the free Weil-divisor group on the height-one
primes modulo principal divisors.
`TauCeti.AlgebraicGeometry.WeilDivisor.FractionalIdealDivisor` adds the Cartier side, the
isomorphism `fractionalIdealDivisorAddEquiv` between invertible fractional ideals and Weil
divisors, and records that it carries principal fractional ideals to principal divisors. Both
files explicitly leave the **quotient-level** comparison open. This file supplies it.
