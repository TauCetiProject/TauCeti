...
`TauCeti.AlgebraicGeometry.WeilDivisor` file: since the point divisors generate the free abelian
group of Weil divisors and `weightedDegree w (ofPoint x) = w x`, the image is the subgroup of `ℤ`
