...
at the penultimate component.  When the numerical type has components outside the fork, every
component in the fork has the same weight, the displayed intersections equal that weight, and
all other intersections between the branch and the chain vanish.  Together with the chain's
no-chord theorem, this identifies the induced intersection graph with a simply-laced fork.
...
* `TauCeti.NumericalType.IsSelfIntersectionMinusTwoFork.exists_weight_intersection_eq`: the
  weights and intersections of a proper fork are all the simply-laced ones.
-/
...
second leaf `branch` meeting its penultimate component `c (t - 2)`. -/
...
/-- The extra leaf of a fork meets no chain component other than the penultimate one, provided
the fork is a proper subset of the components. -/
lemma branch_intersection_eq_zero (hf : T.IsSelfIntersectionMinusTwoFork t c branch)
    (hcard : t + 1 < Fintype.card T.Component) (ht : 4 < t) {i : ℕ} (hi : i < t)
    (hne : i ≠ t - 2) : T.intersection (c i) branch = 0 := by
...
/-- A proper fork of at least six components is simply laced: all its component weights agree,
each displayed intersection is that common weight, and the extra leaf has no other intersection
with the chain.  Together with
...
theorem exists_weight_intersection_eq (hf : T.IsSelfIntersectionMinusTwoFork t c branch)
  ...
  have hbranchZero : ∀ i < t, i ≠ t - 2 → T.intersection (c i) branch = 0 :=
    fun i hi hne ↦ hf.branch_intersection_eq_zero hcard ht hi hne
  ...
  have hleft : (T.weight (c 0) : ℤ) = w := by
    ...
    · exfalso
      ...
      have hrow : ∀ i < t + 1,
        ...
        intro i hi
        rw [show t + 1 = t + 1 from rfl, sum_range_succ]
        by_cases hit : i = t
        · subst i
          rw [hd_t, hyt]
