...
* `TauCeti.NumericalType.sum_sum_intersection_mul_neg`: the same statement for a vector supported
  on a finite set of components which is not all of them.
...
/-- The intersection form evaluated at a vector supported in a finite set of components. -/
private lemma dotProduct_intersection_mulVec_eq_sum {s : Finset T.Component}
    {x : T.Component → ℤ} (hx : ∀ i ∉ s, x i = 0) :
    x ⬝ᵥ T.intersection *ᵥ x = ∑ i ∈ s, ∑ j ∈ s, T.intersection i j * x i * x j := by
  have hrow (i : T.Component) : (T.intersection *ᵥ x) i = ∑ j ∈ s, T.intersection i j * x j := by
    rw [mulVec, dotProduct]
    exact (sum_subset (subset_univ s) fun j _ hj ↦ by rw [hx j hj, mul_zero]).symm
...
theorem sum_sum_intersection_mul_neg {s : Finset T.Component} (hs : s ≠ univ)
  ...
  rwa [T.dotProduct_intersection_mulVec_eq_sum hx, heq] at hneg
