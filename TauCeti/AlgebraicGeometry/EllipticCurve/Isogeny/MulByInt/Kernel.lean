...
theorem mem_ker_mulByIntIsogeny_iff {n : ℤ} (hn : psiFunctionField W n ≠ 0)
  ...
  -- `smul_add` does not apply: the `zsmul` here arrives through `map_zsmul` as
  -- `SubNegMonoid.toZSMul`, not the `Module ℤ` action that lemma is stated for, so its pattern
  -- does not match. `module` sees through the two paths.
  have hsmul : n • (W.genericPoint + Point.baseChange (W' := W) F W.FunctionField P) =
      n • W.genericPoint + n • Point.baseChange (W' := W) F W.FunctionField P := by
    module
