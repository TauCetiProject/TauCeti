...
and nothing more. Of the *target* algebra, `algHom_ext` and `algHom_injective` ask for no
commutativity: the first says `R[W]` is generated by its two coordinate functions, the second
argues with a norm taken in `R[W]`, so both constrain the source alone. The evaluation results
stay over a commutative target — they evaluate a bivariate polynomial in it, and those naming the
base-changed curve `W⁄A` need it to be a Weierstrass curve over `A` as well. `algHom_injective`
