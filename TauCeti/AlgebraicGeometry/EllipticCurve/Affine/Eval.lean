...
and nothing more. The *target* algebra need only be commutative where the statement mentions the
base-changed curve `W⁄A`, which is a Weierstrass curve over `A`; `algHom_ext` says instead that
`R[W]` is generated by its two coordinate functions, so it holds for any target.
