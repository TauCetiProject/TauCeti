...
A globally minimal equation has trivial defect ideal, which gives one direction. Conversely,
start from an integral model `W` whose defect ideal is principal, generated by `g`. At each
height-one prime `v` Mathlib supplies a change of variables `Cᵥ = (uᵥ, rᵥ, sᵥ, tᵥ)` carrying `W`
...
primes where `fᵥ(W) = 0` the same conditions hold for free, `r`, `s`, `t` and `rᵥ`, `sᵥ`, `tᵥ`
all being integral there.
