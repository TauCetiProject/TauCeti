...
* `WeierstrassCurve.VariableChange.exists_units_algebraMap_eq_u_of_isMinimal_smul`: the scaling
...
`VariableChange.exists_units_algebraMap_eq_u_of_isMinimal_smul` supply the unit needed for descent.
...
theorem HasSplitMultiplicativeReduction.of_isMinimal_smul {W₁ W₂ : WeierstrassCurve K}
  ...
  obtain ⟨u₀, hau⟩ := VariableChange.exists_units_algebraMap_eq_u_of_isMinimal_smul R D hD
