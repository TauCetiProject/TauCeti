...
* `WeierstrassCurve.exists_units_algebraMap_eq_u_of_isMinimal_smul`: that scaling factor is the
  image of a unit of the discrete valuation ring.
...
`exists_units_algebraMap_eq_u_of_isMinimal_smul` supply the unit needed for descent.
...
theorem exists_units_algebraMap_eq_u_of_isMinimal_smul {W₁ W₂ : WeierstrassCurve K}
...
theorem HasSplitMultiplicativeReduction.of_isMinimal_smul {W₁ W₂ : WeierstrassCurve K}
  ...
  obtain ⟨u₀, hau⟩ := exists_units_algebraMap_eq_u_of_isMinimal_smul R D hD
